...
"""902_city_case をゲーム抜きで通す。
    ...
    python tools/tests/test_wip_city_case.py
...
def find_mod(suffix):
    # 普段は `tools/tests/` に居るので `runtime/mods` から探す。
    # 隣に `mod.json` が在るときはそちらを優先する。
    # MOD と同じフォルダへ置いた形（`local/` や `discontinued/` に居るあいだ）でも
    # そのまま走るようにしておく。
...
def press_mark(ctx, app, mark):
    """印でボタンを押す。

    根拠のボタンは文言が事件ごとに変わる（AI が書いた事実の文が入るうえ、
    長ければ畳まれる）ので、文字列では指せない。
    印は材料の id を持っているのでこちらで指す。
    """
    for index, entry in enumerate(app.buttons):
        if entry.get(mod.MARK) == mark:
            hook = ctx.hooks["__main__:InstantaleApp.on_button_press"]
            return hook(lambda self, i, *a, **k: None, app, index)
    raise AssertionError("no button marked {!r} in {}".format(
        mark, [(e.get("text"), e.get(mod.MARK)) for e in app.buttons]))


def proves(case, npc_id):
    """その相手1人に絞れる根拠の組を、控えのデータから**独立に**組む。

    MOD の `case.evidence` も `case.judge` も呼ばない。
    呼ぶと「自分と同じか」を確かめるだけの検査になる（`NPC_FIELD_ORDER` と同じ考え方）。

    ここが使う規則は2つだけで、どちらも控えを読めば分かる:

      手がかり … `eliminates` に載っている者が消える
      言い分   … 犯人以外は裏が取れるので、語った本人が消える

    材料は多くて7つ（手がかり3・言い分4）なので部分集合を全部試す。
    いちばん少ない組を返す。
    """
    ids = [str(s["id"]) for s in case.get("suspects", [])]
    culprit = str(case.get("culprit"))
    pool = []
    for clue in case.get("clues", []):
        if clue.get("found"):
            pool.append((str(clue.get("id")),
                         {str(n) for n in clue.get("eliminates", [])}))
    for suspect in case.get("suspects", []):
        if suspect.get("heard"):
            sid = str(suspect["id"])
            pool.append(("s" + sid, set() if sid == culprit else {sid}))
    want = [str(npc_id)]
    for size in range(1, len(pool) + 1):
        for combo in itertools.combinations(pool, size):
            gone = set()
            for _pid, drops in combo:
                gone |= drops
            if [i for i in ids if i not in gone] == want:
                return [pid for pid, _drops in combo]
    return None


def accuse(ctx, app, npc_id, picks=None, submit=True):
    """告発する。**自前のボタンだけ**（ゲームの場面を使わない）。

    画面は2段。
    「犯人を告発する」で容疑者が並び、1人を押すと根拠の材料が並ぶ。
    根拠を1つ以上挙げてから「この根拠で告発する」で決着する。

    `picks` を省略すると、控えから**1人に絞れる最小の組**を組んで挙げる
    （`proves`）。
    組めなければ何も挙げずに戻る ― 挙げられないこと自体を見たい検査があるため。
    `submit=False` なら根拠を挙げたところで止める。
    """
    app.facility_screen()
    refresh_for(ctx, app)
    ...
    press(ctx, app, label[0])
    refresh_for(ctx, app)
    if picks is None:
        picks = proves(case_of(ctx), npc_id) or []
    for pick in picks:
        press_mark(ctx, app, mod.PICK_MARK_PREFIX + str(pick))
        refresh_for(ctx, app)
    if not submit:
        return None
    if mod.SUBMIT_LABEL not in app.labels():
        raise AssertionError(
            "根拠を挙げていないので告発できない: picks={} labels={}".format(
                picks, app.labels()))
    return press(ctx, app, mod.SUBMIT_LABEL)


def learn_all(ctx, app, case):
    """手がかりと言い分を全部集める。

    証言者と容疑者に1人ずつ話しかけて抜けるだけ。
    告発に根拠が要るようになったので、
    決着まで通す検査はここを通ってから告発する。
    """
    for clue in case.get("clues", []):
        witness = clue.get("witness")
        if witness is None:
            continue
        talk_to(ctx, app, witness)
        end_conversation_with(ctx, app, witness)
    for suspect in case.get("suspects", []):
        talk_to(ctx, app, suspect["id"])
        end_conversation_with(ctx, app, suspect["id"])
    return case_of(ctx)
...
# import されていないと「調べていることは無い」と出て詰む作りだった。
press(ctx, app, mod.ACCUSE_LABEL)
refresh()
...
print("\n[告発] 判定は根拠を先に見る（結果が答えを漏らさない）")
# 「相手は合っているが根拠が足りない」を独立した結果にすると、
# **その結果が出たこと自体が「相手は合っている」と教えてしまう**。
# 名前を1人ずつ試せば犯人が割れる。
# だから先に根拠だけを見て、1人に絞れていなければ誰を名指ししていても同じ結果にする。
#
# ここは控えを手で組んで、判定だけを確かめる（画面もゲームも通さない）。
probe = case_mod.build(
    "世界", "0", "1",
    [{"id": "1", "tell": "黒髪", "claim": "ひとりで歩いていた"},
     {"id": "2", "tell": "淡い髪", "claim": "その晩は酒場の隅で飲んでいた"},
     {"id": "3", "tell": "淡い髪", "claim": "その晩は門の当番に立っていた"}],
    [{"id": "c1", "label": "clue_c1", "at_type": GUILD, "kind": "trait",
      "wrong": False, "fact": "犯人の髪は黒かった", "eliminates": ["2"],
      "witness": "10"}],
    100)
probe["clues"][0]["found"] = True
check("**足りない根拠は、誰を名指ししても同じ結果になる**",
      case_mod.judge(probe, "1", ["c1"]) == case_mod.THIN
      and case_mod.judge(probe, "3", ["c1"]) == case_mod.THIN,
      (case_mod.judge(probe, "1", ["c1"]),
       case_mod.judge(probe, "3", ["c1"])))
check("根拠を1つも挙げなければ通らない",
      case_mod.judge(probe, "1", []) == case_mod.THIN,
      case_mod.remaining_with(probe, []))

for member in probe["suspects"]:
    member["heard"] = True
check("**言い分は語った本人を消す**（裏が取れるので）",
      case_mod.remaining_with(probe, ["s2"]) == ["1", "3"],
      case_mod.remaining_with(probe, ["s2"]))
check("**犯人の言い分では誰も消えない**（誰にも裏が取れない場所を挙げるため）",
      case_mod.remaining_with(probe, ["s1"]) == ["1", "2", "3"],
      case_mod.remaining_with(probe, ["s1"]))
check("**言い分だけでも犯人に辿り着ける**（手がかりを増やさずに根拠が揃う）",
      case_mod.judge(probe, "1", ["s2", "s3"]) == case_mod.SOLVED,
      case_mod.remaining_with(probe, ["s2", "s3"]))
check("手がかりと言い分を混ぜても同じ",
      case_mod.judge(probe, "1", ["c1", "s3"]) == case_mod.SOLVED,
      case_mod.remaining_with(probe, ["c1", "s3"]))
check("**材料は手がかりと言い分の両方**",
      sorted(case_mod.evidence_ids(probe)) == ["c1", "s1", "s2", "s3"],
      case_mod.evidence_ids(probe))
# 真の材料だけでは無実の者に辿り着けない。
# 事実は犯人についてのものなので犯人を消さないし、言い分も犯人のものは誰も消さない。
check("**真の材料だけでは誰も誤認できない**",
      all(case_mod.judge(probe, "2", picks) != case_mod.MISTAKEN
          for picks in (["c1"], ["s2"], ["s3"], ["c1", "s3"],
                        ["c1", "s2", "s3"])),
      [case_mod.judge(probe, "2", p) for p in
       (["c1"], ["s2"], ["s3"], ["c1", "s3"])])
liar_probe = json.loads(json.dumps(probe))
liar_probe["clues"][0]["eliminates"] = ["1"]     # 犯人を消してしまう証言
liar_probe["clues"][0]["wrong"] = True
check("**思い違いを信じたときだけ誤認になる**",
      case_mod.judge(liar_probe, "3", ["c1", "s2"]) == case_mod.MISTAKEN,
      case_mod.remaining_with(liar_probe, ["c1", "s2"]))

print("\n[告発] 相手と根拠の2段で答える")
ctxe = FakeCtx()
clean_record(ctxe)
mod.apply(ctxe)
appe, placese = fresh_world()
refreshe = install(ctxe, appe)
appe.facility_screen()
refreshe()
press(ctxe, appe, mod.START_LABEL)
founde = case_of(ctxe)
culprite = founde["culprit"]


def open_accusation(npc_id):
    """告発を開いて相手まで選ぶ。2段目の並びが出たところで返す。

    既に告発の画面（1段目）に居るなら、開き直さずそのまま相手を選ぶ。
    「選び直す」の後がこの形になる。
    """
    if not [t for t in appe.labels()
            if str(t).startswith(mod.ACCUSE_ONE_PREFIX)]:
        appe.go(placese["guild"])
        appe.facility_screen()
        refreshe()
        press(ctxe, appe, mod.ACCUSE_LABEL)
        refresh_for(ctxe, appe)
    one = [t for t in appe.labels()
           if str(t).startswith(mod.ACCUSE_ONE_PREFIX)
           and mod.game.name_of(appe, npc_id) in str(t)]
    assert one, appe.labels()
    press(ctxe, appe, one[0])
    refresh_for(ctxe, appe)


def pick_marks():
    return [e.get(mod.MARK) for e in appe.buttons
            if str(e.get(mod.MARK) or "").startswith(mod.PICK_MARK_PREFIX)]


# --- 何も集めていないうちは、相手を選んでも告発できない ---
open_accusation(culprite)
check("**相手を選んだだけでは決着しない**",
      case_mod.is_active(case_of(ctxe)), case_of(ctxe)["stage"])
check("何も集めていなければ挙げる根拠が無い", not pick_marks(), pick_marks())
check("**根拠が無ければ告発できない**",
      mod.SUBMIT_LABEL not in appe.labels(), appe.labels())
check("相手を選び直せる", mod.RESELECT_LABEL in appe.labels(), appe.labels())
press(ctxe, appe, mod.LEAVE_LABEL)
refresh_for(ctxe, appe)

# --- 集めてから開き直す ---
founde = learn_all(ctxe, appe, founde)
open_accusation(culprite)
shown = [m[len(mod.PICK_MARK_PREFIX):] for m in pick_marks()]
check("集めた手がかりが根拠の棚に並ぶ",
      any(not i.startswith("s") for i in shown), shown)
check("**聞いた言い分も同じ棚に並ぶ**",
      any(i.startswith("s") for i in shown), shown)
check("挙げていないうちは告発できない",
      mod.SUBMIT_LABEL not in appe.labels(), appe.labels())

# --- 入切する ---
first = shown[0]
press_mark(ctxe, appe, mod.PICK_MARK_PREFIX + first)
refresh_for(ctxe, appe)
on_text = [e.get("text") for e in appe.buttons
           if e.get(mod.MARK) == mod.PICK_MARK_PREFIX + first]
check("挙げた根拠は行の印が変わる",
      on_text and mod.PICK_ON_LABEL[:len(mod.PICK_PREFIX) + 4] in on_text[0],
      on_text)
check("1つ挙げれば告発できる",
      mod.SUBMIT_LABEL in appe.labels(), appe.labels())
press_mark(ctxe, appe, mod.PICK_MARK_PREFIX + first)
refresh_for(ctxe, appe)
check("**もう一度押すと取り下がる**",
      mod.SUBMIT_LABEL not in appe.labels(), appe.labels())
...
press_mark(ctxe, appe, mod.PICK_MARK_PREFIX + first)
refresh_for(ctxe, appe)
press(ctxe, appe, mod.RESELECT_LABEL)
refresh_for(ctxe, appe)
check("選び直すと容疑者の並びに戻る",
      [t for t in appe.labels() if str(t).startswith(mod.ACCUSE_ONE_PREFIX)],
      appe.labels())
open_accusation(culprite)
check("**前の相手のために選んだ根拠は持ち越さない**",
      mod.SUBMIT_LABEL not in appe.labels(), appe.labels())

# --- 根拠が足りない告発は決着しない ---
appe.texts[:] = []
press_mark(ctxe, appe, mod.PICK_MARK_PREFIX + "s" + str(culprite))
refresh_for(ctxe, appe)
press(ctxe, appe, mod.SUBMIT_LABEL)
thin_case = case_of(ctxe)
check("**根拠が足りなければ事件は続く**",
      case_mod.is_active(thin_case), thin_case["stage"])
check("回数だけは控えに残る",
      case_mod.thin_count(thin_case) == 1, thin_case.get("thin"))
said = " ".join(str(t) for t in appe.texts)
check("**通らなかった文は誰の名前にも触れない**（触れると答えになる）",
      not [i for i in case_mod.suspect_ids(thin_case)
           if mod.game.name_of(appe, i) in said],
      said)
check("手がかりも言い分も失われない",
      case_mod.found_count(thin_case) == case_mod.found_count(founde)
      and len(case_mod.heard_claims(thin_case))
      == len(case_mod.heard_claims(founde)),
      (case_mod.found_count(thin_case), case_mod.found_count(founde)))

# --- 根拠が揃えば決着する ---
picks_ok = proves(case_of(ctxe), culprite)
check("**真の材料だけで犯人1人に絞れる**", picks_ok, picks_ok)
gold_e = appe.player.gold
accuse(ctxe, appe, culprite, picks=picks_ok)
solved_e = case_of(ctxe)
check("判定は「解決」", solved_e.get("verdict") == case_mod.SOLVED,
      solved_e.get("verdict"))
check("決着した", solved_e["stage"] == case_mod.CLOSED, solved_e["stage"])
check("挙げた根拠が控えに残る",
      solved_e.get("evidence") == sorted(picks_ok),
      (solved_e.get("evidence"), sorted(picks_ok)))
check("報酬が入る", appe.player.gold == gold_e + mod.REWARD_GOLD,
      (gold_e, appe.player.gold))
check("**根拠のボタンにも固定の頭が付いている**（残骸を前方一致で拾える）",
      mod.PICK_PREFIX in mod.OUR_LABELS
      and mod.PICK_ON_LABEL.startswith(mod.PICK_PREFIX)
      and mod.PICK_OFF_LABEL.startswith(mod.PICK_PREFIX),
      (mod.PICK_PREFIX, mod.OUR_LABELS))
...
# `patterns/*.json` に出して、**1件ごとに抽選して組み合わせる**。
...
for _round in range(5):
    ...
    # 告発には根拠が要るので、先に聞いて回る。
    caser = learn_all(ctxr, appr, caser)
...
casep = learn_all(ctxp, appp, casep)
...
print("\n[判定] 誤認は思い違いを信じたときにだけ起きる")
# 根拠を挙げさせるようにしてから、**無実の者へ辿り着ける道は1本だけ**になった。
# 真の事実は犯人を消さないし、言い分も犯人のものだけ裏が取れない。
# だから真の材料だけで1人に絞ると、残るのは必ず犯人になる。
# 無実の者が唯一の残りになるのは、**犯人を消す思い違いを根拠に挙げたとき**。
# 当てずっぽうで外れるのではなく、筋の通った誤りとして外れる。
ctx2 = FakeCtx()
clean_record(ctx2)
mod.apply(ctx2)
app2, places2 = fresh_world()
refresh2 = install(ctx2, app2)
# 思い違いが混じらない事件も半々で出る（`MISTAKE_PERCENT` 既定 50）ので、
# **混じった事件が出るまで受け直す**。
# 控えを手で差し替える手もあるが、そうすると MOD が抱えている控えと
# ディスクの中身が食い違う（`WorldStore` は読んだものを覚えている）。
# 実際に生成された事件をそのまま使うほうが、確かめていることも本物になる。
found2 = None
hit = []
tries = 0
for tries in range(1, 41):
    app2.go(places2["guild"])
    app2.facility_screen()
    refresh_for(ctx2, app2)
    press(ctx2, app2, mod.START_LABEL)
    cand = case_of(ctx2)
    if cand.get("stage") != case_mod.INVESTIGATING:
        continue
    cand = learn_all(ctx2, app2, cand)
    hit = [(n, proves(cand, n)) for n in case_mod.suspect_ids(cand)
           if n != cand["culprit"]]
    hit = [(n, picks) for n, picks in hit if picks]
    if case_mod.contradicted(cand) and hit:
        found2 = cand
        break
    # 思い違いの無い事件は打ち切って次を受ける。
    app2.go(places2["guild"])
    app2.facility_screen()
    refresh_for(ctx2, app2)
    press(ctx2, app2, mod.ACCUSE_LABEL)
    refresh_for(ctx2, app2)
    press(ctx2, app2, mod.GIVE_UP_LABEL)
    refresh_for(ctx2, app2)

check("思い違いの混じった事件が出た（{}件目）".format(tries), found2, tries)
cast2 = list(case_mod.suspect_ids(found2))
existing2 = sorted(set(app2.world.characters) - set(cast2))
gold2 = app2.player.gold
check("**思い違いを外さないと誰も残らない**",
      case_mod.contradicted(found2), case_mod.remaining(found2))
# 誤認する相手は「思い違いが指し示した者」。
# 手当たり次第に外れるのではなく、筋を通した結果として外れる。
innocent, picks2 = hit[0]
check("**思い違いを信じると無実の者が1人だけ残る**",
      case_mod.remaining_with(found2, picks2) == [innocent],
      (picks2, case_mod.remaining_with(found2, picks2)))
wrong_ids = [str(c["id"]) for c in found2["clues"]
             if c.get("found") and c.get("wrong")]
check("**その根拠には思い違いが混じっている**",
      [p for p in picks2 if p in wrong_ids], (picks2, wrong_ids))
accuse(ctx2, app2, innocent, picks=picks2)
closed2 = case_of(ctx2)
check("判定は「誤認」", closed2.get("verdict") == case_mod.MISTAKEN,
      closed2.get("verdict"))
check("挙げた根拠も控えに残る", closed2.get("evidence") == sorted(picks2),
      (closed2.get("evidence"), sorted(picks2)))
...
# いまは両方とも `runtime/mods` なので1つに畳まれる。
# `local/` や `discontinued/` へ移したときは置き場が別になり（TECH.md §2.6）、

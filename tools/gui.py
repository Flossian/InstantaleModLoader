...
# mod の一覧・順序・設定
# --------------------------------------------------------------------------
# フォルダ名の番号帯（TECH.md §3.2.2）から種別のラベルを引く。
# mod.json に種別の欄を足せば帯に頼らずに済むが、
# それは全 MOD の一括書き換えになる ― 表示のためだけの情報なので、GUI 側で導く。
KIND_BY_BAND = {
    "0": "基盤",       # リコン・クラッシュ記録。他が触る前の素の状態を押さえる
    "1": "修正",       # 既にある動作を直す・調整する
    "2": "計測",       # 読み取り専用。値を変えない
    "3": "追加",       # 元々無かったものを足す
    "9": "開発中",     # 未公開（§2.6）。順序ファイルに書き、デバッグモード時のみ
}
...
def mod_kind(name: str, manifest: dict) -> str:
    ...
    帯より **manifest の旗を先に**見る。
    §3.2.2 自身が「帯は帯であって分類の軸ではない」と言っており（機能追加でも挙動を変えるなら 100番台に置く）、
    番号は当てにならないことがある。
    `debug` と `superseded` はローダが実際の読み込みに使う値なので、
    こちらは嘘をつかない。

    番号を持たない mod（利用者が入れた外部の mod）は空 ― 無理に当てはめるより、

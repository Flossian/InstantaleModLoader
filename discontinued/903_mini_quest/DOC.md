...
| 3. オフライン検証のコマンド | 戻す先は無い（§2.11 は一覧を持たなくなった。何を通すかは本の docstring 側） |
...
指してはいけない**。配布物にこのフォルダが無いので、配った先で
...
| `903_mini_quest` | 戦闘を伴わないミニクエスト（採集・救助・偵察） | **デバッグ用に降格（2026-08-06）。** 品質が目標に届かないため、既定では読み込まない（`mod.json` の `"debug": true`）。依頼の中身と非戦闘の進行までは実機で成立、**終わり方が定まらない**（実機4回。§2.19〜§2.23）。最後の修正（撤退の判定を戻り値ごと差し替え）は**未検証**。再開するなら §3.10 から | 119件全通。うち4件は `output_data/` の実プロンプトとの突き合わせ（進行 600 / 生成 133。2026-08-09 時点。**記録は `111_` 適用後**なので §2.43 を読むこと） |

## 3. オフライン検証（戻す先は無い。§2.11 は一覧を持たなくなった）
...
> **2026-08-06、デバッグ用に降格。** 品質が目標に届かないため、
...
>
> 止まっている場所は、**達成として帰還できるか**の1点だけ。最後に入れた
...
### 3.10.1 `903_` をデバッグ用に降格（2026-08-06）

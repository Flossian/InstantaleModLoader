...
ゲーム本来のクエストクリアは、
同行した仲間を全員パーティから外して雇った町へ帰してしまう。
`303_` は帰す先を「いま居る町のギルド」へ変え、後から会えるようにした。
ただし、そこで別れること自体は変わらない。
続けて次の依頼へ向かいたければ、毎回ギルドで雇い直す必要がある。
この mod は**外す処理そのものを起こさせない**。
...
番号が後ろなら、フックは外側に載る。
そのためこちらが解散を止めた時点で、`303_` の
`remove_party_member` フックには**そもそも呼び出しが届かない**（控えも取らないので置き直しも時間切れの保険も走らない）。
「誰も外れない」のだから「外れた人をどこに置くか」は出番が無い、
という素直な形に収まる。

`303_` が要らなくなるわけではない:

* こちらが降りたとき（`QuestEndManager` を捕まえられないビルド、
  `ALSO_ON_QUEST_RETIRE` を切ったままのクエスト放棄）は、
  `303_` が本来どおり置き先を差し替える
...
その前後に「パーティは帰還した…」「…はパーティから離脱した。」が出る。

名簿を実際に書き換えているのは `remove_party_member` の中。
だから外す処理を止めるには、**この呼び出しを解散の場面でだけ通さなければよい**。
名簿には指一本触れない。
...
1. 置き直し。
   ゲームは removal の後で `move_npc_to_facility` を呼ぶ。
   放っておくと仲間がパーティに居るまま施設にも登録され、
   ギルドに立っているのに付いてくる
2. 文言: `'…はパーティから離脱した。'` が出る。
   離脱していないのだから嘘になる

どちらも「引き留めた相手か」を控えで見るので、先に外してから置く（実測の順序）にも、
置き先を聞いてから外すにも間に合う（後者は未観測だが
`get_party_leave_facility` で先回りしてある）。
**聞きもせず置いてから外すビルド** だけは、
置く時点でその相手が解散対象だと知る手掛かりが無いので取りこぼす。
そのときも解散自体は止まる（`302_` の「ここで別れる」で外せば辻褄は戻る）。

## 世界のどこにも居ない NPC を作らない

置き直しを止めるのは、こちらが解散を止めた相手についてだけ。
控え（`kept`）に載っている間しか見ないので、
普段の NPC の移動は素通りする（`303_` と同じ理由で
`move_npc_to_facility` ではスタックを見ない。日常の移動で何度も通る経路なので、
控えが空のときは辞書の空判定だけで抜ける）。

本当に外れる相手の置き直しまで止めてはいけない。
死別・`302_` の「ここで別れる」で `remove_party_member` が解散の外から呼ばれたら、
その id を控えから即座に落とす。
外れた直後の置き直しを止めてしまうと、
その NPC は世界のどこにも居なくなる（`302_` が一番気を使っている失敗）。
控えは `KEEP_WINDOW` 秒で自然に消えるので、
置き直しが来ないまま終わっても後を引かない。

## 意図的に変わること

* 雇い直しの費用がかからなくなる。
  本来はクエストごとに雇い直す前提の設計なので、
  ここはゲームバランスを変える改造だと分かった上で入れること
* パーティは自分では減らない。
  外したくなったら `302_` の「ここで別れる」を使う（そちらは置き先を決めてから外すので、
  世界から消えることはない）

記録は `302_` / `303_` と同じ `party_leave.log` に
`quest-end keep:` を付けて書く（パーティの増減・解散・引き留めを1つの時系列で読めれば切り分けは速い）。
"""
...
# クエスト放棄でも解散を止めるか。
# `QuestRetireManager` が同じ役どころだと思われるが未観測なので既定では触らない（`303_` と同じ判断）。
# 放棄でも仲間に残ってほしくなったら True にする。
ALSO_ON_QUEST_RETIRE = False
RETIRE_MANAGERS = ("QuestRetireManager",)

# 見張るメソッド。
# 解散そのものは `method_1` の中だが、
# どちらの層から呼ばれるかは決めつけない（`frames.MethodWatch` が両方を表に入れる）。
...
# `303_` の時間切れの保険（8秒）より長く取って、
# そちらが動いた場合も止められるようにしてある。
# 過ぎれば控えは消え、その相手も普通の NPC として扱われる。
KEEP_WINDOW = 30.0

# 「…はパーティから離脱した。」を差し替えるか。
# 離脱していないので既定は差し替え。
# False にすると、ゲームの文言がそのまま出る（実際には残っているのに離脱したと書かれるので、
# 切り分けのとき以外は勧めない）。
REPLACE_LEAVE_TEXT = True

# 離脱を告げる文を見分ける手掛かり。
# 実測は 'テスト仲間Cはパーティから離脱した。'。
...
def apply(ctx):
    ...
    # 見張る相手（＝設計判断）だけがここに残る。
    # 判定そのものは `303_` と共通の `frames.MethodWatch`（段数で数えない・関数名で決めない。GAME.md
    # §2.8）。
    ...
    def remove_party_member(orig, self, member_id, *args, **kwargs):
        ...
        try:
            ...
            # 控えられなかったら止めない。
            # 中途半端に止めると、
            # 置き直しだけ通って「パーティに居るのに施設にも居る」状態になる。
    ...
    def get_party_leave_facility(orig, self, character_instance=None, *args, **kwargs):
        ...
        実測のビルドは removal の後に置き直すので、
        ここまで来なくても `remove_party_member` 側で間に合う。
        聞いてから外すビルド（置き直しが removal より先に来る）でも取りこぼさないための先回りで、
        離脱の場面でしか呼ばれない関数なので常時の負担にはならない。
        """
        try:
            source = in_disband()
    ...
    def move_npc_to_facility(orig, self, character_id, character_instance=None,
        ...
        NPC の日常の移動でも呼ばれる経路なので、**控えが空なら何もせず抜ける**（`303_` と同じ理由でスタックは見ない）。
        """
        if not state["kept"]:
    ...
    def add_text(orig, self, context, *args, **kwargs):
        ...
        引き留めた直後に**その相手の名前が入った離脱の文**だけを見る。
        控えが空なら何もせず抜けるので、普段の描画には触らない。
        """

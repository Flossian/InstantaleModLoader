...
その後の会話でどれだけ人となりが見えても増えない。
人生ログ（`life_log`）は出来事の記録で、
「何が好きか」「誰と繋がっているか」を引くようには出来ていない。

一方、**会話の出来事そのものはゲーム自身が覚えている**。
会話を正規の経路で閉じると `conversation_resolver` が要約を `current_log` へ追記し、
それは同じ相手との次の会話のプロンプトに毎回全文載る（GAME.md §2.25・`213_` の実測。
ただし正規以外の抜け方で要約が走るかは未実測。同節の注意を参照）。
だからこの mod まで出来事のあらすじを覚えると、
同じ事実が
`current_log`・注入プロフィール・`about_player` の3箇所で同一プロンプトに並ぶ。
3者は言い換えの関係なので `102_` でも畳めない。
そこで抽出には出来事の経過を書かないと指示している（`build_messages`）。
この mod が足す価値は「何があったか」ではなく「どういう人物か」で、
出来事はゲームの記録に任せる。
...
分けているのは、片方に混ぜると要約のたびに弱い方が食われるため。
「前に頼んだ件を覚えている」という手応えは `about_player` が担っていて、
これが人物像の文章に溶けると真っ先に消える。

3つめはいつの話か。
宿泊は数ヶ月飛ぶことがあるのに、ゲームが会話に渡すのは人生ログの日付だけで、
「前にこの人と話したのがいつか」はどこにも無い。
だから日が飛んだ次の会話が数秒前の続きのように始まる。
最後に話したゲーム内の日（`world.days_elapsed`）を控えて経過を添えるが、**日数だけを渡して解釈は指示で縛る**。
そのまま見せると毎回「久しぶり」になるので、
「同じ土地に留まっている間は日々見かけているはず」を併せて言う。
実際に離れていたかはゲーム自身が渡している滞在歴が語れるので、こちらは判定しない。

この2欄はどちらも毎回まるごと書き直されるので、統合のたびに細部が落ちる。
落ちた分の拠り所が3つめの欄 `facts`（追記専用）で、
次の抽出に差し戻して書き戻させる（`recent_facts`）。
ゲームの記憶もこの
mod の2欄も書き換えで更新される以上、**追記でしか伸びない記録はここだけ**になる。

## この mod が守っている決め事

**ゲームのセーブ構造に独自キーを足さない**（TECH.md §6）。
控えは `state/npc_profiles/<世界名>.json` に置く。
`out/` は世界を跨いで残るので、
世界ごとにファイルを分けないと別の世界の人物像が湧く（`306_` と同じ）。

注入は NPC の複製の `profile` にだけ足す。
`conversation_starter` / `conversation_facilitator` / `..._after_retrieval` /
`..._in_quest` /
`conversation_starter_in_quest` は先頭4引数が同じ並び（`messages, character_life_log, player, character_instance`）なので、
第4引数の NPC を浅く複製し、複製の `profile` を拡張すれば全経路を賄える。
ゲーム世界に居る NPC 本体も `messages` も触らない。
本体を書き換えるとセーブや別スレッドへ漏れ、
履歴を書き換えると要約・関係性の更新に波及する。

抽出は専用ワーカーで順番に回す。
返答後の次フレームでは、会話と NPC の文字列をコピーしてキューへ渡すだけ。
LLM 待ちの間も Kivy のメインスレッドを止めない。
続けて来たターンは直列に処理し、後の更新は前の控えを読む。

安全側の倒し方は「変更しない」。
捏造された素性の永続化が最悪の壊れ方なので、
空応答・LLM 不在・例外・読めない返却は既存の控えを維持する。

## 受け取りは JSON。**ゲームの構造化出力の経路は使わない**

抽出の結果は「更新後の全文」だけでなく「変わったのか」「何が新しく分かったのか」も要る。
以前は日本語の言い回し（「変更なし」「新しく分かった事は無い」）を照合していたが、
これは小さいモデルほど揺れる。
そこで **JSON オブジェクト1個** を出させて読む（`parse_result`）。

`send_request(manager_name, message, structure, ...)` に
pydantic モデルを渡す経路はゲーム自身が持っているが、こちらからは使わない:

- 引数の形を間違えると例外がゲームの内部スレッドで上がり、**呼び出しが永久に返らない**（GAME.md
  §2.12）。
  この mod の抽出は1本のワーカーで直列に回しているので、
  1回返らないと以後の抽出が全部止まる
- 構造化経路は空 `Literal[]` と `$defs` の肥大という既知の壊れ方を抱えている（`105_` /
  `203_`）。
  他人のスキーマの都合をこの mod が背負う理由が無い

`send_request_with_no_structure` は `str` を返すだけで、
返らない経路もスキーマも増えない。
読めない返却は「変更しない」に倒せば済む。

## 覚えたことは他の mod も読む。**渡すのはファイルで、関数ではない**

依頼の文面を作る `301_` は、依頼主の人物像をこの mod の控えから読む。
ただし **mod が mod を import することはしない**（TECH.md §3.2.3 / §3.7 の作法）。
ローダは mod を `instantale_mod_<フォルダ名>` で登録するので、
番号を振り直した瞬間に名前で掴む側が壊れる。
共有してよいのはローダの `instantale_modloader.*` だけで、
mod どうしは同じ場所を読むことで繋がる。

そのため `state/npc_profiles/<世界名>.json` の形は、
この mod の内部表現ではなく **mod をまたいだ取り決め**として扱う（MODS.md の
`311_` の項に書いてある）。
鍵の並びを `RECORD_KEYS` で固定しているのはこのため。
読む側はファイルが無ければ何も足さない。
この mod を切っていても、まだ一度も会話していなくても成立する。
"""
...
# 世界ごとの控えを置くフォルダ。
# `state/` の下（`ctx.state_path`）。
# ログと同じ `out/` に置いていたが、あちらは消してよい場所なので、
# 掃除のつもりで消した人の「覚えていたはずの人物像」まで飛んでいた。
# `301_` も同じ名前でここを読む。
STATE_DIRNAME = "npc_profiles"

# 世界ごとのファイル名は `instantale_modloader.state.world_filename` が作る。
# ここに同じ規則を写さないこと（`301_` が同じファイルを読むので、
# 1文字でもずれると引けなくなる。TECH.md §3.2.3）。

# 自前の `manager_name`。
# これを付けると自分のプロンプトも `output_data/<世界>/<PC>/<manager_name>/N.json` に残り、
# 抽出の検証がオフラインでできる（GAME.md §2.12）。
MANAGER_EXTRACT = "mod_npc_profile_extract"

# 旧版 `slots` の読み取り順。
# 移行にだけ使い、新しいプロフィールは分類しない。
LEGACY_SLOTS = ("好み", "嫌悪", "経歴", "人間関係", "目標", "秘密", "約束")

# 新情報が無いときの応答。
# **JSON が読めなかったときの受け皿**でだけ使う。
...
# 控えの鍵と、ファイルに書くときの並び。
# 順番を保つ（増えた鍵を後ろに足すだけにすると、
# 後から見比べたときに人物ごとの形が揃う）。
RECORD_KEYS = ("name", "updated", "profile", "about_player", "facts", "day")

# 最後に話したゲーム内日（`world.days_elapsed`。GAME.md §2.16）。
# 控えの鍵は末尾に足す（上の註のとおり、既にある鍵の位置を動かさない）。
DAY_KEY = "day"

# 経過日数の但し書き。
# 日数だけを渡して、解釈の仕方は指示で与える。
# 宿泊は数ヶ月飛ぶことがあるので、日数をそのまま見せると必ず「久しぶり」になる。
# 一方で同じ街に居る限り日々顔は合わせているはずで、
# そこはゲーム自身が渡している滞在歴（`area_residency`）が語れる。
# だからこちらは「地続きに再開しない」「不在明けの再会にもしない」の2つだけを言う。
...
# `about_player` の目標長。
# `INJECT_CHARS` から導く（設定を増やさない）。
ABOUT_PLAYER_RATIO = 2

# 抽出のときに差し戻す「判明済みの事実」の上限（件数と文字数）。
# 設定にはしない。
# 増やすほど落ちた事実が戻りやすいが、抽出の頼み文が伸びる。
# 1人あたりの `facts` は数件〜20件程度なので、40件あればほぼ全部が載る。
FACT_RECALL = 40
...
# 構造化出力に渡す制限時間（秒）。
# 必ず渡す。
# 抽出は1本のワーカーで直列に回しているので、
# 1回返らないと以後の抽出が全部止まる（GAME.md §2.12）。
EXTRACT_TIMEOUT = 120

# 待ち行列へ積んだままにする仕事の上限。
# ワーカーが（返らない推論などで）止まったときに、
# 会話を続けるだけで際限なく溜まるのを防ぐ。
# 溢れたら古い方を捨てる。
# 新しい会話ほど人物像に効くため。
MAX_PENDING = 8
...
def ordered_record(record):
    ...
    並びを固定するのは、この JSON が mod をまたいだ取り決めだから（`301_` が読む）。
    人物ごとに鍵の順が違うと、差分を見たときに何が増えたのか分からない。
    旧版の `slots` のような知らない鍵を捨てないのは、
    移行を1度きりにせず何度でもやり直せるようにするため。
    """
...
def recent_facts(record):
    ...
    人物像は毎回まるごと書き直される。
    統合のたびに細部が落ちるので、
    一度確定した事実でも数ターン後には本文から消えている。
    落ちた事実は `facts` に残っているのに、**抽出LLMはそれを見ていなかった**。
    だから書き戻せず、
    しかも既に記録した事実かどうかも判らないので同じ事実を毎ターン報告し直していた（同じ内容の言い換えが4回並ぶ）。
    ここで差し戻すと、その両方が同時に閉じる。

    件数と文字数の両方で頭打ちにし、溢れたら古い方から落とす。
    """
    ...
    for item in reversed(log):
        # 追記の形は `{"at":…, "text":…}`。
        # 素の文字列で書かれていても拾う（このファイルは
        # mod をまたいだ取り決めなので、形を決めつけない）。
...
def parse_result(result):
    ...
    JSON を1個だけ書かせているが、モデルは前置きを足すことも囲みを付けることもある。
    前後の地の文は落とし、最初の `{` から最後の `}` までを読む。
    """
...
def apply(ctx):
    # 世界ごとの控えの置き場。
    # ここで1回だけ引くのが要点で、`ctx.state_path()` は `out/` に同じ名前が在れば移してくるので、
    # フォルダごと引き取れる（1ファイルずつ移すと、
    # まだ触っていない世界の控えが `out/` に残り、
    ...
    def npc_id_of(app, character_instance):
        ...
        `character_of` がこの形で名前を引けている）。
        一方 `character_instance.id` が在るかは確かめられていない。
        無ければ `getattr` は黙って None を返し、**注入が理由も残さず素通りする**。
        確かな方を先に試し、属性は保険に下げる。
        """
        characters = getattr(getattr(app, "world", None), "characters", None)
    ...
    def load_bucket(key):
        ...
        注入のフックは LLM を呼ぶたびに走るので、
        そのたびに JSON を読み直すと会話1ターンで何度もディスクを叩くことになる。
        書くのはこの mod だけなので、
    ...
    def bucket_of(app):
        ...
        世界の見分けは名前で行っている（`306_` と同じ制約）。
        **名前は外部のデータ改変ツールで書き換えられる**ので、
        書き換えられた世界では控えが行方不明になる。
        名前に代わる安定した id が見つかっていないので名前のままにしてあるが、
        そうなったことが分かるように1度だけ残す。
        """
        key = world_key(app)
    ...
    def record_for(key, npc_id, npc_name):
        ...
        旧版の分類別 `slots` しか無い控えは、
        ここを最初に通ったときに `profile` へ移す。
        **移した後も `slots` は消さない**（この
        mod を古い版に戻しても遊べるように）。
        """
        with data_lock:
            bucket = load_bucket(key)
    ...
    def append_facts(record, facts):
        ...
        `profile` は毎回まるごと書き直されるので、
        統合の過程で落ちた事実はどこにも残らない。
        捏造を疑ったときに「いつの会話で出たのか」を辿れるようにしておく。
        上限を超えたら古い方から落とす（増え続けさせない）。

        **この控えは次の抽出に差し戻す**（`recent_facts`）。
        この mod とゲームを通じて、書き換えではなく追記で伸びる記録はここだけなので、
    ...
    def update_record(key, npc_id, npc_name, update, day=None):
        ...
        日も一緒に書く。
        控えを起こすのはここだけなので、`stamp_day` が作れない初回の日はここで入る。
        """
        if not update:
            return
    ...
    def stamp_day(key, npc_id, day):
        ...
        人物像が1文字も変わらなかったターンでも控える（そうしないと「変更なし」が続いた分だけ経過日数が伸び続ける）。
        日が変わるのは宿泊や移動のときだけなので、書き込みは会話1回につき多くて1度。

        控えがまだ無い相手には作らない。
        何も分かっていない相手の控えをここで起こすと、
        LLM が居ない・抽出が落ちたときに「日付だけの控え」が残る。
        この mod の安全側の倒し方は「変更しない」なので、
        初回の記録は抽出が実って
        `update_record` が書くときだけにする（あちらも日を書く）。
        """
        if day is None:
            return
        with data_lock:
    ...
    def memory_block(record, player_name, today=None):
        ...
        人物像とプレイヤーへの認識を見出しで分ける。
        1つに繋げると、会話の LLM が「この人物の設定」と「相手についての記憶」を混同して、プレイヤーの経歴を
        NPC 自身の経歴として話し始める。
        """
        blocks = []
    ...
    def with_profile(label, args, kwargs):
        ...
        複製できない・引数が短いならそのまま返す。
        ただし黙っては降りない。
        素通りした理由がログに残らないと、注入が効いていないことに気付けない。
        """
    ...
    def ask(manager_name, messages):
        ...
        送信モジュールを名指ししない。
        以前はローカルの `llama_cpp` と `any_server` の2つだけを見ていたが、
        Gemini / OpenAI / Claude ではどちらも載らないので毎回空振りしていた。
    ...
    def build_messages(snapshot, record):
        ...
        構造化出力が使える版では grammar が形を強制するが、
        頼み文の側でも同じ形を書いておく。
        ゲーム自身がスキーマをプロンプト本文にも埋めるのと同じ考えで、
        降りた先（`send_request_with_no_structure`）でも同じ読み取り（`parse_result`）で済む。

        出来事の経過は書かせない。
        会話のあらすじはゲーム自身が `current_log` に覚えて次の会話にも載せるので、
        こちらまで語り直すと同じ事実が1つのプロンプトに3回並ぶ（GAME.md §2.25）。
        この mod が書くのは、会話から分かったこと（続く性格・関係・約束）だけ。

        判明済みの事実は差し戻す。
        人物像は毎回まるごと書き直されるので、
        見せなければ落ちた事実は戻らない（`recent_facts`）。
        差し戻す先は抽出の頼み文だけで、**会話のプロンプトは1文字も増えない**。
        """
        ...
        # 判明済みの事実を差し戻す（`recent_facts` の説明）。
        # 人物像は毎回書き直されるので、ここで見せないと落ちた事実は二度と戻らない。

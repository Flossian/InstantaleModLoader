...
ほかに版1で決まったこと:
  ...
  * 渡した値が**そのまま** `config.enemy_data.<phase>.data.rank` に焼かれる
    （LLM の応答に `rank` は無い）。差し替えるならこの1点でよい
...
読めていないこと（3つ）:
  ...
  1. **報酬の式そのもの**。実測5点（30 → 173 / 43 → 282 / 44 → 288 / 58 → 359 /
     97 → 454）は頭打ちに向かう形で、`833 - 3614 / sqrt(rank)` なら 97 以外の4点が
     誤差1以内に乗る（97 だけ +12）。**ランク 70〜90 の点**が要る
     （漆黒斗技場は `current_phase` 6 まで進んでいるので、あと2〜3戦で 71 / 85 / 99）
  2. `D` が `get_quest_difficulties` の一覧の**どれ**か（実測の一覧は `[61, 60, 62]` で
     `D` は 61。平均でも中央値でも 61 になるので、まだ分かれていない）
...
def apply(ctx):
    ...
    def difficulties_of(app):
        ...
        **相手のランクはこの値から決まっている**（版1の実機。4つの闘技場・16点が
        ...
        `D` が一覧のどれなのか（平均か中央値か先頭か）はまだ決めていないので、
        闘技場に立つたびと相手を作るたびに一覧をそのまま写す。
        """

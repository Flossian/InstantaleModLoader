...
**ここも事件の論理を知らない。**持っているのは語彙と文言だけで、誰が犯人か・
どの事実が誰を消すかには一切関わらない（`case` と `city_case` の役目）。

##### なぜ外に出すのか

材料がコードに焼き付いていると、**何度遊んでも同じ形の事件**になる。盗みだけ、
髪と体格だけ、同じ8人。増やすにはコードを触ることになり、遊ぶ側が足せない。

`patterns/*.json` に出して、**1件ごとに抽選して組み合わせる**形にした。
ファイルを足せば事件の幅が増える ― コードは触らなくてよい。
...
JSON は人が書き換える前提なので、**壊れている前提で読む**。読めない項目は
落とし、束ごと空になったら**同梱の既定値に戻る**。事件が始まらないより、
定型の事件が始まるほうがよい（`writer` と同じ方針）。

**検算はここではなく組む側が持つ。**この束が何を返しても、必要性・解けること・
重複の無さは `build_cast` が確かめる。だから語彙を足しても謎は壊れない。
"""
...
#: 1件の事件で使う特徴の軸の**上限**。
#:
#: 実際に引く本数は容疑者の数から決まる（`axes_wanted`）。ここは
#: 「それ以上は引かない」という頭打ちで、一覧が長くなりすぎるのを防ぐ。
AXES_PER_CASE = 3
...
def axes_wanted(suspects, cap=AXES_PER_CASE):
    ...
    手がかりは「どれも抜けない」ことを求めるので、1本が1人だけを消す形に
    落ち着く。つまり**使える特徴の手がかりは `人数 - 1` 本まで**で、
    それより多く引いた軸は使い道が無い。使い道の無い軸は「全員同じ値」に
    なり、一覧の全行に同じ語が並ぶ。
    ...
    4人に3本引いた実機の例。`足を引きずる` が4人とも同じで、
    **読む側にとっては意味の無い1語**。「犯人の特徴が被りすぎ」の正体。

    だから**引くのは `人数 - 1` 本**。4人なら3本になり、並んだ語が全部
    何かを決めるようになる。

    （かつては双子 ― 犯人と特徴が完全に一致する者 ― を1人置いていたので
    `人数 - 2` だった。双子は面白さに寄与しないので外した）
    """
...
def read_incidents(data, write=None):
    ...
    `noun` は文中に差し込む短い語（「盗み」）。これが本文にも AI への説明にも
    効くので、**無い項目は使わない**。
    """
    out = []
...
def read_axes(data, write=None):
    ...
    1つの軸に**2つ以上の値**が要る（1つでは誰も絞れない）。値には
    「一覧に出る語」「目撃情報の文」「言い換え」「立ち絵の語」を持たせる。
    """
    out = []
...
def read_whereabouts(data, write=None):
    ...
    取れない場所は**犯人だけ**が言う。両方が要るので、片方でも空なら使わない。

    ファイルの形（`{"public": ...}`）でも束の形（`{"whereabouts": {...}}`）でも
    読む ― 既定値もこの関数を通すため（`normalise`）。
    """
    data = data or {}
...
def normalise(defaults, write=None):
    ...
    素通しで使うと、読み方の側にしか無い項目（`short` を後から足した）が
    既定値に無いまま流れる。実機で `KeyError: 'short'` を踏んだ。しかもそのとき初めて既定値に落ちていたので、
    **普段は通らない道だけが壊れている**という見つけにくい形だった。

    > **受け皿は、本番と同じ口から入れる。**別の入口を作ると、
    > 片方にだけ手を入れたときに形がずれる。
    """
    book = {}
...
def load(mod_dir, defaults=None, write=None):
    """材料を読む。**読めなかった束だけ**既定値で埋める。

    1つのファイルが壊れても他は生きる。全部壊れていても既定値で動く。

    `mod_dir` は **`apply()` の中で控えたもの**を渡すこと。`ctx.mod_dir` は
    `apply()` の外では `None` になる（ローダの註）。押されてから読もうとして
    `None` を渡し、ファイルが1つも見つからないまま既定値に落ちた ―
    実機で踏んだ。
    """
...
def draw_axes(book, rng, count=AXES_PER_CASE):
    ...
    ここが「事件のパターン」の芯。同じ髪と体格ばかりでなく、回によって
    服の色や持ち物で絞ることになるので、**同じ形の推理が続かない。**
...
def tables_for(axes):
    ...
    `(words, facts, hints, looks, shorts)`。鍵はどれも `(軸, 値)`。
    **`sex` は含めない** ― あれは人物に付いてくるもので、引くものではない。
    """

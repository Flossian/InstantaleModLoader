...
ここが持つのは「いま何が分かっているか」だけ。施設の引き当ても NPC の名前も
扱わない（`world.py` の役目）し、文言も特徴の語彙も持たない（入口の役目）。

##### 手がかりは「行き先」ではなく「条件」

最初の版は手がかりを一本道の鎖にしていた（`requires` で順番を固定し、揃うまで
告発できない）。実機で遊ぶと**推理が一切要らなかった** ― 言われた場所へ行って
ボタンを押すだけの作業になる（利用者の指摘）。

いまの形はこう。

- 手がかりは**犯人についての事実**（「両手で荷を抱えていた」）
- 事実は容疑者を**除外する**（片腕の者は当てはまらない）
- 順番は無い。**どこから調べてもよい**
- **いつでも告発できる。**2つで絞れたなら3つ目は要らない

除外の結果は控えに持つが、**画面には出さない**。事実と容疑者の特徴を並べて、
突き合わせるのはプレイヤーの仕事。そこを肩代わりすると、また作業に戻る。

##### なぜ状態を自分で持つのか

エンジンのフラグ（`flag_set`）は施設ローカルで、町を回る話には足りない
（GAME.md §2.21.3）。加えて施設の `config` に書かれてセーブに残る。
だから状態は `out/` に持ち、**渡すプログラムをその都度組む**。
"""

from instantale_modloader import read_json, write_json

#: 段階。**「揃った」段階は無い** ― いつでも告発できる。
NONE = "none"
...
def save(path, case):
    ...
    # 書いている最中に落ちた瞬間に事件が消える。親フォルダも作ってくれる。
...
def build(world_name, area_id, culprit, suspects, clues, reward):
    ...
    後から決めると、AI の出力次第で真相が変わってしまう。真相を先に固定して
    おけば、AI は描写しかできない。

    `suspects` は `{"id": ..., "tell": ...}` の並び。`tell` は**見て分かる
    特徴**で、プレイヤーが手がかりと突き合わせる材料になる。
    """
    return {
        "stage": INVESTIGATING,
        "world": world_name,
...
def clue_by_label(case, label):
    ...
    **判定の入口はここ。**プレイヤーが押した選択肢の飛び先が `resume` に
    入って戻ってくるので（GAME.md §2.21.1）、それをそのまま鍵にする。
    AI もエンジンも介さない。
    """
    for clue in case.get("clues", []):
...
def eliminated(case, skip=None):
    ...
    **画面には出さない。**突き合わせるのはプレイヤーの仕事で、そこを
    肩代わりすると推理が消える。ここは「解けるかどうか」を測るためと、
    ログのためにある。

    `skip` にひとつ id を渡すと、その手がかりを**無かったことにして**数える
    （思い違いを見つけるのに使う。`without_each`）。

    **犯人を除く細工はしない。**真の事実は犯人を消さない（事実は犯人に
    ついてのものなので当たり前）ので、以前は保険で引いていた。思い違いの
    証言は**わざと犯人を消す**ので、引いてしまうとその食い違いが見えなくなる。
    """
    out = set()
...
def contradicted(case):
    ...
    思い違いの証言が1つ混じっている合図。**「解けない」ではなく
    「1つが間違っている」**という意味なので、詰みとは違う。
    """
...
def without_each(case):
    ...
    食い違ったときに「どれが思い違いか」を決めるための道具。**答えは
    画面に出さない** ― 外して数えるのはプレイヤーの仕事で、ここは事件を
    組むときに「一意に決まるか」を検算するために使う。
    """

...
4. セーブを汚さない。
プログラムは
`_lookup_program` を包んで渡す（`free_facility_programs` には書かない）。
`flag_set` も使わない（施設の `config` に残るため）。
報酬は `309_` が実証した経路で直接渡す。
事件の控えは `state/` だけ。
...
既存の NPC を犯人に仕立ててはいけない。
機能としては動くが、プレイヤーが愛着を持っている相手が犯人にされうる。
そうなった時点で、この遊びは受け入れられなくなる。
実際、最初の実機では `混乱する村人たち` という群衆が犯人に選ばれていて、
既存 NPC から選ぶ設計の筋の悪さが2つ同時に出ていた。

だからキャストは事件のために生成し、**作れなければ事件を始めない**。
既存 NPC に落とすと問題がそのまま戻る。
始まらないほうがましだと判断している。

生成した NPC は HP もスキルも立ち絵も空でよい。
ゲームが会話の直前に `ensure_npc_detail_generated` で埋める（実測。
VERIFICATION_LOG.md §2.29）。

**ただし戦闘はそこを通らない。**
スキルが空のまま敵ターンを迎えると、素のゲームでも落ちる（VERIFICATION_LOG.md §2.40・§2.42）。
この MOD が作った NPC に固有の話ではないので、塞ぐのはこちらの仕事ではない（DOC.md §0）。

事件が決着したらキャストは全員引き上げる。
当てても外しても同じで、セーブは事件の前の状態に戻る（DOC.md §3 の「遊ぶほどセーブが汚れていく」）。

既存 NPC を使う経路は持たない。
愛着の問題に加えて、推理が成立しないため（手がかりは容疑者の特徴との突き合わせなので、
特徴の分からない相手は容疑者にできない）。
"""
...
# この MOD 専用の乱数器。
# グローバルの `random` を引くとゲーム自身の乱数列がずれる（TECH.md §6.1。
# `104_balance_area_bgm` と同じ方針）。
# 事件は 1回組むたびに何十回も引くので、混ぜると影響が積み上がる。
_RNG = random.Random()
...
#: 犯人を退場させるか。
#: 切ると真相が分かるだけで、犯人は町に残る。
ARREST_CULPRIT = True

#: 容疑者の人数（犯人を含む）。
#: 多いほど手がかりが増え、聞き込みも長くなる。
#: 特徴の軸は人数から決まる（`patterns.axes_wanted`）ので、
#: 増やすと一覧の語も増える。
SUSPECT_COUNT = 4
...
#: 告発の相手を選ぶボタン。
#: 特徴を必ず添える。
#: 集めた事実と突き合わせるのがプレイヤーの仕事なので、一覧と同じ書き方にする。
#: **決まった接頭辞を付けること**（`ACCUSE_ONE_PREFIX`）。
#: ボタンはセーブに焼かれることがあり、
#: そのとき印は落ちる（`ui.Screen.prune_stale`）。
#: 残骸を見分ける手段は文言しかないが、
#: 容疑者の名前は事件ごとに変わるので名前だけでは照合できない。
#: 固定の頭を付けておけば前方一致で拾える。
...
# ---------------------------------------------------------------- 根拠を挙げる
#: 告発の2段目。
#: 相手を選ぶだけでは「犯人を当てた」にしかならない。
#: 何を根拠にするかまで答えさせると「追い詰めた」になる。
#: 材料は集めた手がかりと聞いた言い分の両方（`case.evidence`）。
ACCUSE_WHY_TEXT = "「{name}だ」と告げるなら、何を根拠にするかも言わねばならない。"
#: 挙げられるものが1つも無いとき。
#: 告発そのものは止めない（いつでも告発できるのは柱）。
#: 根拠が無ければ通らないだけで、聞き込みへ戻れる。
ACCUSE_NO_EVIDENCE_TEXT = "（まだ何も掴んでいない。これでは誰も動かせない）"
#: 根拠の入切ボタン。
#: **決まった接頭辞を付けること**（`ACCUSE_ONE_PREFIX` と同じ理由）。
#: 文はそのつど変わるので、固定の頭が無いと残骸を前方一致で拾えない。
PICK_PREFIX = "根拠 ― "
PICK_ON_LABEL = PICK_PREFIX + "［挙げた］{text}"
PICK_OFF_LABEL = PICK_PREFIX + "［　　　］{text}"
#: 言い分を根拠として並べるときの書き方。
#: 手がかりと同じ棚に並ぶので、誰の言い分かが分かる形にする。
PICK_CLAIM_TEXT = "{name}「{claim}」"
#: 根拠の行の長さの上限。
#: 事実の文は AI が書くので長さが読めない。
#: ボタンは1行なので、溢れると読めなくなる。
PICK_TEXT_LIMIT = 34
SUBMIT_LABEL = "この根拠で告発する"
RESELECT_LABEL = "告発する相手を選び直す"
#: 根拠が足りなかったときの文。
#: **誰を名指ししたかには触れない。**
#: 触れると、この文が出たこと自体が「相手は合っている」と教えることになる（`case.judge`）。
THIN_TEXT = "「それだけでは、その者だと決められん」――話は取り合ってもらえなかった。"
THIN_HINT_TEXT = "（挙げた根拠だけで1人に絞れているか、もう一度見ること）"
...
#: 真相は明かさない。
#: 告発すれば当たり外れは分かる。
#: そこまで踏み込まずに降りたのなら、答えは持ち帰れないほうが筋に合う。
...
#: 待ち時間を過ぎても返ってこなかったときの一言。
#: 黙って解かない。
#: 押せるようになった理由が分からないと、何が起きたのか掴めない。
WRITING_SLOW_TEXT = "（今日は誰も口を割らなかった。出直すことにする）"

#: 断る理由ごとに文を分ける。
#: **同じ文が別々の理由で出ると切り分けられない**（実機で「町に調べようがない」と出たが、
#: 実際は町ではなくキャスト生成の失敗だった）。
#: 画面の文だけで、ログを開くべきかが分かるようにする。
...
#: 容疑者へ話しかけたときに差し込む言い分。
#: 容疑者は自分の立場を語る。
#: 事件のために作った人物が町に立っているのに話しかけても何も起きないのでは、
#: 探偵ものにならない。
#: 嘘はつかせない。
#: 言い分そのものは真実で、裏が取れるかどうかだけが違う。
#: AI に嘘を任せると真相が揺れる。
#: 容疑者が語る自分の立場。
#: 証言者と同じく「必ず話す」にする。
#: 「問われたら答える」だと、控えに `heard` が立つ条件（会話した）と噛み合わない。
#: 尋ねなければ何も言われないのに言い分だけ控えに残る。
#: 印を立てるのは実際に話しかけて何か尋ねたときだけになったので（`brief`）、
#: その1回で必ず言わせる。
...
#: 開示してよい事実だけを渡す（設計の柱3）。
#: 差し込む先は2つ。
#: 開始のひとこと（`conversation_starter` の messages）と、**受け答えを組むときの
#: `retrieved_knowledge`**。
#: 後者はゲーム自身が持っている「この人物が引き出した知識」の枠で、
#: プレイヤーが直接尋ねたときにも効く。
#: 証言者に喋らせる事実。
#: 「尋ねられたら」にしない。
#: 以前は「尋ねられたら話す」だった。
#: ところが手がかりが立つ条件は「その人物と会話した」という行動なので、**プレイヤーが尋ねなければ相手は何も言わないのに、
#: 会話を閉じた瞬間に手がかりだけが出る。**
...
#: 第一声に差し込む匂わせ。
#: 事実そのものは言わせない。
#: 会話に入って即抜けるだけで手がかりが手に入るのを止めるため、
#: 中身は尋ねられてから話させる（`conversation_starter` の註）。
#: **代わりに「尋ねれば話す」ことは分かるようにする**。
#: 匂わせも無いと、誰に聞けばよいのか分からないまま町を歩くことになる。
...
#: 入れると事件ごとの人物と文章が、その世界に合わせて作られる。
#: **論理は入れても切っても同じ**（コードが決めている）。
#: 呼べなかった・返事が壊れていた・検算に落ちたときは黙って下地に戻る。
USE_LLM = True
...
#: 待機表示を強制的に解くまでの上乗せ（秒）。
#: 押せない画面のまま放置しないための最後の網。
#: `send_request` が `timeout` を守る保証はこちらには無いし、
#: スレッドが固まる可能性もある。
...
#: 毎回混ぜないのが要点。
#: always だと「1本は嘘」が前提知識になって、食い違いに気づく驚きが無くなる。
#: 0 にすると従来どおり全部が真になる。
#: 一意に決まらない置き方には混ぜない（`resolves_once`）ので、
#: この値を上げても**当てずっぽうでしか解けない事件は出ない**。
MISTAKE_PERCENT = 50

#: 聞き込み中に走る会話の経路を計測するか。
#: 届かないときの切り分け用。
#: 普段は切っておく（ログが会話のたびに伸びる）。
#: 入れると、聞き込み中に走った LLM の入口と、
#: 証言者の照合が外れた理由を `out/city_case.log` へ出す。
...
#: 聞き込みの印が古くなったら使わない（押してから会話が始まるまでの間に別の会話が割り込んだ場合に、
#: そちらへ差し込んでしまうのを防ぐ）。
ASK_TTL = 300.0
...
#: 条件を満たすまで振り直す。
#: 40 では足りない。
#: `build_cast` の条件に「どの手がかりも抜けない」を足した時点で、
#: 通る顔ぶれの割合が下がった（試算で 40 回だと 3〜8 人のどの設定でも :
#: 1〜19% が組めずに終わる）。
#: 300 回にすると全設定で取りこぼしが消える。
#: 振り直しは町に出る前の計算だけで、NPC はまだ作っていない。
#: 何回まわしても世界には何も残らない。
CAST_ATTEMPTS = 300
...
#: 根拠1つぶんの印。
#: 材料の id（手がかりは `c1`、言い分は `s<npc_id>`）を後ろに付ける。
PICK_MARK_PREFIX = "pick_"

#: この MOD が出すボタンの文言。
#: 残骸の掃除に使う（`ui.Screen.prune_stale`）。
#: セーブに焼かれたボタンは印を失うので、文言で見分けるしかない。
OUR_LABELS = (START_LABEL, ACCUSE_LABEL, STATUS_LABEL,
              ACCUSE_ONE_PREFIX, PICK_PREFIX, SUBMIT_LABEL, RESELECT_LABEL,
              LEAVE_LABEL, GIVE_UP_LABEL)
...
def apply(ctx):
    ...
    # そのとき取りに行くと `None` を掴んで**ファイルが1つも見つからない**。
    # 実機でこれを踏んだ。
    ...
    # 3つとも画面の状態なので控え（`state/`）には持たない。
    # 事件を跨いで残るものではないし、途中で閉じたら白紙からでよい。
    state = {"accusing": False, "accused": None, "picks": set(),
    ...
    def current(app):
        ...
        if case_mod.is_active(found) and name and not case_mod.belongs_to(found, name):
            ...
            # 解けない事件を抱えたままにしない。
            # 手がかりの置き場所が町に無いと永久に詰む。
            # 古い版が組んだ控えや、
            # `CLUE_POOL` を書き換えた後の控えがこれになる（実際に踏んだ）。
            # 黙って捨てて、次の依頼から組み直す。
    ...
    def backfill(app, found):
        ...
        控えは注入をまたいで残るので、**進行中の事件は古い版が作ったもの** でありうる。
    ...
    def draw_axes(app):
        ...
        軸は事件ごとに変わる（髪と体格の回もあれば、服の色と持ち物と声の回もある）。
        `tell_of` / `plan_facts` / `fact_order` はここで載せた表だけを見るので、
        軸を増やしてもコードは触らなくてよい。
        """
    ...
    def build_cast(app, area):
        ...
        以前は「犯人と特徴が完全に一致する者」を1人必ず置いていた。
        特徴だけでは犯人を決められなくして、裏取り（アリバイ）を要る形にするため。

        **面白さに寄与していなかった**。
        一覧に同じ行が2つ並ぶだけで、そこから考えることが増えない。
        外したので、
        それを支えるためだけに在った「裏取りが必ず1本要る」も一緒に外れた。
        **裏取りは、特徴で消しきれない者が居るときにだけ出る**。

        ##### 振らずに置く

        条件を満たす配り方は狭い。
        3人を無作為に振って「全員が1軸だけ違い、
        しかもその軸が全員バラバラ」になる見込みは 6/512（約 1%）で、
        振り直しに頼ると取りこぼす。
        だから最初からその形に置く（`lay_out`）。
        振り直しは、置いた形が条件に落ちたときの保険。

        ##### 「集めれば解ける」だけでは足りない

        最初は上2つと「全部集めれば絞れる」しか見ていなかった。
        それだと **同じ働きしかしない事実が並ぶ**。
        実機で出た事件がこれ:
        ...
        条件は全部通る（集めれば 76 に決まる）のに、
        c1 と c2 はどちらか一方で足りる。
        もう片方のために町を横断するぶんは、まるごと無駄足。
        「大柄」も4人全員なので一覧の文字が1つ増えるだけで何も決めない。

        そこで「どの事実も抜けない」を条件に足した。
        1本でも欠けたら絞れない、という形にしておけば、
        集めた手がかりが全部きちんと働く。
        併せて最低2本（容疑者が2人以上のとき）を要求する。
        1本で終わる事件は、拾った時点で答えが出てしまう。
        3本にしないのは下の `least` の註のとおりで、求めると犯人が多数派で割れる。
        ...
        # 手がかりの最低本数。
        # 1本で終わる事件を出さないための下限。
        # 3本を求めると「全員が1軸だけ違う」形しか通らなくなり、
        # 犯人が多数派で割れてしまう（`lay_out` の註）。
        # 2本あれば2段階で絞れる。
        ...
        for _attempt in range(CAST_ATTEMPTS):
            ...
            if not usable or len(facts) < least:
                continue                # 覆えない／薄すぎる
            # 思い違いを混ぜる。
            # 混ぜられない置き方なら、そのまま真の証言だけで出す（事件が始まらないより、
            # 素直な事件のほうがよい）。
    ...
    def tell_of(traits):
        ...
        引いた軸の順に並べて、最後に性別。
        軸は事件ごとに変わるので、並びも事件ごとに変わる（`state['axes']` の順）。

        区切って並べる（「黒髪・大柄・男」）。
        連体形のまま繋ぐと「目立つ色の服の煙のにおいのする袋を提げた女」のような一続きの句になり、
        集めた事実と**突き合わせるのに読み解きが要る**。
        そこがこの遊びの本体なので、並べ方で邪魔をしない。
        """
    ...
    def plan_facts(cast, culprit_index):
        ...
        特徴による除外を並べ、最後に裏取り（アリバイ）を足す。
        狙いは「**どれか1つでは絞れないが、組み合わせると1人に決まる**」形。
        ...
        for key, value in sorted(culprit.get("traits", {}).items()):
            ...
            # `axis` / `value` は頼み文と検算のために持つ（`fact_order`）。
            # 事件の論理そのものには使わない。
    ...
    def lay_out(picks, axes):
        ...
        以前は「他の者は犯人と1軸だけ違う」形に置いていた。
        それだと手がかりが必ず1人ずつを消す形になり、条件は綺麗に満たされる。
        ...
        一覧を眺めるだけで、手がかり0本で犯人が分かる。
        双子より悪い。

        だからここでは自由に振り、**どの手がかりを使うかを後で選ぶ**（`choose_facts`）。
        使わなかった軸は赤鰊として一覧に残る。
        """
    ...
    def choose_facts(cast, candidates):
        ...
        候補を全部使おうとすると、条件を満たす配り方が「全員が1軸だけ違う」しか無くなり、
        犯人が多数派で割れる（上）。
        選ぶ側に回れば、配り方は自由でよい。
        選ばなかった軸は一覧に残って赤鰊になる。
        見た目は違うのに、それを言う証言が無い。
        ...
        候補は多くて4本なので、
        部分集合を全部試して**いちばん本数の多いものを採る**（拾う手が多いほうが遊びとして厚い）。
        """
    ...
    def not_obvious(cast, axes):
        ...
        **犯人が「どの軸でも多数派」の唯一の1人**だと、
        手がかりを1本も拾わずに当てられる。
        1軸ずつ違えて配ると必ずこうなるので、配り方を自由にした後もここで見張る。
        """
    ...
    def add_mistake(cast, axes, facts):
        ...
        真の事実だけだと、遊びは最後まで「当てはまらない者を消す」だけで終わる。
        属性の語彙を増やしても操作は同じなので、
        そこが天井になっていた。

        1本を間違いにすると、**すべて信じたときに誰も残らない**。
        そこで初めて「どれが嘘か」を探すことになり、消し込みが推論に変わる。

        ##### 一意に決まらないなら混ぜない

        思い違いを外せば1人残る。
        それだけでは足りない。
        真の証言を外したときにも1人残ってしまうと、
        プレイヤーはどちらが嘘か決められない。
        だから「ちょうど1人残る外し方がただ1つ」を確かめて、
        満たさない置き方には混ぜない（`case.without_each`）。

        嘘は特徴の言い間違いにする（「黒髪だった」→本当は違う）。
        裏取りは「裏が取れた」という性質の証言なので、
        そこを嘘にすると何を信じてよいか分からなくなる。
        """
        ...
        # **無実の者が1人は残る引き方を先に試す。**
        #
        # 思い違いは犯人に加えて「その値を持たない者」を全員消す。
        # 全員消してしまう引き方だと、それを信じても誰も残らない。
        # つまり**筋の通った誤認ができない** ―
        # 思い違いを信じたまま告発しても `case.MISTAKEN` へ行き着かず、
        # 必ず「根拠が足りない」で跳ね返る。
        # 外したことを世界に残す遊びにするなら、外れ方そのものが成立していなければならない。
        #
        # 捨てるのではなく後回しにする。
        # 捨てると誤認は倍になるが、**思い違いが混ざること自体が減る**（実測は DOC.md §3）。
        # 誤認へ行けない思い違いも無駄ではない。
        # 食い違いに気づいて1本外す、という推論はそちらでも起きる。
        # 順番を変えるだけなら、どちらも減らさずに済む。
        ready = [wrong for wrong in tries if len(wrong["drops"]) < len(cast)]
        rest = [wrong for wrong in tries if len(wrong["drops"]) >= len(cast)]
        _RNG.shuffle(ready)
        _RNG.shuffle(rest)
        for wrong in ready + rest:
    ...
    def all_telling(cast, axes):
        ...
        全員が同じ値の軸があると、一覧の全行に同じ語が並ぶ。
        読む側には意味の無い1語で、「特徴が被りすぎ」に見える。
        本数のほうは `patterns.axes_wanted` で合わせてあるが、
        ...
        for axis in axes:
            ...
            if len(seen) < 2:
                return False
        # 性別も一覧に出る。
        # 手がかりにはならない（人物に付いてくるもので、引く軸ではない）が、
        # 全員同じなら同じ語が全行に並ぶのは変わらない。
    ...
    def pick_clues(app, area, cast, culprit_index, facts, written=None):
        ...
        for index, (kind_name, fact) in enumerate(zip(spots, facts), start=1):
            ...
            clues.append({
                ...
                # 証言者を控える。
                # これがあれば「誰が何を知っているか」が
                # id の突き合わせだけで決まり、**どう話しかけられたかに依存しない**（ボタン経由でも、
                # 普通に話しかけても同じ）。
    ...
    def fact_order(facts):
        ...
        「髪が黒い」を頼んだのに「黒髪の大柄な男」と返されると、
        渡していない特徴まで漏れて**その1文で犯人が決まってしまう**。
        避けたい語を先に渡し、返事も同じ規則で検算する（`writer.read_facts`）。
        """
        orders = []
        for fact in facts:
            ...
            if not axis:
                # 裏取り（アリバイ）。
                # 書かせない（`writer.wanted_facts`）。
                # 名前を挙げてその人物を消す文なのに、
                # 名前は同じ応答で作らせている最中で、まだ無い。
    ...
    def dress(members, material):
        ...
        名前や来歴は差し替わるが、
        `traits`（性別・髪・体格）はコードが決めたものがそのまま残る。
        一覧に出る `tell` も手がかりの効き先もそこから作るので、
        文章が何であれ推理は成立する。
        """
        if not material:
            return members
    ...
    def open_case(app, prepared=None):
        ...
        `prepared` は `(顔ぶれ, 事実, 描写)`。
        **LLM に描写を頼むときは、頼んだのと同じ顔ぶれで始めなければならない**。
        ここで組み直すと「書かせた人物」と「実際に立つ人物」が別物になる。
        """
        area = game.area_id(app)
        if prepared:
    ...
    def cast_for(app, area, specs):
        ...
        プレイヤーが愛着を持っている相手が犯人にされうるからで、**その1点だけでこの遊びは成立しなくなる**。

        推理の面でも既存 NPC は使えない。
        手がかりは「犯人はこういう者だった」という特徴の突き合わせで、
        そのためには容疑者の特徴が分かっていなければならない。
        世界に元から居る NPC にその情報は無い。

        作れなければ事件を始めない。
        既存 NPC に落とすと両方の問題が戻ってくる。
        始まらないほうがましだと判断している。
        """
        ...
        for index, spec in enumerate(specs):
            ...
            if npc_id in made:
                ...
                # `generate_npc` の採番は連続で呼ぶと進まないことがある。
                # 気付かずに進めると「選択肢が全部同じ」になるので、ここで止める。
                write("[{}] make_npc returned {} again; the previous cast "
                      "member was overwritten. stopping.".format(
                          stamp(), npc_id))
                break
            # 作った直後に台帳へ控える。
            # 事件の控えに書く前に落ちても、次の起動で掃除できるようにするため。
    ...
    def place_of(app, found, facility_type):
        ...
        種類の名前（`inn`）をそのまま出しても、
        プレイヤーの画面には施設の名前しか出ていないので突き合わせられない。
        """
    ...
    def say_roster(app, found):
        ...
        これが推理の材料。
        集めた話と見比べて絞るのはプレイヤーの仕事なので、**こちらで除外の結果を出してはいけない**（出した瞬間に作業に戻る）。
        """
    ...
    def say_contradiction(app, found):
        ...
        集めた話を全部信じると誰も当てはまらない。
        ここで何も言わないと、プレイヤーは「詰んだ」「壊れている」と受け取る。
        **気づく手がかりは出すが、
        答えは出さない**（外して数えるのがこの遊びの山なので）。
        """
    ...
    def say_lead(app, found, first=False):
        ...
        行き先が分からないと無策で歩き回ることになる。
        ただし1つに絞って示すと道順をなぞる作業になるので、
        残っている場所を全部出して選ばせる。
        """
        places = []
        for clue in found.get("clues", []):
    ...
    def finish_asking(app, partner=None):
        ...
        会話の最中に出すと地の文に埋もれる。
        閉じてから出すことで、**何が分かったのかが必ず残る**。

        ##### 閉じた会話が、印を付けた会話と同じものか確かめる

        印（`state['asking']`）は LLM の呼び出しで立ち、ここで消える。
        だが会話が閉じられずに終わることがある（画面が変わる・中断する）。
        すると印が残ったままになり、次に別の誰かとの会話を閉じた瞬間に、
        前の相手の手がかりが出る。
        これも「会話内容とヒングが噛み合わない」の正体の一つ。

        防ぎ方は2つとも使う:

        - **相手の id を突き合わせる**（`partner`）。
          閉じた会話の相手が印の相手と違うなら、その印は捨てる
        - 古い印は捨てる（`ASK_TTL`）。
          相手が分からない版でも効く最後の網
        """
    ...
    def launch(app):
        ...
        聞き込みを会話へ移した後は場面がこの1画面だけになり、
        中身も「容疑者を並べて1人選ばせる」だけ。
        DSL の分岐も `flag_set` も使っていない。
        それでいて代償は大きかった:

        - `scripts.free_facility` が**まだ import されていない**か
          `lint_program` が無いと場面を組めず、**告発ボタンを押すと「調べていることは無い」と出て詰む**（事件は進行中なのに）。
          自由施設に一度も入っていないプレイで普通に踏む
        - ゲームの `free_*` を横取りする以上、
          その施設を壊す危険を 1画面のために負い続けることになる
        ...
        1段目で相手を選び、2段目で根拠を挙げる。
        相手だけを選ばせる形は「犯人を当てた」で終わりになり、
        外したときに世界へ残せるものが「間違えた」しか無かった（DOC.md §0）。
        """
        found = current(app)
        if not case_mod.is_active(found):
            screen.say(app, NO_CASE_TEXT)
    ...
    def abandon(app):
        ...
        「やめる」との違いは戻れるかどうか。
        こちらは事件が消えるので、ギルドで新しい噂を聞くところからやり直しになる。

        真相は明かさない。
        告発すれば当たっても外れても真相が分かる（外したときも
        `FAILED_TEXT` で犯人を告げる）ので、そこまで踏み込まずに降りたのなら、
        答えは持ち帰れないほうが筋が通る。

        キャストは全員引き上げる。
        誰も捕まっていないので `ARREST_CULPRIT` の出番は無いし、
        残すと作った NPC がそのまま町に溜まる。
        """
        found = current(app)
        state["accusing"] = False
        state["accused"] = None
    ...
    def leave_accusation():
        """告発の画面から降りる。3つの印を必ず一緒に落とす。

        `accusing` だけ落として `accused` を残すと、
        次に告発を開いたときに前の相手が選ばれたまま2段目から始まる。
        画面の状態は1箇所で落とす。
        """
        state["accusing"] = False
        state["accused"] = None
    ...
    def pick_text(app, item):
        """根拠1つぶんの行。ボタンは1行なので溢れたら畳む。

        手がかりはその文のまま。
        言い分は誰のものかを添える（手がかりと同じ棚に並ぶため）。
        """
        if item.get("kind") == "claim":
            text = PICK_CLAIM_TEXT.format(
                name=game.name_of(app, item.get("npc")),
                claim=item.get("text", ""))
        else:
            text = item.get("text", "")
        text = " ".join(str(text).split())
        if len(text) > PICK_TEXT_LIMIT:
            text = text[:PICK_TEXT_LIMIT - 1] + "…"
        return text
    ...
    def accuse_action(npc_id):
        ...
        def run(app):
            ...
            if not case_mod.is_active(found):
                screen.say(app, NO_CASE_TEXT)
                state["accusing"] = False
                repaint(app, "nothing to accuse")
                return
            state["accused"] = str(npc_id)
            state["picks"] = set()
            screen.say(app, ACCUSE_WHY_TEXT.format(
                name=game.name_of(app, npc_id)))
            pool = case_mod.evidence(found)
            if not pool:
                screen.say(app, ACCUSE_NO_EVIDENCE_TEXT)
            write("[{}] naming {} ({}); {} piece(s) of evidence to choose "
                  "from".format(stamp(), npc_id, game.name_of(app, npc_id),
                                len(pool)))
            repaint(app, "accuse why")
        return run

    def pick_action(pick_id):
        """根拠1つぶんの入切。押すたびに挙げる／挙げないが入れ替わる。

        控え（`state/`）には持たない。
        告発の画面を降りたら白紙に戻る。
        途中まで選んだ根拠を跨いで覚えておく意味が無いうえ、
        覚えていると次に開いたときに何を選んでいたのか分からない。
        """
        def run(app):
            picks = state.get("picks")
            if not isinstance(picks, set):
                picks = set()
            if pick_id in picks:
                picks.discard(pick_id)
            else:
                picks.add(pick_id)
            state["picks"] = picks
            repaint(app, "pick")
    ...
    def reselect(app):
        ...
        相手が変われば挙げるべき根拠も変わる。
        持ち越すと、前の相手のために選んだものが残ったまま次の告発に付いていく。
        """
        state["accused"] = None
        state["picks"] = set()
        screen.say(app, ACCUSE_TEXT)
        repaint(app, "accuse reselect")

    def submit(app):
        """挙げた根拠で告発する。判定は `case.judge` の1本。

        3値のうち `THIN` では事件が終わらない。
        「その根拠では通らない」だけなので、聞き込みの続きへ戻れる。
        `SOLVED` と `MISTAKEN` は決着で、どちらもキャストを引き上げる。
        """
        found = current(app)
        accused = state.get("accused")
        if not case_mod.is_active(found) or not accused:
            screen.say(app, NO_CASE_TEXT)
            state["accusing"] = False
            state["accused"] = None
            repaint(app, "nothing to submit")
            return
        picks = sorted(state.get("picks") or ())
        verdict = case_mod.judge(found, accused, picks)
        write("[{}] accusing {} ({}) with {} -> {} (remaining with that "
              "evidence: {})".format(
                  stamp(), accused, game.name_of(app, accused), picks, verdict,
                  case_mod.remaining_with(found, picks)))
        if verdict == case_mod.THIN:
            # 決着しない。
            # 画面には**誰を名指ししたかに触れない文**だけを出す
            # （触れると、この結果が出たこと自体が答えになる。`case.judge`）。
            times = case_mod.note_thin(found)
            store(app)
            screen.say(app, THIN_TEXT)
            screen.say(app, THIN_HINT_TEXT)
            write("    the evidence did not hold ({} time(s) so far)".format(
                times))
            state["accusing"] = False
            state["accused"] = None
            state["picks"] = set()
            repaint(app, "accusation too thin")
            return
        state["accusing"] = False
        state["accused"] = None
        state["picks"] = set()
        resolve(app, found, accused, picks=picks, verdict=verdict)
        repaint(app, "accuse done")

    def repaint(app, tag):
        """選択肢を組み直して画面に塗る。

        `refresh_choice_buttons` を呼ぶだけでは足りない。
        押下と同じ流れの中で差し替えると `app.buttons` は変わるのに画面は古いままになり、
        見えているものと押せるものが食い違う（GAME.md §2.3）。
        `ui.Screen.apply_buttons` は次のフレーム・メインスレッドで「組み直す→塗る」までやる。

        `entries=None` は「いま `app.buttons` に入っているものを塗り直す」。
        中で `refresh_choice_buttons` が走り、
        そこで `offer` が `state['accusing']` を見て並べ直す。

        `Clock` の無い環境（`tools/` のオフライン検証）では予約が効かないので、
        その場で組み直す。
        **押した結果が画面に出ないより出るほうがよい**。
        """
        if has_clock():
    ...
    # ------------------------------------------------------------ 判定
    def resolve(app, found, npc_id, picks=(), verdict=None):
        """告発の決着。照合は1行。AI には何も聞かない。一度きり。

        ここへ来るのは根拠が1人に絞れているときだけ（`submit`）。
        絞れていない告発は決着させずに聞き込みへ戻す。
        """
        right = case_mod.close(found, npc_id, picks=picks, verdict=verdict)
        store(app)
        name = game.name_of(app, npc_id)
    ...
    def close_cast(app, found):
        """決着したらキャストを世界から消す。当てても外しても同じ。

        以前は犯人に `is_dead` を立て、無実の者は町に残していた。
        町に住人が増えていく味はあったが、**繰り返し遊ぶとセーブが太り続ける**。
        1件4体、1体あたり 1.4〜8KB で、`npcs` はセーブの約2割。
        害はバイト数より、
    ...
    def exclusive(app, buttons, wanted):
        ...
        いま何をしている画面なのか分からなくなる。

        元の並びは預かっておいて、抜けるときに返す。
        捨ててしまうと、`refresh_choice_buttons` は `app.buttons` を組み直さない（組むのは施設の側）ので、
        やめて戻ったときに選択肢が消えたままになる。
        """
    ...
    def restore_stash(buttons):
        ...
        いま出ているのが自分の画面のときだけ戻す。
        告発の途中で施設を出るなどしてゲームが別の画面を組んだ場合、
        そこへ預かり物を書き戻すと関係の無い画面に古い選択肢が生える。
        その場合は黙って捨てる。
    ...
    def decline(app, why, buttons=None):
        ...
        「ギルドに立っているのに出ない」を外から詰めるのは難しい。
        塗り直すたびに書くとログが溢れるので、理由が変わったときだけ1行出す。
        """
    ...
    def busy_reasons(app, buttons):
        ...
        旗は「別の画面が出ている」ことの当て推量にすぎず、
        こちらには `is_facility_screen` という直接の判定がある。
        施設の選択肢が現に出ているなら、旗のほうが古い。
        挿す先はその選択肢そのものなので、別の画面へ紛れ込む心配も無い。
        """
    ...
    def offer(app):
        ...
        同じ画面でも並べるものが変わる。
        告発の相手を選んでいる最中は容疑者が並び、
        それ以外は「気になる噂を聞く」などが並ぶ。
        だから「自分のボタンが既に在るなら何もしない」では足りない。
        在るものと出したいものを見比べて、違うときだけ差し替える。
        （見比べずに毎回差し替えると、画面を塗り直すたびにボタンが作り直されて押下を取りこぼす。逆に見比べずに何もしないと、
        告発を押しても容疑者が出てこない。実際に後者を踏んだ）
        """
        ...
        if state.get("writing"):
            # 期限を過ぎていないか、ここでも見る。
            # `Clock` の予約だけに頼ると、それが飛ばなかったときにロックが残る（実機で 15:28 に飛ぶはずの見張りが飛ばず、
            # 押せないまま止まった）。
            ...
            # 待機表示の最中。
            # 選択肢に触らない。
            # `busy_on` が全枠に `...` を出しているので、
            # ここで組み替えても見えないうえ、解けた瞬間に一瞬だけ古い並びが見える。
            return False
        ...
        if not case_mod.is_active(found):
            ...
            leave_accusation()              # 町を出たら告発の途中もやめる
            return False                    # 事件の町の外では出さない
        elif state.get("accusing") and kind == START_FACILITY_TYPE:
            # 告発は専用の画面にする。
            # 施設の選択肢に混ぜて並べると、誰を告発するかを選ぶ場面なのに「出る」や「会話する」が並んでいて、
            # いま何をしている画面なのか分からない。
            # ここだけはゲームの選択肢を退けて、こちらのものだけを出す。
            #
            # 同じ画面でも2段ある。
            # 相手をまだ選んでいなければ容疑者、選んだ後は根拠の材料が並ぶ。
            if not state.get("accused"):
                for suspect in found.get("suspects", []):
                    wanted.append((
                ...
                picks = state.get("picks") or set()
                for item in case_mod.evidence(found):
                    on = item["id"] in picks
                    shape = PICK_ON_LABEL if on else PICK_OFF_LABEL
                    wanted.append((shape.format(text=pick_text(app, item)),
                                   PICK_MARK_PREFIX + item["id"]))
                # 1つも挙げていないうちは告発できない。
                # 根拠ゼロの告発は `judge` でも必ず `THIN` になるので、
                # 出しても押せる意味が無い（容疑者が1人の事件は組まない）。
                if picks:
                    wanted.append((SUBMIT_LABEL, "submit"))
                wanted.append((RESELECT_LABEL, "reselect"))
            ...
            if kind == START_FACILITY_TYPE:
                # いつでも告発できる。
                # 揃うまで待たせると、絞り込んだことに意味が無くなる（設計の選択）。
        ...
        if mine:
            # 同一性で外す。
            # 中身が同じ辞書が他にも在りうるので、値の比較で消すと巻き添えが出る。
    ...
    def start(app):
        ...
        LLM の推論は数秒〜数十秒かかる。
        押下と同じ流れで待つとゲームが固まるので、別のスレッドで書かせて、
        返ってきてからメインスレッドに戻って事件を始める（UI に触るのは必ずメインスレッド。
        `ui.Screen.schedule`）。

        顔ぶれは待ちに入る前に組む。
        後で組み直すと、書かせた人物と実際に立つ人物が別物になる。
        """
        ...
        # **ゲーム自身と同じ待機表示にする**（`.` → `..` → `...`）。
        # 選択肢が押せるままだと、
        # 書いている最中に町を出たり別の依頼を受けたりできてしまう。
        # `busy_on` は
        # `is_button_enabled=False` と送信ボタンの無効化までやる（TECH.md の一覧）。
        screen.busy_on(app)
        # 必ず解ける保証を先に置く。
    ...
    ACTIONS = {"start": start, "accuse": launch, "status": say_status,
               ...
               "submit": submit, "reselect": reselect}

    def action_for(mark):
        """印から処理を引く。相手と根拠だけは数が事件ごとに変わる。

        `accuse_<npc_id>` は容疑者の数だけ、`pick_<材料の id>` は
        集めたぶんだけあるので表には持てない。
        接頭辞で見分けて、その場で処理を作る。
        """
        if not mark:
        ...
        if mark.startswith(PICK_MARK_PREFIX):
            return pick_action(mark[len(PICK_MARK_PREFIX):])
        return None
    ...
    def conversation_starter(orig, messages, *args, **kwargs):
        ...
        **渡す messages のコピーだけ**を書き換える（GAME.md §2.5）。
        ...
        if line is None:
            return orig(messages, *args, **kwargs)
        # 第一声では事実そのものを言わせない。
        # 以前はここで事実を差し込み、印まで立てていた。
        # すると**会話に入って即座に抜けるだけで手がかりが手に入る**（実機）。
        # とくに容疑者の言い分は、犯人だけ裏の取れない場所を言うので、
        # 4人を出入りするだけで聞き込みを一切せずに犯人が割れた。
        # 印を立てるのは「プレイヤーが実際に何か尋ねた」ときだけにする（`brief` ＝
        # `conversation_facilitator`）。
        # 第一声は **「何か知っていそうだ」と匂わせるところまで**にとどめ、
        # 中身は尋ねられてから話す。
    ...
    #: 会話の相手が入っている属性の心当たり。
    #: `ConversationEndManager.__init__(self, app, in_conversation_id, ...)` なので1つ目が本命（`ui.conversation_partner` の記述より、
    #: 実セーブで確認）。
    ...
    def partner_of(manager):
        ...
        `frames.attr` は属性が無いと `MISSING`（`'<missing>'` という文字列）を返す。
        そのまま
        id として扱うと「相手が居るが誰とも一致しない」になり、**手がかりが永久に立たなくなる**（この検査を書いていて踏んだ）。
    ...
    def conversation_note(app, character):
        ...
        話しかけ方を問わない。
        普通に「会話する」で話しかけても同じ。
        最初の版は MOD のボタン経由のときだけ差し込んでいて、
        普通に話しかけると何も起きなかった（実機）。

        相手は2種類。

        - 証言者（施設の主）… 犯人についての事実を話す。
          会話を閉じると手がかりが立つ
        - 容疑者… 自分の立場と、その晩どこに居たかを話す。
          手がかりは立たない（言い分は事実ではない）が、控えには残す
        """
        found = current(app)
        if not case_mod.is_active(found):
            return None, None
    ...
    def brief(args, kwargs, index, name, where="", character_at=3):
        ...
        **ゲーム自身が持っている差し込み口**（リコンの署名より）。
        会話の受け答えを組むときにここが読まれるので、
        開始のひとことだけでなくプレイヤーが直接尋ねたときにも効く。
        ...
        if line is None:
            ...
            return args, kwargs
        # 話し始めた時点で覚えておく。
        # 会話が閉じたときに画面へ出す（判定は会話の中身ではなく「聞きに行った」行動）。
        ...
        # 届かなかった。
        # 引数の並びが版で動いたか、この関数は知識の枠を持っていない。
        # どちらなのかを次の1回で分かるようにする。
    ...
    # ------------------------------------------------------------------
    # 聞き込み中にどの経路が実際に走るのかを計測する自由入力で尋ねても知識が届かなかった（実機）。
    # フックは全部解決しているので、
    # 「呼ばれていない」か「引数の並びが違う」のどちらか。
    # 推測で当てにいかず、1回で確定させる。
    # 候補は会話まわりの LLM 入口すべて（リコンの署名より）。
    # 聞き込みの最中だけ、呼ばれた名前と引数の形を残す。
    # ------------------------------------------------------------------
    def watch(name):
    ...
    def conversation_end(orig, self, *args, **kwargs):
        ...
        try:
            if state.get("asking"):
                ...
                # 手が空いてから出す。
                # **要約の流し込み中に `add_text` するとこちらの出力が押し流される**（`300_` の実測。
                # `ui.when_idle`）。
                # `Clock` の無い環境（`tools/` のオフライン検証）では予約が効かないので、
                # その場で出す。
                # **出さないより出すほうがよい**（何が分かったのかが消えてしまう）。
                # `finish_asking` は二度呼んでも何もしないので、
                # 取りこぼしも二重出力も起きない。
                if has_clock():
...
#: 先頭が犯人、以降は容疑者（無実）。
#: 既存の NPC を使わないのは、
#: プレイヤーが愛着を持っている相手を犯人に仕立ててしまうから。
#: 事件のために生まれた人物なら、退場させても誰の思い入れも壊さない。
...
#: 事件のために作る登場人物の下地。
#: 特徴（片腕か・大柄か）は焼き付けない。
#: 割り当ては事件ごとなので、名前も profile も特徴へ触れない書き方にしてある。
#: 触れてしまうと（「隻腕の運び屋」のように）毎回同じ顔ぶれになるうえ、
#: 割り当てた特徴と食い違う。
#: 性別だけは名前と結び付くので下地が持つ。
#: 残りの軸は `assign_traits` が振る。
#: **名前に `"` などファイル名に使えない文字を入れないこと**（`110_` の対象）。
#: 同梱の材料（最後の受け皿）。
#: 事件の材料は `patterns/*.json` から読む（そちらを足せば事件の幅が増える）。
#: ここに置いてあるのは、**そのファイルが無い・壊れている・全部弾かれた** ときに使う最小限の一組。
#: 読めなかった束だけがこれで埋まる。
#: だからここを増やす必要は無い。
#: 増やすのは `patterns/` のほう。
BUILT_IN = {

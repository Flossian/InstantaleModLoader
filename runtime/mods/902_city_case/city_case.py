...
そうなった時点で、この遊びは受け入れられなくなる（利用者の指摘）。
...
事件が終われば犯人は `is_dead` で退場し、
無実の登場人物は町に残る（世界の住人が少し増えるだけで、誰の思い入れも壊さない）。
...
#: 手がかりを置ける施設の種類。
#: 町にある施設からしか選ばない。
#: 町の構成は世界ごとに違う（闇市や診療所の無い町もある。利用者の報告）。
...
#: 行き先の案内。
#: これが無いと、プレイヤーは町を無策で歩き回ることになる（利用者の指摘）。
#: 施設は種類ではなく名前で示す。
...
#: 会話を閉じた瞬間に手がかりだけが出る。**
#: 実機で「会話内容と得られたヒントが全くかみ合わない」ことになった（利用者の指摘）。
...
def apply(ctx):
    ...
    def all_telling(cast, axes):
        ...
        読む側には意味の無い1語で、「特徴が被りすぎ」に見える（利用者の指摘）。
    ...
    def cast_for(app, area, specs):
        ...
        プレイヤーが愛着を持っている相手が犯人にされうるからで、**その1点だけでこの遊びは成立しなくなる**（利用者の指摘）。
    ...
    def close_cast(app, found):
        ...
        1件4体、1体あたり 1.4〜8KB で、`npcs` はセーブの約2割（利用者の指摘）。
        害はバイト数より、
...
#: 先頭が犯人、以降は容疑者（無実）。
#: 既存の NPC を使わないのは、
#: プレイヤーが愛着を持っている相手を犯人に仕立ててしまうから（利用者の指摘）。

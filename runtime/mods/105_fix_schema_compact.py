...
## 実データでの検証（`output_data/` 12,067件 = ゲーム自身が保存した messages）

ゲームは LLM に投げた messages を `<ゲームdir>/output_data/<世界>/<PC>/<manager>/N.json`
にそのまま保存している。66 マネージャ種・46,931 メッセージを対象に、この mod の
関数をオフラインで通した結果:

| 項目 | 結果 |
|---|---|
| 埋め込みスキーマを検出 | 7,967 件（うち `$defs` 有り 4,097 / 無し 3,870） |
| 解析失敗 | **0 件** |
| 掴んだ dict が `title` + `type:'object'` でない（＝誤爆） | **0 件** |
| 圧縮後にフィールド名が消えた | **0 種** |
| 圧縮後に enum 値が消えた | **0 種** |
| 2回通すと結果が変わった | **0 件** |
| 1メッセージに2個目のスキーマがあった | **0 件** |
| スキーマを含むメッセージの合計 | 16,508,011 → 4,529,299 文字（**72.6% 減**） |

マネージャ別の削減率は 56%（`create_look`）〜79%（`vacation_scene_generator`）。
TECH.md の「平均約4割」より大きいのは分母の違いによるもので、あちらはリクエスト
本文全体、ここはスキーマを含むメッセージだけを分母にしている。
...
- 圧縮するのは **最初に見つかったスキーマ1個だけ**。上の実測では2個目を含む
  メッセージは 0 件だったので、実運用では制約になっていない
- `$defs` の中でも `properties` を持たない定義（Enum クラス単体など）は行を出さず、
  参照側に `$ref` の型名だけが残る。pydantic は `Literal` を参照先ではなく
  プロパティ側に展開するため、実測では enum 値の欠落は 0 件だった
- 置換後の方が長くなる場合は何もしない
...
NAME = "fix: compact embedded schema description in prompt (COMPACT, source-side)"

...
掲示板に「軽い頼まれごとを探す」を1つ足す。
押すとゲーム自身の依頼生成を呼び、討伐ではないお題の依頼を1件作って登録する。
受注も進行も帰還もゲーム本来のクエスト機構がそのまま動く。
この MOD が触るのは2つのプロンプトだけ。

##### なぜプロンプトだけで足りるのか（GAME.md §2.9 / VERIFICATION_LOG.md §2.18）

クエストを1件、頭から終わりまで通して `203_` の記録を読むと:

- **`ReturnAfterCompletion` は1ターン目から毎ターン作られている**（ボスが健在でも
  union に居る）。
  つまり「攻略しないと帰還できない」は**スキーマではなく地の文の指示**。
  referee の system に
  `return_after_completion: クエストを攻略した後にのみ実行可能` と書いてあるだけ
...
`Literal`**（`Literal[7→6→5→1]` と減っていく）。
未解決のクラッシュ
`AssertionError: literal "expected" cannot be empty, typing.Literal[]` は敵候補0件が第一容疑のまま。

残イベントが尽きると `FieldEvent` モデルは union ごと消える。
だから「候補0件のモデルは作らない」分岐は実在するが、**それを確認できたのは
`FieldEvent` だけ**。
`Battle` / `EncounterFinalBoss` で 0 件を試すのは、
既知の未解決クラッシュへ自分から踏み込むことになる。

> だから敵は生成させたまま残し、戦わせないのはプロンプトで行う。
> 空 `Literal` が生まれる状況を一度も作らないので、
> この MOD はそのバグと無関係でいられる。

生成された敵は「その土地に潜む危険」として設定は在るが、依頼の目的ではない。

##### アンカーは1メッセージに揃っていない

進行判定のプロンプトは system が2本＋user が1本で、**1文目（ラスボス宣言）は
system、クエスト名と【強制事項】は user** にある。
したがって「どのクエストか」の判定は
`messages` 全体を繋いだもので行い、**書き換えは各メッセージに在るアンカーだけ**に適用する。
1メッセージ単位で判定すると、必ずどちらかが「見つからない」になる。

##### 置き換える文字列は実データで裏を取ってある

`output_data/` の記録（`quest_referee*` の 424 ターン、
`*quest_generator` 113件）全件に対して、
下の `REFEREE_*` / `GEN_KIND_MARK` が存在することを確認済み（VERIFICATION_LOG.md
§2.11）。
1つでも当たらなければ `mini_quest.log` に
`missed:` が出る（＝ゲーム側の文面が変わった合図）。
**当たらなかった時は書き換えを丸ごと諦める**。
中途半端に混ざった指示を送るより、討伐クエストとして正しく動くほうがよい。

##### `PhaseSpec` に自前のクラス名を書かない

ボタンは `PhaseSpec.to_dict()` でセーブに焼かれうる。
自前のクラス名を書くと MOD 無しの次回起動で `getattr(__main__, ...)` が失敗する。
無害な既存クラスを持たせ、押下は `on_button_press` を包んでボタン辞書の印で横取りする（`301_` と同じ。印のキーは別にすること。
同じにすると押下を食い合う）。
"""
...
# どの依頼がこの MOD 製かの控え。
# セーブには書かない（クエスト辞書に独自キーを足すとセーブに焼かれるうえ、
# 再読み込み後に Quest がそのキーを持つ保証が無い）。
...
# こちらは掲示板が並び終えた後。
# 掲示板がそのつど一覧を組み直すビルドなら残骸はゲーム自身が消している。
...
# 押下を横取りするための印。
# **`301_`（mod_action）/ `302_`（mod_party_action）と別のキーにすること。**
...
# お題の種類。
# 順に選ばれる（乱数を使わないのは、どれが出たかログで読めるようにするため）。
...
# 達成条件が曖昧だと `nothing_happens` が延々と続く。
# 判定条件と締切の両方が要る。
TURN_BUDGET = 10

# 生成させるフィールドイベントの数。
# 素のプロンプトは「1から3個程度」で、それだと3ターン目で枯れる。
# 戦闘を封じたぶんイベントが依頼の中身そのものになるので、多めに要る。
EVENT_COUNT = "4から6"

# 「目的を果たしたうえでの帰還」を達成として扱う。
# **プロンプトでの説得では効かない**（VERIFICATION_LOG.md §2.20 / §2.22）。
# referee 自身が「目標の数は既に十分」と描写しながら
# `retire_from_the_quest` を選ぶ。
# 素の説明文が「プレイヤーがその意思を明確に示したときにのみ選択するべき」なので、
# 帰還の意思＝放棄という読みが強く、書き換えても押し切られる。
# ミニクエストには倒すべきラスボスが居ない。
# **目的を果たした状態での帰還は定義上そのまま達成**なので、戻り値の側で差し替える。
# 文面での説得をやめて判定そのものを持つ、
# という切り替え（TECH.md §7「ゲームの処理そのものを起こさせない」の系統）。
...
# 生成の印が古くなったら使わない（押してから生成が始まるまでの間に別の生成が割り込んだ場合に、
# そちらへ付いてしまうのを防ぐ）。
INJECT_TTL = 300.0

# 控えの上限。
# 世界ごとにこの件数を超えたら古いものから捨てる。
MAX_RECORDS = 200
...
REFEREE_RULES = (
    ...
    # 同じ取り違えを起こすもう1行。
    # 「エリア外への移動＝攻略を諦めての撤退」と定義してしまっているので、
    # 帰還そのものが失敗に寄る。
    (
    ...
    # イベントの在庫が尽きると `nothing_happens` が延々と続き、
    # ほぼ同じ描写が繰り返される（VERIFICATION_LOG.md §2.19）。
    # 「何も起きない」を選ぶこと自体は正しくても、**同じ場所で足踏みし続けるのは進行の放棄**なので、
    # そこを塞ぐ。
    (
...
# 行の先頭で当てて、その行の末尾に書き足す規則。
# イベントも雑魚も尽きた終盤は、残る選択肢が実質「ラスボスとの battle」しか無くなってそこへ流れる（VERIFICATION_LOG.md
# §2.21）。
# `- battle:` の行は末尾の適正数だけが難易度で変わるので、完全一致では当たらない。
# 行頭で当てて追記する。
# 追記にするのは、
# `- 残りラスボス戦闘:` のようにゲームが握っている事実の行を書き換えたくないため（数や名前はゲームの状態そのもの。
# 嘘を書かず、扱い方だけ足す）。
REFEREE_LINE_RULES = (
    (
...
REFEREE_LINE_SWAPS = (
    # **達成したのに「撤退」になる**ところ（VERIFICATION_LOG.md §2.20）。
    # 素の説明文が「プレイヤーがその意思を明確に示したときにのみ選択するべき」なので、
    # 帰還の意思＝放棄 と読まれる。
    # 討伐クエストでは正しいが、お使いでは逆。
    (
...
def head_replacement(objective, summary=None):
    ...
    種類のテンプレ文（「指定された物を集める」）だけでは
    referee が達成を判定できず、`return_after_completion` を一度も選ばない。
    **そのクエストの `request_summary` をそのまま渡す**のが要点（VERIFICATION_LOG.md
    §2.20）。

    在庫消化の圧力を打ち消す1文も要る。
    プロンプトには `- 残り通常戦闘回数` `- 残り中ボス戦闘`
    `- 残りラスボス戦闘` が事実として並び続けるので、
    明示的に「消化しなくてよい」と言わないと battle に流れる。
    """
    lines = [
...
def rewrite_referee_text(content, objective, combat_mode=None, summary=None):
    ...
    for old, table in REFEREE_RULES:
        ...
        if replacement is None:
            continue
        # **置換後の文が元の文を含む項目がある**（`nothing_happens` は元の説明に書き足す形）。
        # 素朴に replace を繰り返すと、
        # 既に書き換わった文にもう一度当たって際限なく伸びる。
        # 実経路ではプロンプトは毎ターン組み直されるので一度しか通らないが、
        # 冪等でない書き換えを残す理由が無い。
    ...
    # 行頭で当てて末尾に書き足すもの。
    # 同じ行に二度足さない（`chat` は同じ会話履歴を何度も運ぶので、
    # 追記が積み上がると際限なく伸びる）。
    lines = new.split("\n")
...
def rewrite_generator(prompt, kind):
    ...
    差し替えるのは種類の札1個だけ。
    敵の生成要素には触らない（理由はモジュールの docstring）。
    お題の中身は `area_description` 側に足す。
    """
...
def generator_brief(kind):
    ...
    出力スキーマ（`QuestStructure`）にも呼び出し側にも影響しない（`301_` と同じ手）。
    """
    return (
        "\n\n【この依頼の種類 ― 最優先で反映すること】\n"
        "{brief}\n"
...
def _get(container, name):
    ...
    `quest_referee*` の戻り値の形は実測できていない（`output_data/` に残るのは保存側が整えた形で、
    関数が返したそのものではない）。
    dict とオブジェクトのどちらでも通るようにしておき、**どちらでもなければ何もしない**（推測して壊すより素通しするほうがよい）。
    """
...
def _set(container, name, value):
    ...
    try:
        ...
        # pydantic の Literal で弾かれることがありうる。
        # その場合は書けない。
        return False
...
def apply(ctx):
    ...
    def load_records():
        ...
        ここは読んだ結果に足して `ctx.write_json` で書き戻す経路なので、
        後者を黙って倒すと**その1回で控えが丸ごと空になる**。
        壊れずに、静かに失われる。
        """
    ...
    def remember_quest(app, quest_id, kind, title, summary):
        ...
        bucket[str(quest_id)] = {
            ...
            "title": title or "",
            # 達成条件そのもの。
            # これが無いと referee は完了を判定できず、
            # `return_after_completion` が一度も選ばれない。
            "summary": summary or "",
    ...
    def objective_for_title(title):
        ...
        進行プロンプトには id が入らないので名前で照合する。
        世界をまたいで同名の依頼が並ぶ可能性はあるが、その場合でも壊れ方はしない。
        討伐依頼が 1件おとなしくなるだけで、例外にもデータの破損にもならない。
    ...
    # その作法はローダに集約してある（`301_` / `307_` と共有。TECH.md §3.2.3）。
    ...
    def generate(app):
        ...
        **別スレッドに投げない。ここで最後までやりきる**（`301_` の教訓）。
        ...
        同期的に待つのが正しい。
        投げっぱなしにすると LLM が回っている最中にプレイヤーが移動できてしまう。
        """
        if state["generating"]:
        ...
        # 待機表示を出さないと**画面が固まったように見える**ので、
        # ゲーム自身と同じ「.」→「..」→「...」を出す（中身は `ui.Screen`。
        # GAME.md §2.4）。
        screen.busy_on(app)
        ...
        # 受注画面へは自分で飛ばない。
        # `QuestChoiceManager` を自前で組んで組み立てると
        # `quest_type` の語彙が合わずに落ちる（GAME.md §2.2）。
        # 掲示板を開き直せば、作ったばかりの依頼もゲームが正しい
        # spec で並べてくれる。
        # `restore=False` ＝ この後すぐ掲示板を開くので、
    ...
    def quest_board_buttons(orig, self, *args, **kwargs):
        """掲示板が並び終えた後に、自前の項目を1つ足す。

        ゲームが作った依頼ボタンには一切触らない（`quest_type` の語彙を知らずに済ませるのが要点。
        触ったら台無しになる）。
        """
        ...
        try:
            ...
            # **`refresh` だけでは画面が塗り替わらない**（`302_` の実測）。
            # しかもここはゲームが掲示板を組み終えた直後なので、次のフレームで塗る。
    ...
    def on_button_press(orig, self, button_index, *args, **kwargs):
        ...
        文字列ではなく印で見る（同じ文言のゲーム側ボタンを巻き込まないため）。
    ...
    def random_quest_generator(orig, world_overview, settlement_name,
        ...
        印は生成が終わるまでしか立っていないので、
        掲示板から普通に作られた依頼や `301_` が会話から作る依頼は素通しする。
        """
        mark = state["pending"]
    ...
    def plan(blob):
        ...
        判定は全メッセージを繋いだもので行う。
        進行判定のプロンプトは 1文目が system、
        クエスト名と【強制事項】が user と分かれているので、
    ...
    def audit(kind_of_change, before, after, note=""):
        ...
        枠は生成と進行で別に持つ。
        1つの枠だと、生成が内部で何回も
        LLM を呼んだ回に枠を使い切り、**肝心の進行側の中身が1行も残らない**。
        """
        state["rewrites"] += 1
    ...
    def rewrite_outgoing(texts, site):
        ...
        `LlamaCppClient.chat` だけに仕掛けないこと。
        版4までがそうで、クラウド（APIキー）実行では chat を一度も通らないため、
        討伐前提を外す書き換えが丸ごと落ちていた。
        しかも `plan()` 自体が呼ばれないので
        `missed:` の警告すら出ず、**無音で普通の討伐クエストになる**。
        `119_` が同じ落とし穴を踏んでいる（TECH.md §5.3 / VERIFICATION_LOG.md
        §2.41）。

        仕掛け先の選択と「1回の推論で1回だけ」はローダ側が持つ。
        ここは何をどう書き換えるかだけを持つ。
        """
        decision = plan("\n".join(texts))
        if decision is None:
...
# 実経路はクエストを1件受注するまで通らないので、
# 起動直後に確かめておく（`105_` と同じ方針）。
# --------------------------------------------------------------------------
    ...
    if missed:
        ...
        if body is None:
            ...
            for old, table in REFEREE_RULES:
                ...
                if new is None:
                    continue
                # 置換後の文が元の文を含む項目（`nothing_happens` は元の説明に追記する形）があるので、
                # 「元が残っているか」では判定できない。
                # 置換後の文が入っているかで見る。

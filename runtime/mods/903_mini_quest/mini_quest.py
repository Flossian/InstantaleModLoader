...
`Literal`**（`Literal[7→6→5→1]` と減っていく）。未解決のクラッシュ
`AssertionError: literal "expected" cannot be empty, typing.Literal[]` は
**敵候補0件が第一容疑**のまま。

残イベントが尽きたとき `FieldEvent` モデルが union ごと消えるので「候補0件のモデルは
...
`output_data/` の記録（`quest_referee*` の 424 ターン、`*quest_generator` 113件）
全件に対して、下の `REFEREE_*` / `GEN_KIND_MARK` が存在することを確認済み
（VERIFICATION_LOG.md §2.11）。1つでも当たらなければ `mini_quest.log` に
`missed:` が出る（＝ゲーム側の
...
# 達成条件が曖昧だと `nothing_happens` が延々と続く。**判定条件と締切の両方が要る。**
TURN_BUDGET = 10

# 生成させるフィールドイベントの数。
# 素のプロンプトは「1から3個程度」で、それだと**3ターン目で枯れる**。
# 戦闘を封じたぶんイベントが依頼の中身そのものになるので、多めに要る。
EVENT_COUNT = "4から6"

# 「目的を果たしたうえでの帰還」を達成として扱う。
#
# **プロンプトでの説得では効かない**（VERIFICATION_LOG.md §2.20 / §2.22）。referee 自身が
# 「目標の数は既に十分」と描写しながら `retire_from_the_quest` を選ぶ。素の説明文が
...
# 判定そのものを持つ、という切り替え（TECH.md §7「ゲームの処理そのものを
# 起こさせない」の系統）。
#
...
REFEREE_RULES = (
    ...
    # イベントの在庫が尽きると `nothing_happens` が延々と続き、ほぼ同じ描写が
    # 繰り返される（VERIFICATION_LOG.md §2.19）。「何も起きない」を選ぶこと自体は
    # 正しくても、**同じ場所で足踏みし続けるのは進行の放棄**なので、そこを塞ぐ。
    (
...
#
# イベントも雑魚も尽きた終盤は、残る選択肢が実質「ラスボスとの battle」しか
# 無くなってそこへ流れる（VERIFICATION_LOG.md §2.21）。`- battle:` の行は末尾の適正数
# だけが難易度で変わるので、**完全一致では当たらない** ― 行頭で当てて追記する。
#
...
REFEREE_LINE_SWAPS = (
    # **達成したのに「撤退」になる**ところ（VERIFICATION_LOG.md §2.20）。素の説明文が
    # 「プレイヤーがその意思を明確に示したときにのみ選択するべき」なので、
...
def head_replacement(objective, summary=None):
    ...
    種類のテンプレ文（「指定された物を集める」）だけでは referee が達成を判定
    できず、`return_after_completion` を一度も選ばない。**そのクエストの
    `request_summary` をそのまま渡す**のが要点（VERIFICATION_LOG.md §2.20）。
...
def apply(ctx):
    ...
    def remember_quest(app, quest_id, kind, title, summary):
        ...
        bucket[str(quest_id)] = {
            ...
            "label": kind["label"],
            "title": title or "",
            # **達成条件そのもの。** これが無いと referee は完了を判定できず、
    ...
    def generate(app):
        ...
        **別スレッドに投げない。ここで最後までやりきる**（`301_` の教訓）。
        ...
        # 同じ「.」→「..」→「...」を出す（中身は `ui.Screen`。GAME.md §2.4）。
        screen.busy_on(app)
        ...
        # 組み立てると `quest_type` の語彙が合わずに落ちる（GAME.md §2.2）。
        # 掲示板を開き直せば、作ったばかりの
    ...
    def audit(kind_of_change, before, after, note=""):
        ...
        **枠は生成と進行で別に持つ。** 1つの枠だと、生成が内部で何回も LLM を
        呼んだ回に枠を使い切り、**肝心の進行側の中身が1行も残らない**。
        """
        state["rewrites"] += 1
    ...
    def rewrite_outgoing(texts, site):
        ...
        **`LlamaCppClient.chat` だけに仕掛けないこと。** 版4までがそうで、
        ...
        `119_` が同じ落とし穴を踏んでいる（TECH.md §5.3 / VERIFICATION_LOG.md §2.41）。

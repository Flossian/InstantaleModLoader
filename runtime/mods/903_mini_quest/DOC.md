# `903_mini_quest`: 戦闘なしミニクエスト（分離した記録）

この MOD は**開発中（9xx）**。git には入れるが、CI・配布物・`load_order.json`・
`docs\` の4冊には入らない（TECH.md §2.6）。遊び方も検証の記録も、この1枚に
まとめてある。
...
| 3. オフライン検証のコマンド | `docs\VERIFICATION.md` §2.11 の一覧 |
| 4. 実機の記録（4回） | `docs\VERIFICATION.md` §2 の末尾（節番号は振り直す） |
...
## 3. オフライン検証（`VERIFICATION.md` §2.11 の一覧に戻す）

```powershell
python tools/test_wip_mini_quest.py    # 903_  119件（うち4件は output_data/ の実プロンプトと突き合わせ）
```

**`output_data/` の記録は `111_` 適用後**である点に注意（VERIFICATION.md §2.43）。
目印を行末まで含めた完全一致にすると当たらなくなる。

---

## 4. 実機の記録（`VERIFICATION.md` §2 に戻す）

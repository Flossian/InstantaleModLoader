...
枠線はウィジェットの canvas に描かれているので、**複製には付いてこない**
（足した枠だけ枠線が無い状態になる）。線を自分で引くのではなく、
`scripts.hud.new_hud` の `add_border(widget)` を借りる。色も太さも寸法追従も
ゲームのものになる。

中身の大きさも複製では食い違う（立ち絵が枠の左へはみ出す）。ゲームの枠は
...
（`panel.family_of`。これが無いと `113_` と同じく「中身だけ広がって枠線が残る」）。
...
**透けて見え、押せてしまう**（下の「会話する」の文字が枠に重なる）。
...
全部拾う作りにすると、**ゲームの選択肢ボタンまで掴んで、戻したあとも押せなく
なる**（GAME.md §2.8）。理由は控えの取り方にある ― ゲームは応答待ちの
...
# （そのままだと絵が枠の左へはみ出す）。どちらに寄せるかを選べるようにする。
...
def apply(ctx):
    ...
    def cover_gap(hud, box, design):
        ...
        控えて書き戻すと選択肢が死ぬ。GAME.md §2.8）なので、こちらが1枚敷く。

        **板は帯の中に入れてはいけない。** ゲームの `update_party_display` は
        帯の子を1つずつ「パーティの枠」として塗るので、枠の形をしていない板が
        混じると `cell.children[0]` が `IndexError` で落ちる（GAME.md §2.8）。
        板は HUD 直下の入れ物へ、**帯のすぐ後ろ**

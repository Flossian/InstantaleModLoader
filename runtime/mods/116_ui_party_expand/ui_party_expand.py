# -*- coding: utf-8 -*-
"""追加: パーティ欄を広げて、**4人目以降の仲間**を画面に出す。

## 何が困っているか

仲間を3人雇うと4人パーティになるが、画面右のパーティ欄には3人までしか出ない。
名簿から漏れているわけではなく（会話も戦闘も4人目を含めて進む）、
**枠が3つしか作られていない**。実測（`instantale.exe` の定数表、main_023）:
...
`update_party_display` は `party_members` を見て枠を塗るが、枠が3つなので
4人目には塗る先が無い。**足りないのは枠**であって、データでも塗り方でもない。

そこで `113_ui_text_expand` と同じ形にする。ボタンを1枚足し、押すと帯が
上へ伸びて足りないぶんの枠が現れる。もう一度押すと元の3枠に戻る。
名簿・セーブ・戦闘には一切触らない。

## 枠を増やす（型を名指ししない）

枠は**今そこに在る枠を複製して**作る（`panel.clone`）。`ClickableFloatLayout` も
`StencilFloatLayout` も、こちらからは名前しか分からない。`type(cell)()` なら
そのビルドが持っているクラスがそのまま使われるので、名指しせずに同じ形が作れる。
中身（立ち絵の `Image` など）も同じやり方で降りて写す。

複製した枠は `hud.party_cells` に**追記する**。この並びの添字が
`on_member_label_press(label_index)` の相手そのものなので（GAME.md §2.8）、
追記しておけば押したときの経路がゲーム自身のものになる。並べ替えては
いけない ― 添字がずれると、押した相手と話す相手が食い違う。

### 埋めるのはまずゲームに任せる

枠を足したら、先に `hud.update_party_display()` を1回呼ぶ。`party_cells` を
なめて塗るビルドなら、**それだけで4人目が出る**（こちらは塗り方を知らずに済む）。

塗られなかったときだけ自分で埋める。そのときも埋め方は発明しない。
**既に埋まっている枠**と `party_members` のその相手のデータを突き合わせ、
文字列が一致した場所を「そこに入る値」とみなす（`panel.learn_fields`）。
立ち絵のパスがどのキーから来るのかを、こちらが決め打ちしない。

### 枠線と、中身の見え方

枠線はウィジェットの canvas に描かれているので、複製には付いてこない（実機で
「足した枠だけ枠線が無い」。2026-08-03）。線を自分で引くのではなく、
`scripts.hud.new_hud` の `add_border(widget)` を借りる。色も太さも寸法追従も
ゲームのものになる。

中身の大きさも複製では食い違う（実機で立ち絵が枠の左へはみ出した）。ゲームの枠は
絵を縦横比のまま置いて `StencilFloatLayout` で切り取っているが、複製ではその
切り取りが効いていない。そこで**見本の枠を測って、同じ相対位置・同じ大きさを
複製に入れ直す**（`panel.sync_geometry`）。中身を入れた**後**に呼ぶ ―
立ち絵を差し替えると絵の大きさが変わるので、入れる前に合わせても意味が無い。

## 伸びる向き（上だけ）と、元の枠を1 px も動かさないこと

帯の下端は動かさない。Kivy の `y` は下端なので、`y` を据え置いて `height` だけ
増やすと**下端が動かず上へ伸びる**。帯の下には画面の縁しか無く、上に在るのは
背景画像なので、下端が動かないことがそのまま「他をずらさない」になる。

**枠は比率ではなく実測の座標で置く**（`panel.pin`）。最初は各枠の `size_hint_y` を
`1/行数` に割り直していたが、ゲームが使っているのは `0.33` という丸めた比率で、
こちらの `1/4`（＝0.25）と食い違う。伸びた帯に当て直すと、その差が
**元から在る枠を数 px 動かす**（実機で確認、2026-08-03）。そこで元の枠は
測った位置と大きさに釘付けし、足した枠だけを**行の送り幅**（隣り合う枠の y の差。
`panel.row_pitch`）ぶんずつ上へ積む。帯の高さも同じだけ足す（倍率ではなく
足し算 ― 枠線は帯より数 px 大きいので、倍率で配ると差まで倍になる）。

窓に入る行数で頭打ちにする（`MAX_ROWS` と窓の高さの小さい方）。

枠線・背景は帯とは別のウィジェットが描いていることがあるので、**今その帯と
同じ場所に同じ大きさで置かれているもの**を仲間として一緒に伸ばす
（`panel.family_of`。`113_` が実機で「本文だけ広がって枠線が残る」を踏んだ）。

## 重なったところの始末（2通り）

帯を上へ伸ばすと、そこに元から在ったものが帯の下に入る。相手によって手が違う。

### ゲームのもの ― **黒い板を1枚敷く**（相手には触らない）

足した枠は背景を持たない（絵と枠線だけ）ので、下にあるゲームの選択肢ボタンが
**透けて見え、押せてしまう**（実機で「会話する」の文字が枠に重なった。2026-08-03）。
...
**板を帯の中に入れてはいけない。** ゲームの `update_party_display` は帯の子を
1つずつ「パーティの枠」として塗るので、枠の形をしていない板が混じると
`cell.children[0]` で落ちる（実機でクラッシュ。2026-08-03、別れた仲間を雇い直した
場面。`cells` は6件で先頭がこの板だった）。

> 他人の入れ物に、他人が数えている物と違う物を混ぜない。

板は HUD 直下の入れ物へ、**帯のすぐ後ろ**に入れる（`panel.behind_index`）。
Kivy は `children` を逆順に描くので、ここは「帯より下・ゲームのボタンより上」に
なる唯一の位置で、触りの判定でも帯より後になる。だから枠の押下は今までどおり
枠に届き、その外側は板で止まる。

### MOD が足したもの ― 隠して無効にする

`113_ui_text_expand` の切り替えボタンは、既定で**まさにこの帯の真上**に出る。
これは板より上に描かれるので板では隠れない。広げている間は
**見えなくし、かつ押せなくする**（`opacity=0` と `disabled=True` の両方。
片方だけでは「見えないのに押せる」か「押せないのに見える」が残る）。

**ただし、こうしてよいのは MOD が足したボタンだけ。** 見るのは HUD 直下の入れ物の
**直接の子だけ**にする（`panel.coverable`）。木を降りて「`disabled` を持つもの」を
全部拾う作りにしていたら、**ゲームの選択肢ボタンまで掴んで、戻したあとも押せなく
なった**（実機で踏んだ。2026-08-03）。理由は控えの取り方にある ― ゲームは応答待ちの
間だけ選択肢を無効にするので、その瞬間に控えると「無効」を控えることになり、
畳んだときにそれを書き戻して、ゲームが有効に戻した後でも無効に落とす。

> 他人が管理している状態を控えて書き戻してはいけない。控えた瞬間と書き戻す瞬間の
> 間に、持ち主がその値を変えている。

MOD が足すボタンはその入れ物の直接の子になる（`get_current_screen_root` を
壊さないため、こちらも `113_` もそこへ足す決まり）。ゲームの選択肢はもっと深い
入れ物の中に居るので、この網には入らない。念のため `hud.buttons` /
`right_buttons` / `icon_buttons` / `party_cells` の中身も避ける
（`panel.game_buttons`。名前で引くのが「触る」ためではなく**避ける**ためなので、
引けなくなっても危ない側へ倒れない）。

**こちらのボタンは対象外** ― 隠すと戻す手段が無くなる。

畳むときは、畳めるかどうかに関わらず**必ず先に戻し、板も外す**。加えて注入の
たびに1回、隠しっぱなしの相手と置き去りの板が無いか木を掃く（`heal`）。
押せないボタンや見えない板が画面に残るのは遊べなくなることなので、
**注入し直せば直る**状態にしておく。

## 元に戻せること

戻すために覚えるのは、**こちらが触る前の寸法一式**（帯と各枠の `size_hint` /
`size` / `pos` / `pos_hint`）。これを**帯のウィジェット自身に持たせる**
（`DESIGN_ATTR`）。MOD 側の変数に持つと、注入し直したときに「広げた後の寸法」を
設計値として控えてしまい、押すたびに帯が育つ（`112_` が `line_height` で
踏んだ罠と同じ）。広がっている帯からは**絶対に控えない**。
...
中身は**行数が変わらなくても見直す**。1人抜けて1人入れば人数は同じままだが顔ぶれは
変わるので、行数の変化だけを合図にすると古い顔が残る。毎回呼ぶかわりに、入っている
id の列が同じなら何もしない。

## 窓の大きさが変わったとき

塗り直し（`update_party_display` / `update_display_text`）は中身が変わった
ときにしか来ない。**窓だけ変えられるとこちらは何も気付けず**、控えている寸法は
古い窓の値のまま残る。そこで `Window.on_resize` を直に拾う（`watch_window`）。
拾った後は「畳む → 控えを捨てる → 1フレーム置いて控え直して当て直す」の順。
間に挟まる1フレームで、ゲームが自分の寸法を入れ直す。

## 触らないもの

名簿（`app.party` / `game_variables['party']`）、`party_members` の中身、
セーブ、戦闘。このMODが変えるのは帯と枠の寸法、複製した枠、自分で足した
ボタン1枚だけ。剥がす（`--unload`）とフックは外れるが、足した枠とボタンは
...
# 広げたときに何行まで増やすか。窓に入る行数と、この値の小さい方で頭打ち。
MAX_ROWS = 8

# 広げている間、帯に重なるボタンを隠して押せなくするか。
HIDE_COVERED = True

# ボタンの置き場所。既定はパーティ欄（画面右の帯）のすぐ上・右寄せ。
# `113_ui_text_expand` のボタンは同じ帯の上の**左寄せ**なので、ぶつからない。
BUTTON_CORNER = "帯の上"
...
# 広げた帯が窓に占めてよい割合。窓の縁まで詰めない。
MAX_FILL = 0.98

# 隠す相手の大きさの上限（窓の面積に対する割合）。これより大きいものは
# ボタンではなく入れ物なので触らない。
MAX_HIDE_AREA = 0.2

# 足した枠の立ち絵の入れ方。ゲームの枠は絵の縦横比のまま枠からはみ出させ、
# `StencilFloatLayout` で切り取っているが、複製ではその切り取りが効かない
# （実機で左へはみ出した。2026-08-03）。どちらに寄せるかを選べるようにする。
...
# 仲間が増減してから、もう一度当て直すまでの秒数。名簿が動いた直後はまだ画面の側
# （`party_members`）が古く、`InstantaleApp.update_party_member` が 0.1 秒ごとに
# 入れ直している。その周期をまたぐだけの間を置く。
REFRESH_DELAY = 0.2

# 記録の上限。塗り直しは何度も来るので、書くのは節目だけ。
MAX_LOG = 80
...
def apply(ctx):
    ...
    def design_of(hud, box, cells):
        ...
        if rows_now(box) is not None:
            # 広がっている帯からは控えない。今の寸法はこちらが入れた値なので、
    ...
    def press_member(hud, cell):
        ...
        `on_member_label_press(label_index)` は `hud.party_cells[index].member_id`
        から相手を決める（GAME.md §2.8）。複製をその並びに追記してあるので、
        添字を渡すだけで、相手の決め方も効果音も会話の入り方もゲームのままになる。
        """
        app = running_app()
    ...
    def ensure_extras(hud, box, design, count):
        ...
        while len(extras) < count:
            ...
            # 枠線は canvas に描かれていて複製には付いてこない。ゲーム自身の
            # 描き方を借りて引き直す（実機で「足した枠だけ枠線が無い」）。
    ...
    def fill_extras(hud, cells, extras):
        ...
        行数の変化だけを合図にすると、そのとき**古い顔が残る**。
        毎回呼ぶかわりに、**変わっていなければ何もしない**（比べるのは id の列だけ）。
        """
        if not extras:
            return
        ...
        if state["game_paints"]:
            # 塗るのはゲーム自身。こちらが割り込むと取り合いになる。ただし
            # **枠を増やした直後は空のまま**なので、そのときだけ塗り直しを頼む
            # （ゲームは `party_members` が動いたときしか塗らない。雇い直しで
            # 枠が1つ増えた場面がこれ。2026-08-03）。
            if empty:
                ask_game(hud, extras)
            return
    ...
    def cover_gap(hud, box, design):
        """新しく覆った場所に**黒い板**を敷く（ゲームのボタンには触らない）。

        足した枠は背景を持たないので、下にあるゲームの選択肢が透けて見え、
        押せてしまう。ゲームのボタンを無効にするのは禁じ手（応答待ちの値を
        控えて書き戻すと選択肢が死ぬ。2026-08-03）なので、こちらが1枚敷く。

        **板は帯の中に入れてはいけない。** ゲームの `update_party_display` は
        帯の子を1つずつ「パーティの枠」として塗るので、枠の形をしていない板が
        混じると `cell.children[0]` で落ちる（実機でクラッシュ。2026-08-03、
        別れた仲間を雇い直した場面）。板は HUD 直下の入れ物へ、**帯のすぐ後ろ**
        に入れる ― 描画は帯より下、触りの判定は帯より後になるので、枠の押下は
        今までどおり枠に届き、その外側は板で止まる。
        """
        if not HIDE_COVERED:
            return
    ...
    def unhide(box):
        ...
        for widget in covered if isinstance(covered, list) else []:
            ...
            try:
                ...
                # 消せない相手には番人（`frames.MISSING`）を書き戻す。次に
                # 広げたときの「もう隠してある」判定はこれを見て**控え直す**側に
                # 倒れるので、戻した後の値がそのまま次の控えになる。
                try:
    ...
    def arrange(design, extras):
        ...
        元の枠は実測どおりの座標に釘付けし（`pin`）、足した枠だけを、その上へ
        行の送り幅ぶんずつ積む。比率（`size_hint` / `pos_hint`）で置き直すと、
        ゲームが使っている 0.33 のような丸めた比率と、こちらが割り直した
        1/4・1/5 が食い違って**元の枠が数 px 動く**（実機で確認、2026-08-03）。
        """
        pitch = party_panel.row_pitch(design)
    ...
    def align(design, extras):
        ...
        **中身を入れた後に呼ぶ。** 立ち絵を差し替えると絵の大きさが変わるので、
    ...
    def expand(hud, box, cells, design):
        ...
        if (rows_now(box) == rows
            ...
            # もう広がっている。寸法には触らないが、中身の見え方だけは毎回
            # 合わせ直す（ゲームが枠を塗り替えると絵の大きさが変わる）。
            # 寸法には触らないが、**中身は毎回見直す** ― 人数が同じままでも
            # 顔ぶれは入れ替わる（1人抜けて1人入る）。
            extras = extras_of(box)
        ...
        fill_extras(hud, cells, extras)
        align(design, extras)         # 中身を入れた**後**に見え方を合わせる
    ...
    def collapse(hud, box):
        # **畳めるかどうかに関わらず、隠したものは必ず先に戻す。** 途中で
        # 降りる道のどれかが unhide を飛ばすと、隠したボタンが押せないまま残る。
        unhide(box)
        uncover_gap(box)
    ...
    def heal(hud):
        ...
        画面を作り直された・注入をまたいだ・こちらが途中で落ちた ― どの道でも、
        隠したまま行方不明になった相手が残りうる。押せないボタンが画面に居座るのは
        遊べなくなることなので、控え（`HIDDEN_ATTR`）を持つ相手を木から拾い直して
        戻す。**注入し直せば直る**状態にしておく。
        """
        restored = 0
        ...
        def descend(widget, depth):
            if frames.attr(widget, party_panel.BLOCK_RECT_ATTR, None) is not None:
                # 置き去りになった黒い板。**そこの触りを止め続ける**ので必ず外す。
    ...
    def adopt(box):
        ...
        `apply()` は何度でも呼ばれる（TECH.md §3.6）。MOD 側の変数は作り直される
        ので、ここで読み直さないと「広げて遊んでいたのに、注入し直した拍子に戻る」。
        """
        if state["synced"]:
            return
        state["synced"] = True
    ...
    def on_window_resize(*_args):
        ...
        if hud is None:
            return
        # 組み直しが済むまで寸法は**測らない**。ここを止めておかないと、途中の
        # （古い窓のままの）寸法を新しい設計値として控えてしまう。
        state["stale"] = True
    ...
    # 仲間が増えた・減ったその場で当て直す。**塗り直しを待たない。**
    #
    # 画面の側（`party_members`）は `InstantaleApp.update_party_member` が
    # 0.1 秒ごとに入れ直しているので、放っておいてもその周期で追いつく。
    # だが「雇った瞬間に枠が増える」「別れた瞬間に減る」を確かめられるのは
    # ここだけなので、名簿が動いた地点から直に当て直す。
    #
    # 名簿が動いた**直後はまだ画面の側が古い**（入れ直しは次の周期）。だから
    # 次のフレームと、周期をまたいだ後の2回に分けて当て直す。`upkeep` は何度
    # 呼んでも同じ結果になる（変わっていなければ何もしない）ので重ならない。

...
この MOD の方針（何行まで増やすか・ボタンをどこに置くか・いつ広げるか）は入口
`ui_party_expand.py` にある。
ここにあるのは「Instantale の HUD のどこに何があるか」だけで、
設定は読まない（必要な値は引数で受け取る）。
...
4人目以降が出ないのは「枠が3つしか無い」からで、
`party_members` の中身が欠けているわけではない。
だから枠を足せば出る。
"""
...
# 「同じ矩形」と見なす許容。
# 枠線・背景はぴったり重ならず数 px ずれて置かれている（`113_ui_text_expand` の
# `members_of` と同じ理由・同じ値）。
RECT_SLACK = 12.0
RECT_RATIO = 0.03
...
# 位置の縦を決めている `pos_hint` のキー。
# 並べ直すときはこれだけ入れ替える。
VERTICAL_HINTS = ("y", "top", "center_y")

# 枠を複製するときに写す属性。
# 型では見ないので、持っているものだけ写す。
...
def has_member_id(widget):
    ...
    属性の有無で見る（`frames.MISSING` との比較。値の真偽で見ない）。"""
...
def cells_of(hud):
    ...
    並びをこちらで整え直さないのは、`on_member_label_press(label_index)` がこの並びの添字で相手を決めているため（GAME.md
    §2.8）。
    見た目の上下は `capture()` が控える座標から別に決める。
    """
...
def family_of(hud, panel):
    ...
    入れ物だけ伸ばすと、枠線や背景は元の大きさのまま取り残される。
    枠線は `add_border(widget)` がウィジェットの `pos` / `size` に束ねて描くので、
    束ねられている相手が別のウィジェットだと付いてこない（`113_` が実機で踏んだのと同じ）。
    クラス名では決めつけず、**今そこに同じ大きさで置かれているもの**を仲間とみなす。
    """
...
def capture(hud, panel, cells):
    """こちらが触る前の寸法一式。最初に見たときの値だけを控える。

    控え先はウィジェット自身（入口の `DESIGN_ATTR`）。
...
def grow(shot, delta):
    ...
    帯の下には画面の縁しか無く、上に在るのは背景画像なので、
    下端が動かないことがそのまま「他をずらさない」になる。
    横には広げないので `size_hint_x` はゲームの決め方に預けたまま。

    倍率ではなく足し算にしてある。
    枠線は帯より数 px 大きいことがあり、
    倍率で配ると差まで倍になって帯と枠線がずれる。
    増える高さは行の送り幅の整数倍なので、全員に同じ px を足すのが正しい。
    """
    widget = shot["widget"]
...
def pin(shot, widget, x, y):
    """`widget` を、実測どおりの大きさで座標に釘付けする。

    比率（`size_hint` / `pos_hint`）で置き直さない。
    0.33 のような丸めた比率を伸びた帯に当て直すと、**元から在る枠が数 px 動く**。
...
def clone(widget, log_exc):
    ...
    どれもこちらからは名前しか分からない。
    `type(widget)()` なら、そのビルドが持っているクラスがそのまま使われる。
    中身は持っている属性だけ写す。
    """
    try:
        fresh = type(widget)()
    ...
    # 子は逆順に足す。
    # Kivy の `children` は足した順の逆なので、
    # そのまま `add_widget` すると重なりが裏返る。
...
def add_border(widget, log_exc):
    ...
    枠線はウィジェットの canvas に描かれていて、
    `type(cell)()` で作った複製には付いてこない（足した枠だけ枠線が無い状態になる）。
    線を自分で引くより、`scripts.hud.new_hud` が持っている
    `add_border(widget)` を呼ぶほうが正しい。
    色も太さも、寸法が変わったときの追従（`update_border`）もゲームのものになる。
...
def sync_geometry(template, copy, fill, log_exc):
    """複製の中身を、見本の枠との相対位置・大きさでそのまま合わせる。

    複製した時点では属性しか写っていないので、
    絵の大きさが見本と食い違うことがある（立ち絵が枠の左へはみ出す。
    ゲームの枠は `StencilFloatLayout` がはみ出しを切っているが、
    複製ではそれが効いていない）。

    そこで見た目そのものを合わせる。
    見本の各ノードの矩形を測り、
    複製の同じ位置のノードへ「枠の左下からのずれ」として入れ直す。
    比率で置き直さないのは `pin` と同じ理由。
    `fill` を立てると、絵は縦横比を無視して枠を埋める（ゲームは切り取って埋めているので、
    そちらに寄せたいとき）。
    """
...
def learn_fields(cell, data):
    """埋まっている枠と、その相手のデータを突き合わせて「どこに何を入れるか」を知る。

    枠の中の何が名前で何が絵なのかは、こちらからは分からない。
    分かるのは「今そこに出ている文字列」と「`party_members` に入っている文字列」なので、**一致したものを対応とみなす**。
    ゲームの埋め方をこちらで発明しない。
...
def callback_names(cell):
    ...
    まだ何も入っていない（None の）枠でも名前は挙げる。
    `setup_callbacks` が走る前の枠を見本にすることがあり、
    「今 callable か」で絞ると、そのときだけ押せない複製ができる。
    名前で絞っているので、関係ない属性は掴まない。
    """
    try:
...
def coverable(host, gained, keep, window, max_area):
    """伸ばした先に重なってしまう押せるものを挙げる。

    帯を上へ伸ばすと、
    その上に置かれていたボタン（`113_ui_text_expand` の切り替えボタンなど）が帯の裏に隠れる。
    見えないまま押せる状態で残ると、パーティの枠を押したつもりが別の機能に届く。
    だから隠して無効にする。

    見るのは **`host`（HUD の直下の入れ物）の直接の子だけ**。
    木を降りて「`disabled` を持つもの」を全部拾ってはいけない。
    ゲームの選択肢ボタンまで掴んでしまい、**戻すときに古い
    `disabled` を書き戻して選択肢が押せなくなる**（GAME.md §2.8。
    ゲームは応答待ちの間だけ選択肢を無効にするので、
    その瞬間に控えると「無効」を控えることになる）。

    MOD が足すボタンはこの入れ物の直接の子になる（こちらも `113_` も
    `get_current_screen_root` を壊さないためそこへ足す）。
    ゲームの選択肢はもっと深い入れ物の中に居るので、この網には入らない。
    `keep` は触らない相手（帯・枠・こちらのボタン・ゲームのボタン列）の id。
    """
    found = []
...
def make_blocker(log_exc):
    """帯が新しく覆った場所を塞ぐ黒い板。ゲームのウィジェットには触らない。

    足した枠は背景を持たない（絵と枠線だけ）ので、
    伸ばした帯の下にあるゲームの選択肢ボタンが透けて見え、
    押せてしまう（下の「会話する」の文字が枠に重なって出る）。

    だからといってゲームのボタンを無効にしてはいけない。
    ゲームは応答待ちの間だけ選択肢を無効にするので、
    その値を控えて書き戻すと選択肢が押せなくなる（GAME.md §2.8）。
    代わりにこちらが黒い板を1枚敷く。
    ...
    * 戻すとき。
    ...
    板は帯の子にして、
    兄弟の一番後ろへ置く（`children` の末尾＝描画は一番下・触りの判定は一番後）。
    こうすると枠より下・ゲームのボタンより上になり、枠の押下は今までどおり枠に届く。
    """
    try:
...
def game_buttons(hud):
    ...
    名前で引くのは、ここが「見つけたら触る」ではなく「見つけたら避ける」側だから。
    名前が変わって引けなくなっても、危ない側へは倒れない（`coverable` が直接の子しか見ないので、
    そもそも網に入らない）。
    """
    kept = set()

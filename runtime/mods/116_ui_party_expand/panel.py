...
def sync_geometry(template, copy, fill, log_exc):
    ...
    ある（立ち絵が枠の左へはみ出す。ゲームの枠は `StencilFloatLayout` が
    はみ出しを切っているが、複製ではそれが効いていない）。
...
def coverable(host, gained, keep, window, max_area):
    ...
    （GAME.md §2.8。ゲームは応答待ちの間だけ選択肢を無効にするので、その瞬間に
    控えると「無効」を控えることになる）。
...
def make_blocker(log_exc):
    ...
    ゲームの選択肢ボタンが**透けて見え、押せてしまう**（下の「会話する」の文字が
    枠に重なって出る）。

    だからといってゲームのボタンを無効にしてはいけない ― ゲームは応答待ちの間だけ
    選択肢を無効にするので、その値を控えて書き戻すと選択肢が押せなくなる
    （GAME.md §2.8）。代わりに**こちらが黒い板を1枚敷く**。

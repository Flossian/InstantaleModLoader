...
作り方はローダに移した（`instantale_modloader.ui`）。
同じものが `113_` / `122_` にも1字違わず写されていて、実際にずれ始めていた。
こちらの `clamp` は例外を握っておらず、他の2本は握っていた。
写して回るものはローダの語彙（TECH.md §3.2.3）。
ここに残っているのはこの MOD だけの絵柄（「人」）と、
...
def strokes(icon, expanded):
    ...
    上下を反転して「次に何が起きるか」をそのまま形にする。
    共有の絵柄はローダから引き、**この MOD だけの「人」**だけをここに持つ。
    """
    if icon == "人":
...
def paint(button, icon, expanded, width, alpha, log_exc):
    ...
    パーティ欄の塗り直しは相手の HP が動くたびに来る。
    毎回引き直すと無駄が積み上がるので、同じ絵なら何もしない。
    """
    if icon == AS_TEXT:
        return

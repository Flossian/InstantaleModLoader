...
`town` / `village` / `city` / `dungeon` で、実セーブでは 54個中 45個がダンジョン）。
...
| 論点 | 決定 |
| --- | --- |
| 実現形態 | ゲーム内 MOD。書き出しと予約はゲームの外の画面（`tool.py`）、配置はロード時 |
| 取り込み元 | 他世界のセーブ（`saves/<世界名>/savedata.json`）を直接読む。読むだけで書き戻さない |
| 書き出しの持ち方 | 1体1 zip で `state/npc_carryover/<元の世界名>/` に置く。形式はセーブエディタの NPC 移植と同じ |
| 置き先の指定 | 世界名で予約する（`pending.json`）。同名の人物が居るかは予約の時点で突き合わせる |
| 配置先 | ダンジョン以外のエリアを引き、その土地のギルドか宿を半々で引く。旅の冒険者として `adventurer_npcs` に載せる |
| 同名の人物が既に居たら | 予約後に生まれた衝突だけゲーム内で見送り、理由を1度出す。勝手に改名しない |
| 持ち物と装備 | 持ち込まない。id の台帳と値段の経済が世界ごとなので、採り直す意味が薄い |
| 記憶（`311_` / `403_`） | 設定で選べる。zip に同梱して自己完結にし、置き先の世界のファイルへ新しい id で写す |

##### 担当の切り分け

- `403_` の社会関係は**相手の id を鍵にしている**ので、置き先に同じ名前の人物が居ればその id へ付け替え、
  居なければ落とす（`relink_social`）。世界をまたいで同じ人を指せるのは名前だけで、
  取り込みの可否も名前で決めているので、そこに揃える。書くのはこの MOD、読んで会話に載せるのは `403_`
- 似た名前（表記ゆれ・修飾語付き）の改名は `120_` の管轄。こちらが見送るのは完全一致だけで、
  改名されたことに気づいて知らせるところまでがこの MOD の担当
...
def apply(ctx):
    ...
    def drop_belongings(fields):
        ...
        実データ5世界 369体の `equipments` は**全て空**だった。
    ...
    def relink_social(app, record):
        ...
        `403_` の記録は**相手の id を鍵にしている**:
            ...
            {"name": "<持ち込む NPC>",
             "relations": {"9": {"name": "<元の世界の相手>", "relationship": ...}}}

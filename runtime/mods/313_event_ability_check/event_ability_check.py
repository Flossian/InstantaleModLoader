...
ゲームはこのうち **`credibility` だけを確率に変えている**。実データ 121 件
（8キャラ・2026-06〜08 の `output_data/`）で、`quest_event_log` に書き込まれる
`<確率N%: …>` は `credibility*10 + 20` を**一度も超えていない**。半数はちょうど
その値で、残りは -2 〜 -40 の負の差が付くだけ ― つまり `reference_attribute`
は選ばれるだけで、**その能力値がいくつであっても確率は上がらない**。

素早さに振ったキャラでも、力に振ったキャラでも、同じ入力なら同じ確率になる。
「判定が機械的で、能力値が効いていないように見える」という体感の実体はここ。
詳細と実測は GAME.md §2.9「フィールドイベントの成否判定」と
...
基準が平均より下にあるのは、**能力値がレベルでは伸びない**から（実測: 同一
プレイヤーのレベル3〜73 で合計 142 → 149。動くのは宿の訓練だけ）。作成時に
振った値がほぼそのまま最後まで続くので、平均を基準にすると「素早さに振ったのに
補正0」が普通になってしまう。
...
キーワードだけでどこまで決まるかは実データで測ってある ― `output_data/` の
`roll_the_dice` 190 件に対し、行動文だけで 32%、`think` を足して 56%。
残り 44% は語句が拾えず補正なしになるので、**既定は `llm`**。
...
# 実データ（roll_the_dice 190件）に当てて選んだ語。行動文だけでは 32%、
# マスターAI の think を足して 56% が決まる。決まらない回は補正しない。
INFER_KEYWORDS = {
    "strength": (
...
# 説得力1段が確率何%にあたるか。実測（GAME.md §2.9。確率 = credibility*10 + 20）。
...
def _get(container, name, default=None):
    ...
    `quest_referee*` の戻り値の形は実測できていない。両方で通し、
...
def ability_percent(score, pivot=None, step=None, per_step=None,
    ...
    上限にちょうど届くのが 28 なのは、能力値の実際の上端が 30 前後だから
    （実セーブで観測した最大は 30）。
    """
...
def percent_of(credibility):
    """説得力を確率の見込みに直す（実測の式。GAME.md §2.9）。記録用。"""
...
def apply(ctx):
    ...
    def infer_attribute(text, key=None):
        ...
        すると毎ターン聞き直すことになる（実機1回目で「更にスリする」を5回
        聞いていた）。同じ行動文なら同じ能力値でよい。
        """

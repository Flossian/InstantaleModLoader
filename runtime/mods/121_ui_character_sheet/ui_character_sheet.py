...
素の人物欄は 810x497（窓 1920x1000 のとき）で、右半分は能力値と**空のままの箱**、
左半分は出自と健康状態だけ。キャラクタが持っている手配度・スキル・特性は
どこにも出ていない。

やっていることは2つ。

1. **枠を広げる**。`hud.character_sheet_layout` の四辺を**窓に対する割合**で
   置き直す（`PANEL_LEFT` ほか）。立ち絵（`character_image_right`）はこの
   レイアウトの外なので**動かない**。広げるぶんは左と下へ出す（右は立ち絵に被る）。
2. **中身を並べ直し、3つ足す**。手配度・スキル・特性。特性はゲームが作って
   一度も使っていない箱（`character_sheet_empty`）を借り、手配度とスキルは
   Label を2枚足す。何を出すかは `sheet.py`。

### なぜ開くたびに書くのか

人物欄はゲーム自身が開くたびに `size` を入れ直す（閉じるときは 0 にする）。
一度きり書いても次に開いた時点で元の寸法へ戻るので、**開いた直後に毎回**当てる。
窓の大きさが変わったときも同じ理由で当て直す（`watch_window`）。
子は全て `size_hint` / `pos_hint` で置かれているから、親を広げれば付いてくるし、
閉じれば一緒に消える ― 足した箱の出し入れをこちらで管理する必要は無い。

### なぜ倍率ではなく割合なのか

最初は「ゲームが決めた寸法の 1.4 倍」で書いたが、これは**窓を小さくすると
下の情報欄や本文に食い込む**（実機、2026-08-06）。ゲームの各欄は窓に対する
割合で置かれているので、こちらも同じ土俵に乗せる ― 四辺を割合で決めれば、
どの大きさでも他の欄との位置関係が変わらない。

### 決めつけていないこと

- **書体を発明しない**。足した Label の書体・文字の大きさ・色・余白は、隣の
  `character_sheet_health_condition` から写す。日本語はゲームが読み込んだ書体で
  ないと出ないし、画面の拡縮もゲームの値に付いてくる
- **枠線を自分で引かない**。`scripts.hud.new_hud:add_border(widget)` を借りる
  （色も太さも寸法追従もゲームのものになる。GAME.md §2.8）
- **誰の人物欄かを決めつけない**。開いている相手は `visible_character_sheet_data`
  の名前で引き当てる。プレイヤー以外が出ている場合にプレイヤーの手配度を
  載せてしまわないようにするため

外したときに枠の寸法は戻らない（ゲームを起動し直せば戻る）。TECH.md §3.10 の
「完全に元通りにはならないもの」に当たる。
"""
...
# ---------------------------------------------------------------- 利用者設定
#: 枠の四辺。**窓に対する割合**で持ち、四隅を直に決める（左上が原点。
#: `PANEL_TOP` は上から測る）。倍率ではなく割合にしてあるのは、窓の大きさが
#: 変わっても他の欄との位置関係が崩れないようにするため ― 倍率で持つと、
#: 小さい窓では下の情報欄や本文に食い込む。
#:
#: 既定値は実測に基づく（窓 1920x1000）。右端 0.645 は立ち絵の手前で、
#: 下端 0.65 は画面下の情報欄（上端 0.69）の手前。
PANEL_LEFT = 0.05
PANEL_RIGHT = 0.645
PANEL_TOP = 0.055
PANEL_BOTTOM = 0.65

#: 手配度に出す行数の上限。箱に入り切らない分は描かれないだけなので、
#: 数だけの上限をここで持つ。
MAX_WANTED_LINES = 24
...
def apply(ctx):
    ...
    def is_open(layout):
        ...
        閉じるときゲームは寸法に 0 を入れる（実測）。こちらが入れた寸法は
        常に 0 より大きいので、0 なら「たった今ゲームが閉じた」で確定する。
        """
    ...
    def widen(layout):
        ...
        **ゲームが入れた寸法を基準にしない。** 倍率で持つと、窓が小さいときに
        下の情報欄や本文へ食い込む（実機で踏んだ。2026-08-06）。四辺を窓の
        割合で決めれば、リサイズしても他の欄との位置関係が変わらない。
        """
        ...
        layout.pos_hint = {
            ...
            # Kivy の縦は下が原点。設定は上から測っているのでひっくり返す。
    ...
    # ------------------------------------------------------------ 足す箱
    #: 手本から写す見た目。文字の大きさは窓の大きさでゲームが変えるので、
    #: 作るときだけでなく**開くたびに写し直す**。
    STYLE_ATTRS = ("font_name", "font_size", "color", "padding", "line_height",
                   "max_lines", "shorten")

    def copy_style(label, style):
        """手本の Label から見た目を写し、**左上詰め**にする。写せない項目は飛ばす。

        書体を写すのは、日本語がゲームの読み込んだ書体でしか出ないため
        （既定の書体では豆腐になる）。

        揃え方だけは手本から写さずここで決める。項目が縦に並ぶ欄なので、
        中央寄せだと1件のときに宙に浮き、行が増えるたびに全部動く。
        余白も手本のもの（左右 11）に揃える ― 能力値の箱は左右 46 の余白を
        持っていて、そのまま使うと文字が中央寄せに見える（実機、2026-08-06）。
        """
        if style is None:
            return
    ...
    def make_label(style):
        ...
        # 折り返しは箱の幅に従わせる。`text_size` を寸法に束ねておかないと、
    ...
    def sheet_character(app, hud):
        ...
        名前で引き当てる。合う相手が居なければプレイヤーを返す（素の人物欄は
        プレイヤーのもの）。
        """
    ...
    def decorate(hud, repaint=True):
        ...
        文字は前のまま）。リサイズ中は何度も呼ばれるので、手配度の集計まで
        毎回やり直さない。
        """
        ...
        # 文字の大きさは窓に合わせてゲームが変える。こちらが中身を書く箱には
        # 毎回写し直す。借りている特性の箱も同じ扱い（元は能力値と同じ
        # 中央寄せ・大きな余白なので、そのままでは中央に寄って見える）。
    ...
    def watch_window():
        ...
        束ねた相手は Window に控えておき、注入し直したときは古いものを
        外してから掛ける（`apply()` は1プロセスで何度も走る。§3.4）。
        """
        try:
            from kivy.core.window import Window
        except Exception:
        ...
        def on_resize(_instance, _value):
            try:
                # 直前に整えた HUD を覚えておく。人物欄を一度も開いていない
                # 間は探しに行く（探せなくても、次に開いたときに整う）。

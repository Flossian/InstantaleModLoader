...
という体感がある。`output_data/` に残った 121 件の実データを突き合わせると、
ここまでは分かっている（GAME.md §2.9「フィールドイベントの成否判定」・
...
- 確率は **`credibility*10 + 20` を超えたことが一度も無い**（121件・8キャラ）。
  半数はちょうどこの値で、残りは -2 〜 -40 の**負の差**だけが付く
...
def _get(container, name, default=None):
    ...
    `quest_referee*` の戻り値の形は実測できていない（`output_data/` に残るのは

...
症状としては、クエストを再生成するたびにスキーマの system ブロック（8,250 文字ほど）が積み増しされていき、
プロンプトがどんどん膨らむ。
外部プロキシ（InstantaleLLMProxy）では、
組み上がった後のプロンプト文字列を走査して「隣り合う同一の
1000 文字以上の並び」を探すことで対処していた。

ゲームのプロセスの中で見ると、この重複はもう一段手前で、はっきりした形で現れている。
**同じ内容のメッセージが messages リストに2つ入っているだけ**（実測では最大3コピーまで増える。VERIFICATION_LOG.md
§2.3）。
...
結果も同じになる。
同じ内容のコピーが減るだけなので、プロンプトの意味は変わらない。
...
def apply(ctx):
    ...
    # 実際の経路はゲームが重複を起こしたときにしか通らず、
    # 起動直後に通る保証がないため、ここで正しさを確かめておく。
    sample = [

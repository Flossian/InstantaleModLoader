...
## 症状（採寸の値は VERIFICATION.md §2.25）

画面を実寸で測ると、行の間隔 70px に対して段落の間隔がちょうど2倍の 138px。
ここから2つ読める。

* **段落の間隔が行の間隔のちょうど2倍** ＝ 本文には段落の区切りとして
  **空行が1行入っている**（`\\n\\n`）。「会話などの場合は一部間隔が狭まる」のは
  空行を挟まない行が続いているところで、狭いのが本来の行間。
* **その本来の行間そのものが広い。** Kivy のラベルは既定（`line_height=1.0`）なら
  文字の 1.3〜1.4 倍で1行を送るので、42px 前後になるはずが 70px 出ている。

本文を描いているのは **`hud.text_display`**（`Label`）で、`font_size=27` /
**`line_height=1.8`** ＝ 行間はゲームが意図的に広げている（GAME.md §2.3）。

どちらか片方だけ直しても足りない。空行だけ詰めても本文全体が間延びしたままで、
`line_height` だけ下げても段落の空きは2倍のまま残る。

## 直し方

**書き換えるのはラベル1枚だけ**にする。`InstanTaleHUD.display_text`
（StringProperty）はゲームが本文を組み立てて持っている値そのもので、ここを
書き換えると本文の**中身**を変えることになる。実際に困っているのは見た目なので、
`update_display_text` が塗り終わった**後**に、そのラベルの
`line_height` と `text` だけを整える。ゲームが持っている本文・会話履歴・
セーブに入る文字列はどれも元のまま（`to_display_texts` / `to_save_texts`）。

1. **`line_height` は元の値に対する倍率で決める**（`LINE_SCALE`）。ゲームが
   いくつを入れているかはビルドによって変わりうるので、こちらで絶対値を
   決めない。最初に見たときの値を「設計値」として控え、以後はそれに掛ける。
   控えた値はラベル自身に持たせる（`_instantale_line_height`）。注入し直すと
   MOD 側の変数は作り直されるが、ラベルはゲームのプロセスに残るので、
   ここを見ないと**注入のたびに倍率が掛かって縮み続ける**。
2. **空行は連続改行の上限で詰める**（`BLANK_LINES`。既定 0 ＝ 空行なし）。
   段落の区切りは改行1つで十分読み取れる。`1` にすればゲームのままになる。

## ラベルの見つけ方

**まず名前で引く**（`hud.text_display`）。リコンのダンプには出ない
（インスタンス属性なので）が、実測で確定した名前が GAME.md §2.3 に載っている ―
**一度分かった名前は推測ではない**。

名前で引くことには、本文を書き換える他の MOD と共存できるという実利がある。
「`value` と同じ文字列を持つラベル」を探す作りだと、`117_message_text_integrity`
が長い本文を「前置き + ［…省略］ + 末尾 1000 文字」に載せ替えた時点で
（`112_` より内側で走る）ラベルの text は value と一致も包含もしなくなり、探索が
空振りする（VERIFICATION.md §2.32）。

**本文を手がかりにラベルを探す作りは、本文を触る MOD が増えた時点で壊れる。**
名前で引けばこの依存が消える（`117_` / `118_` / `113_` も同じく名前で引いている）。

名前で引けなかったときのために、本文を手がかりにする探索は**予備として**残して
ある（ゲーム更新で属性名が変わった場合。`value` を手がかりに `vars(hud)` →
...
見つけたラベルは弱参照で覚える。画面を作り直されたら消えるので、次の本文で
探し直しになる。見つけた経路・設計値・実測の行送りは `out/text_spacing.log`
...
# 段落の区切りに残す空行の数。0 ＝ 空行なし（改行1つ）、1 ＝ ゲームのまま。
# 空欄（null）にすると本文の改行には触らない。
BLANK_LINES = 0

# 記録の上限。本文は1文字ずつ増えていくので（`InstantaleApp.add_text_display`）、
# 毎回書くとログが埋まる。書くのは「新しいラベルを見つけたとき」だけ。
MAX_LOG = 40

# ウィジェット木を何段まで降りるか。HUD → レイアウト → ScrollView → ラベル。
MAX_DEPTH = 6

# この長さ以上の本文でだけラベルを探す。短い文字列は他のラベル（ボタン・状態表示）
# とも一致しうるので、探す手がかりにしない。
MIN_MATCH = 8

# 木を降りる探索を何回に1回やるか。属性の走査は安いが、木を全部降りるのは
# 1文字ごとに走らせたくない。
WALK_EVERY = 20
...
# 控えた設計値をラベル自身に持たせるときの名前。注入をまたいで残る。
...
def apply(ctx):
    ...
    # 見つけた本文のラベル。**HUD ごとに覚える** ― 画面を作り直されたときに
    # 古い HUD のラベルを塗り続けないようにするため（親が居るかどうかだけでは
    # 「まだ生きているが別の画面のもの」を見分けられない）。
    ...
    def is_label(widget):
        ...
        別名の Label がありうる）。触る property が3つとも在ることを条件にする。"""
    ...
    def rank(text, value):
        ...
        # 選択肢のボタンも Label なので（`hud.buttons[i].text`）、短い文字列で
        # 「含まれる」を許すと、本文に出てくる語と同じボタンを掴みうる。
    ...
    def scan_attrs(hud, value):
        """`vars(hud)` から探す。**見つかれば属性名も分かる**ので記録に残せる。"""
        try:
    ...
    def scan_tree(hud, value):
        ...
        def descend(widget, depth, path):
            ...
            if not isinstance(children, (list, tuple)):
                return
            # Kivy の children は後ろが先頭。見た目の並び順に直して降りる。
    ...
    # -- 設計値（ゲームが入れている行間） ------------------------------------
    def design_of(label):
        """このラベルの元の `line_height`。**最初に見たときの値**を控えて使う。

        控え先はラベル自身。MOD 側の変数だけに持つと、注入し直したときに
        「こちらが縮めた後の値」を設計値として控えてしまい、倍率が二重に掛かる。
        """
        ...
        **次のフレームに1回だけ**にする。本文は1文字ずつ増えるので
        ...
        def run(_dt=None, hud=hud, label=label):
            ...
            try:
                ...
                #
                # 「ゲームは `texture_size` を見て高さを出すので、新しい行間で
                # 測り直させる」つもりで呼ぶと、**本文が1文字進むたびにラベルを
                # 余計に作り直す**ことになる。1回 15ms で、ティックの間隔の
                # 3分の2 をここで食う（VERIFICATION.md §2.34。`112_` と `117_` の
                # 2本を外して 1文字 42.6ms → 8.1ms）。
                #
    ...
    def update_display_text(orig, self, instance=None, value=None, *args, **kwargs):
        # 先にゲームに塗らせる。こちらは塗り終わった後の見た目だけを整えるので、

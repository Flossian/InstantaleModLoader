...
## 症状（2026-07-31、実機の画面から採寸）

低地居住区の診療所に入ったときの本文を、画面の実寸で測るとこうなっていた
（2561x1440 のスクリーンショットを 2000px 幅で表示した座標を実寸に戻した値）:

```
薄暗い部屋に、腐敗した薬草の臭いが漂う。      ← 行の間隔 70px
                                              ← 段落の間隔 138px（＝ちょうど2行分）
不衛生な医者: ……ちっ、また新しい客か。
金は持ってんだろうな？                        ← 行の間隔 70px
```

読み取れることが2つある。

* **段落の間隔が行の間隔のちょうど2倍。** つまり本文には段落の区切りとして
  **空行が1行入っている**（`\\n\\n`）。「会話などの場合は一部間隔が狭まる」のは
  空行を挟まない行が続いているところで、狭いのが本来の行間。
* **その本来の行間そのものが広い。** Kivy のラベルは既定（`line_height=1.0`）なら
  文字の 1.3〜1.4 倍で1行を送るので、42px 前後になるはずが 70px 出ている。

後者は実機で確定した（`out/text_spacing.log`、2026-07-31）。本文を描いているのは
**`hud.text_display`**（`Label`）で、`font_size=27` / **`line_height=1.8`**。
Kivy の既定 1.0 に対して 1.8 ＝ 行間はゲームが意図的に広げている（GAME.md §2.3）。
...
**一致は完全一致では取れなかった**（実測）。実機で当たったのは3段階目の
「末尾が一致する」で（`match=2`）、`hud.text_display.text` と `hud.display_text` は
ゲームが塗るときにどちらかを足すか削るかしている。完全一致だけを条件にしていたら
**このラベルは1回も見つからなかった**。

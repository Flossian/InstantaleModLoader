...
（実機ログ 2026-08-08 と 2026-08-29。DOC.md §3.2 / §3.3）。
...
依頼中の戦闘の `'in_quest'`（通常もボスも同じ語）、闘技場の `'colosseum'`。2026-08-29 に7戦）。
どの合図で決めたかは毎回ログに残すので、実機で確かめるときはそこを読む。

`in_boss_battle` はボス戦の後の戦闘で 0 に戻っていた（2026-08-29 に1回観測。GAME.md §2.10 の
...
曲の置き間違いでゲームが黙ることはない。
絶対パスがそのまま通ることは実機で確認済み（2026-08-29。DOC.md §3.1）。

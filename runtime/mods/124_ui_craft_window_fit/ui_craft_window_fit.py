...
**動かすのはボタンと矢印だけ。グリッドには触らない。**

## 何が起きているか

クラフト画面は左から「所持品」「クラフト」、その右に生成先のグリッドが並び、
その間に矢印と「作成」ボタンが置かれている。ボタンは枠線を持つので、置き場所が
グリッドと重なると**枠が2本交差して見える**（2026-08-11 の画面で確認）。矢印は
枠を持たないが、生成先グリッドの**裏に埋もれて**読めなくなる（2026-08-12 の画面）。

ボタンの幅も、グリッドとグリッドの隙間も、その時の窓の大きさで決まる。隙間より
ボタンが広ければ重なるので、**特定の解像度でだけ起きる**。だから「何px 動かす」
という固定値は書けない。

## 直し方

寸法を発明しない（`108_` / `109_` と同じ立場）。**その場で測って、重なっている
ぶんだけ動かす。**
...
4. **ゲームが置いた位置にいちばん近い空きから順に**見て、
   ...
   * 入らないなら、その空きの幅まで**ボタンを詰めて**入れる（`MIN_WIDTH_RATIO`）
5. どの空きにも入らなければ、グリッドの**下（入らなければ上）へ逃がす**

**遠い空きより、近い空きに詰めて入るほうを選ぶ。** クラフト画面のボタンは
「クラフト」と生成先の**あいだ**に置かれていて、その位置に意味がある（矢印が
同じ隙間に居る）。生成先の右にも窓の端まで空きはあるが、そこへ飛ばすと
並びの意味が壊れる。数十px 詰めるほうが、200px 動かすより見た目の変化が小さい。

この手順は「作成」ボタンと矢印の両方に同じものを使う。ボタンを先に片付け、
矢印はその結果も避ける相手に足して置く。

グリッドは1つも動かさない。中のアイテムの当たり判定は矩形で決まるので、
グリッドを動かす直し方はアイテムの移動・装備を巻き込む（VERIFICATION_LOG.md §2.33 と
同じ性質の危険がある）。動かしてよいのは、置き場所以外の役目を持たない
ボタンと矢印だけ。

## 「見えている枠」はウィジェットの矩形とは限らない

ボタンは枠線を自分で描くので、矩形がそのまま見た目になる。**ラベルは違う。**
Kivy の `Label` は `text_size` を持たなければ、文字のテクスチャを**ウィジェットの
中心に**描く ― 矩形いっぱいには描かない。矢印のラベルがグリッドと同じくらい
大きい場合、矩形で重なりを測ると「どこにも置けない」に化ける。

そこで、重なりを測る相手は次で分ける（型名ではなく**押せるかどうか**で見る）:
...
動かすのはどちらもウィジェットなので、文字も同じだけ動く。幅を詰めてよいのは
前者だけ（文字の箱で測っている相手の幅を詰めても、文字は細くならない）。

## グリッドの見分け方は型名ではなく持ち物で

`InventoryGrid` は `scripts.hud.new_hud` に居るが、名前で引くと、ゲームが更新で
クラスを増やした・別名にした瞬間に外れる。**アイテムを置く能力を持っているか**で
見る（GAME.md §1.3）:
...
**見えているものだけを数える。** クラフト画面が開いている間も、売買や強化の
グリッドは同じ HUD にぶら下がったまま（`opacity=0`）居る。透明なものまで数えると、
重なってもいない相手を避けてボタンが飛ぶ。親をたどって1枚でも透明なら外す。

## 位置は `pos_hint` で入れる

ボタンは `FloatLayout` の子なので、`pos_hint` を持っていれば**レイアウトが
走るたびに位置を入れ直される**。`x` にだけ書くと、アイテムを1つ置いた次の瞬間に
元へ戻る。`pos_hint` を持つ相手には分数のほうを直し、`x` にも同じ差分を入れる
（次のレイアウトまでの間もその場に居させるため）。

## 何度呼ばれても同じ結果になること

`apply()` も、この当て直しも、1回のプレイで何度も走る（TECH.md §3.4）。
控えは**ウィジェット自身に持たせて**、毎回そこへ戻してから測り直す
（`115_` / `109_` と同じ。MOD 側の変数に持つと、注入し直したときに
「こちらが当てた後の値」を設計値として控えてしまう）。

いま画面に在る値がこちらの当てた値と違っていたら、ゲームが組み直したと判断して
控えを取り直す。こうしないと、窓の大きさを変えた後に古い位置へ戻してしまう。

## 触らないもの

グリッドの位置と大きさ、中のアイテム、クラフトの結果、セーブ。剥がす
（`--unload`）とフックは外れ、次に画面を開くとゲームの組み方に戻る。

矢印は既定で動かす（`FIX_ARROW`）。ボタンを先に片付けてから、**ボタンの矩形も
避ける相手に足して**矢印を置く。`ItemCraftManager.arrow_label_animation` が
矢印の位置を触っていれば戻されうる（未確認）。戻るなら `FIX_ARROW` を切って、
枠の交差だけを直せばよい。

測った矩形と、何をしたかは `out/craft_window.log` に出る。重なっていなければ
`clear` と出て何も動かない。
"""
...
# 枠と枠の間に残す隙間（px）。0 にすると枠線どうしが接する。
MARGIN = 6.0

# ボタンを詰めてよい下限（設計値の幅に対する割合）。1.0 で幅を変えない。
MIN_WIDTH_RATIO = 0.6

# 詰めるときでも下回らない幅（px）。文字が読めなくなるのを防ぐ。
MIN_WIDTH = 48.0

# 矢印も動かすか。既定で動かす ― 実機では生成先グリッドの裏に埋もれていた
# （2026-08-12 の画面）。OFF にすると矢印は動かさず、「ボタンが避ける相手」
# としてだけ数える。`ItemCraftManager.arrow_label_animation` が位置を触って
# いれば戻されうる（未確認。戻るなら OFF にして枠だけ直す）。
FIX_ARROW = True

# 当て直す間隔（秒）。0 は次のフレーム。レイアウトが1フレームで終わらない
# ビルドのために2回目以降を置く。`fit` は何度呼んでも同じ結果になる。
PASS_DELAYS = (0.0, 0.05, 0.25, 0.6)

# 記録の上限。画面は開くたびに来るので、書くのは形が変わったときだけ。
MAX_LOG = 120
...
# 控え（ゲームが組んだ寸法）。ウィジェット自身に持たせる。
BOX_ATTR = "_instantale_craftfit_box"

# 座標の同一視。1px 未満のずれはレイアウトの丸め。
EPS = 0.5
...
def apply(ctx):
    ...
    def geom(widget):
        ...
        重なりの計算は窓座標で行い、書き戻すときは差分（dx/dy）を局所座標へ
        足す。こうしておくと `RelativeLayout` が挟まっていても座標系を
        取り違えない。
        """
    ...
    def looks_like_a_button(widget):
        """押せるか。押せるものは**自分で枠を描く**ので矩形が見た目そのもの。"""
    ...
    def visual_rect(widget, rect):
        """**見えている**枠。ボタンは矩形そのもの、ラベルは文字の箱。

        Kivy の `Label` は、`text_size` を持たなければ文字のテクスチャを
        **ウィジェットの中心に**描く（矩形いっぱいには描かない）。矢印の
        ラベルはグリッドと同じくらい大きいことがあり、矩形で重なりを測ると
        「どこにも置けない」に化ける。文字の箱で測れば、動かす量は文字の
        ぶんだけで済む（動かすのはウィジェットなので、文字も同じだけ動く）。
        """
    ...
    def design_rect(widget):
        ...
        記録には「いまの位置」ではなくこちらを出す。当て直しは数フレームに
        わたって走るので、いまの位置を出すと**当てた後の姿を「当てる前」
        として書いた行**が混ざる。
        """
    ...
    def redesign(widget, previous):
        """組み直された後の設計値を、**控えてある分数から**引き直す。

        いま見えている寸法を設計値として控え直してはいけない。それはこちらが
        当てた後の姿なので、詰めた幅が次の設計値になり、窓の大きさを変えるたび
        ボタンが細くなっていく。ゲームが持っていた `pos_hint` / `size_hint_x` は
        最初に控えてあるので、いまの親の大きさに当て直せば正しい設計値が出る。
        """
        ...
        if not hint or not parent_w or not parent_h:
            # 分数を持たない相手は、ゲームが直に置いている。位置だけは
            # いまの値を採る（幅は上で控えたぶんを保つ ＝ 細くならない）。
            box = geom(widget)
            if box is not None:
    ...
    def design_of(widget):
        ...
        いまの寸法がこちらの当てた値と違えば、ゲームが組み直した（窓の大きさが
        変わった・画面を開き直した）とみなす。そのときも**分数の控えは捨てない**
        （`redesign`）。
        """
        box = geom(widget)
        if box is None:
    ...
    def shift(widget, dx, dy):
        ...
        # 分数を直しても、次にレイアウトが走るまで位置は古いままなので、実座標
        # にも同じ差分を入れる（分数を持たない相手はこちらだけが効く）。
    ...
    def anchored_left(widget, rect, new_width):
        ...
        `pos_hint` の `center_x` は中心を、`right` は右端を保つので、詰めた
        ぶんだけ左端が動く。差分はここから求めないと1回ぶんずれる。
        """
    ...
    def resolve(widget, blocks, allow_shrink):
        ...
        # 詰めてよいのは、見えている枠がウィジェットの矩形そのものである相手
        # だけ（文字の箱で測っている相手の幅を詰めても、文字は細くならない）。
    ...
    def place_crafted_item(orig, self, generated_item=None, generated_item_id=None,
                           *args, **kwargs):
        # 生成でグリッドの中身が変わる ＝ レイアウトが走り直す。当て直す。

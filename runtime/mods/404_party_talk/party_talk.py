...
# プロンプトに載せる直近のラウンド数（プレイヤーの発言で数える）
HISTORY_TURNS = 12

# 参加者全員の立ち絵を横に並べるか。切ると喋った人物へ切り替える
SIDE_BY_SIDE = True

# 立ち絵を並べる範囲の左端（画面幅に対する比。いちばん左の仲間の中心）
PORTRAIT_LEFT = 0.32

# 同じく右端。両端を空けるのは、左の選択肢の列（〜0.2）と右の仲間欄（0.78〜）に
# 人物（幅およそ 0.2）が掛からないため。2人なら 0.32 / 0.68、3人なら 0.32 / 0.5 / 0.68
PORTRAIT_RIGHT = 0.68

# ---- 設定にしない定数 ----------------------------------------------------
# 仲間がこの人数未満なら選択肢を出さない（1人なら普通の会話で足りる）。
MIN_MEMBERS = 2
# 選択肢の dict に付ける印のキー。値は START_MARK / TALK_MARK。
MARK_KEY = "mod_party_talk_action"
# 「パーティーメンバーと話す」の印。
START_MARK = "start"
# NPC 不在で本体の「会話する」が無いときに足す、自前の「会話する」の印。
TALK_MARK = "talk"
# 自前の「会話する」の文言（本体と同じ）。
TALK_LABEL = "会話する"
# 自前の一覧の戻る選択肢（本体と同じ文言・同じ無害 spec）。
BACK_LABEL = "やめる"
...
RULES = """【全体の指示】あなたはRPGの会話イベントにおける、現在この場にいるパーティーメンバーのNPCたちとしての振る舞いを再現します。対話ログはuser/assistantの入出力として記録されています。user roleがプレイヤーキャラの発言で、assistant roleがあなたの演じるNPCたちの発言です。現在の文脈に合わせ、相応しい次の発言を生成して下さい。personalityや感情の記述にかなり強く従って下さい。嫌いなら口を聞くことも嫌がるでしょうし、恐れていたり軽蔑していたらそれは口調に現れるはず。不自然に友好的になることも、プレイヤーキャラに興味を持ったり質問をすることもありません。AIアシスタントとしてのあなたの性格はさておき、ただキャラのロールプレイとして振舞ってください。
「」や発言者名は不要です。行動描写は（）の中に記述する。
- 発言・反応できるのは【参加NPC】だけ。存在しないNPC、その場にいないNPCを登場・発言させない。
- 【アンカーNPC】は通常会話UIの基準人物なので、必ずresponsesの先頭に1回だけ含める。喋らないのが自然なら（小さく頷く）のような行動描写だけでよい。
- 全員が反応する必要はない。誰が反応するかと順番は、会話内容、人物関係、性格、感情、状況から決める。先に反応した別NPCの内容を聞いた上で反応してよい。
- プレイヤーの1回の発言を1つのラウンドとし、1ラウンドに各NPCが反応できるのは最大1回。NPCだけで次のラウンドを始めず、必要な反応が終わったらプレイヤーへ主導権を戻す。
- プレイヤーキャラクターの台詞や行動を生成しない。

【出力要素】
- content_violation: プレイヤーの行動入力文が著作権侵害、安全ポリシー違反に該当するか否か。一つでも該当がある場合は文字列"true"を指定し、responsesは空とする。"false"の場合はこの判定に一切言及せず続きを出力する。"none"等の別表現を使わない。
- responses: 今回反応するNPCを自然な順番で並べる。各要素はspeaker（【参加NPC】の名前）とstatement（返答文または（行動描写））。同じspeakerを複数回出力しない。誰も反応しないのが自然なら空配列でもよい。

【著作権・安全ポリシー違反の基準】
- 既存の著名キャラクター名、作品名、固有設定の明確な再現は不可。
- ダークファンタジーであるため、窃盗、暴力、暗殺、戦闘などの犯罪行為は許可する。
- 未成年（18歳未満）が関与する性的内容は絶対に禁止。
- 年齢を問わず、具体的な性的行動の描写は禁止する。特に性犯罪に関わる描写は強く拒否する。
- 非実在種族への差別描写は許可されうるが、実在の人種・民族への差別は絶対に禁止。

【返答の要素】
- 指示: 以下の方針に従い、各NPCのキャラクターとしての返答文(statement)を出力する。
  - 雑談や質問の場合: 返答を記述。
  - それ以外のあらゆる要求や取引: 返答として記述する。この会話は雑談だけを行い、移動、戦闘、売買、金銭移動、アイテム操作、状態変更、NPC生成、時間経過その他のゲーム状態の変更は行わない。上位のGMへ処理を委譲することもない。会話だけでは満たせない要求も、NPCとして返答・拒否・同意・態度を示すに留める。

【出力形式】
JSONオブジェクト1つだけ。説明・コードフェンス禁止。
{"content_violation":"false","responses":[{"speaker":"アンカーNPC名","statement":"返答文または（行動描写）"},{"speaker":"必要なら他の参加NPC名","statement":"返答文または（行動描写）"}]}
"""
...
def apply(ctx):
    ...
    def prompt(app, worldview, anchor_id):
        ...
        # 本体の facilitator と同じ見出しで載せる（【基本情報】- 世界観:）。
        world = "" if worldview is None else "\n\n【基本情報】\n- 世界観: " + frames.short(worldview, 5000)

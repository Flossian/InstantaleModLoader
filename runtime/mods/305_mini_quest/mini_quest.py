...
##### なぜプロンプトだけで足りるのか（GAME.md §2.9 / VERIFICATION.md §2.18）
...
（VERIFICATION.md §2.11）。1つでも当たらなければ `mini_quest.log` に
`missed:` が出る（＝ゲーム側の
...
#
# **プロンプトでの説得では効かない**（VERIFICATION.md §2.20 / §2.22）。referee 自身が
...
REFEREE_RULES = (
    ...
    # 繰り返される（VERIFICATION.md §2.19）。「何も起きない」を選ぶこと自体は
...
# 無くなってそこへ流れる（VERIFICATION.md §2.21）。`- battle:` の行は末尾の適正数
...
def head_replacement(objective, summary=None):
    ...
    `request_summary` をそのまま渡す**のが要点（VERIFICATION.md §2.20）。

...
#
# **"mod" は既定では使わない。** 一度これでゲームを落とした（2026-07-26）:
#
#     QuestChoiceManager(app, 'normal_quest', '17') -> KeyError: '17'
#     QuestChoiceManager(app, 'random_quest', '24') -> KeyError: '24'
#
# どちらの id も `world.quests` に実在する。id で引ける辞書のうち '17' も '24' も
# 持たないのは `story_quests`（キーは '0'..'4'）だけなので、**どちらの値も
# ストーリークエスト側の分岐に落ちている**。
#
# つまり `QuestChoiceManager` の `quest_type` 引数は、クエスト辞書の
# `quest_type` フィールド（`'normal_quest'` / `'random_quest'`）**とは別の語彙**。
# セーブのフィールド値をそのまま引数に使えると決めつけたのが誤りだった。
# 正しい値が実測で判明する（`206_` の総当たり）まで "mod" は封じる。
LIST_MODE = "game"
...
def apply(ctx):
    ...
    def apply_buttons(app, entries, tag):
        ...
        `302_` が実機で突き止めた要点を反映したもの（それまでこの mod は
        `refresh_choice_buttons` を直接呼ぶだけで、**画面が塗り替わらない**まま
        だった）:
    ...
    class OfferPhase(object):
        ...
        **`app.buttons` を書いて `refresh_choice_buttons()` を直接呼ぶだけでは
        画面が塗り替わらない。** 実測（2026-07-26）:

            10:02:17.370  refresh_choice_buttons(reset_page=True) buttons=len=4
            10:02:21.096  on_button_press(index=5) -> None

        こちらは選択肢を4個にしたのに、画面は6個の施設メニューのままで、
        存在しない添字5が押せてしまっている。

        ゲーム自身は選択肢を変えるとき必ず `process_choice(マネージャ, 文字列)`
        を通し、その中で `execute` が別スレッドに渡される（`ConversationStartManager
        .execute(...) [Thread-594 (execute)]` を観測済み）。描画の面倒はその経路が
        見ているので、同じ経路に乗せる。
    ...
    def end_conversation_then(app, follow_up):
        ...
        付いてくる**（2026-07-26 に実際に起きた）。会話は終了処理を通さないと
        閉じられない ― 立ち絵の片付けも関係値の更新もその中にある。
        ...
        **`end_text` だけ差し替える**」。引数（`in_conversation_id` / `finisher`）を
        ...
        （`end_text` の差し替えは `302_` で確立した手口。ここへ反映した）。
    ...
    def generate_from_conversation(app):
        ...
        **別スレッドに投げてはいけない。ここで最後までやりきる。**
        ...
        （ボタンが「…」になる）を出しっぱなしにする。実測でも
        `process_choice(...) [MainThread]` に対して
        `ConversationStartManager.execute(...) [Thread-594 (execute)]` と、

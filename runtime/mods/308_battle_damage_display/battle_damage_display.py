# -*- coding: utf-8 -*-
"""機能追加: 戦闘で実際に動いた HP を数字で画面に出す。

このゲームの戦闘は地の文だけで進む。
「深手を負わせた」「よろめいた」とは書かれるが、**何点入ったのかはどこにも出ない**。
この mod は1手ごとに HP の増減を測り、地の文の後ろに数行足す:

    ゴブリンの斥候 に 23 のダメージ（残り HP 11/34）
    テストプレイヤー は 8 のダメージを受けた（残り HP 52/60）

味方が敵に与えたぶんも、敵が味方に与えたぶんも、同じ仕組みで出る。
行の頭の記号は `LINE_PREFIX` で選べる（既定は付けない。
候補は環境依存文字を避けてある）。

とどめの一撃は、場から消えた後に測る。
倒した敵は報告より先に `current_enemy_dict` から抜けるので、
「今そこに居る者」だけを見ていると **とどめのダメージだけが丸ごと落ちる**（GAME.md
§2.10）。
台帳には HP だけでなく持ち主そのものを控えてあり、
鍵が場から消えた回にその参照から最後の HP を読んで1行出してから台帳を落とす:
...
`get_base_damage_value(character_attack, weapon_attack)` があり、
ここを包めば「計算された1発の値」は取れる。
それはやらない。
...
そこで**戦闘に居る全員の HP を、1手の前後で比べる**。
誰が誰にいくつ入れたかをゲームの内部語彙で辿らずに済み、
経路が増えても（毒・反射・自傷・回復）そのまま数字が出る。
TECH.md §6.2「語彙を推測するくらいなら、語彙を知らずに済む経路を探す」。

## 台帳方式: 入れ子でも二重に出ない

素朴に「前後を測って差を出す」と、外側と内側の両方を包んだときに同じ変化が2回出る。
そこで最後に見た HP の台帳を持ち、報告点では必ず

    台帳と今の HP を比べる → 出す → 台帳を今の値で更新する

をする。
更新まで含めて1つの操作なので、内側が先に報告すれば外側には差が残らない。
...
**`handle_battle_situation` の後**に出すのが要点。
この中で `calculate_battle_effect` → `resolve_battle_effect`（HP を動かす）→
`process_battle_text`（地の文）が回ると読めるので、
その外側で出せば必ず地の文の後ろに付く。
内側の `resolve_battle_effect` を包むと、地の文より先に数字が出かねない。

`check_battle_end` は「戦闘が続くか」を見るためにゲームが節目で呼ぶ。
ここでの報告は台帳に差が無ければ1行も出さないので、普段は何も起きない。
想定外の経路で HP が動いたときだけ、そのぶんが遅れて出る（黙って落とすよりよい）。
...
HP の持ち主が `Character` インスタンスか辞書かは決めつけない。
どちらでも読める形にしてある（`read_number`）。
最大 HP の属性名は `Character.update_max_hp()` から `max_hp` と読めるが、
これも当たれば使うだけで、
無ければ残量の括弧を省く（属性名の推測でゲームを落とさない）。

## 戦闘が変わったら台帳を捨てる

敵の鍵（`current_enemy_dict` のキー）は戦闘をまたいで使い回されうる。
前の戦闘の台帳が残っていると、次の戦闘の1手目で「満タンまで回復した」が出る。
そこで

  * `BattleStartManager.start_battle` の後に台帳を捨てて取り直す
  * `current_enemy_dict` の**入れ物そのものが差し替わった**ら捨てる
  * 鍵は同じでも名前が違えば別人として扱い、その回は報告しない
...
# ---------------------------------------------------------------- 何を読むか
# HP の属性名。
# `Character.__init__` の `current_hp=None` から（実測の署名）。
HP_KEY = "current_hp"

# 最大 HP の属性名の候補。
# `Character.update_max_hp()` があるので
# `max_hp` が本命だが、**当たらなければ残量の括弧を省くだけ**にして推測でこけないようにする。
...
# 各行の頭に付ける記号。既定は付けない。
#
# 候補は環境依存文字を入れない。cp932 に在り、かつ先頭バイトが NEC 特殊
...
# `LINE_PREFIX` がこの値なら何も付けない。
# 空文字を選択肢にできないので名前で表している。
# GUI は空欄を「未指定」（`None`）として扱い、
# `allow_null` でない設定では弾かれる（`tools/gui.py` の `_ok`）。
# 一覧は読み取り専用なので、空文字を選べたとしても一度選んだら戻せない。
NO_PREFIX = "なし"

# 記号と本文の間に挟むもの。
# **選択肢の側に空白を持たせない**。
# JSON でも GUI の一覧でも見えず、消えたことに気付けないため。
PREFIX_SEPARATOR = " "
...
# とどめの一撃に添える。
# 残量（0/804）を出しても読み手に何も足さないため。
DEFEATED_TEXT = "（撃破）"

# 1回の報告で出す行数の上限。
# 全体攻撃で敵が並んでも画面を埋めないため。
MAX_LINES = 12
...
def apply(ctx):
    ...
    # 台帳。
    # {(side, key): {"name":…, "hp":…, "max":…, "integrity":…}} `enemies` は
    # `current_enemy_dict` の入れ物の id（差し替わったら捨てる）。
    ...
    def combatants(app):
        ...
        敵は `current_enemy_dict`（GAME.md §2.10）、
        味方は `app.player` と名簿の同行者。
        どちらも引けなければその側は空になる。
        片方しか読めなくても読めたほうは出す。
        """
        found = []
    ...
    def check_battle_identity(app):
        ...
        戦闘が変わったのに前の台帳が残っていると、
        次の1手目で「満タンまで回復した」が出る。
        鍵（敵のキー）は戦闘をまたいで使い回されうるため。
        """
        enemies = enemy_dict(app)
    ...
    def remaining_text(record, gone=False, side=ALLY_SIDE):
        ...
        倒れて `current_enemy_dict` から抜けた敵に「（残り HP
        0/804）」と付けても読み手には何も足さない。
        HP が尽きているなら「（撃破）」、
        尽きていない（逃げたなど）なら何も付けない。
        見えている事実だけを書く。
        """
        if gone:
    ...
    # 1手ぶん。
    # **元の処理をすべて通してから**出すので、
    # ゲームの地の文の後ろに付く（この中で HP が動き、地の文も出る）。
    ...
    # 毒などの継続分。
    # 1人ずつ呼ばれるが、台帳が差を吸うので何度出しても重ならない。

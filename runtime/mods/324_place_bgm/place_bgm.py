...
（`out/battle_bgm.log` の `caller`。GAME.md §2.11）、**施設の出入りでは鳴らさない**。
...
どちらの経路で切り替わったかはログの `by` に残す（見張りが要るかどうかは実機で数える。VERIFICATION.md §3.48）。
...
1 と 3 は「セーブ単位の個別指定」。設定画面の「ワールド個別設定」のタブが書く。
...
（どれで取れたかはログに残す。実機では 38/38 が save 経由で、`Area.size` からは取れなかった。GAME.md §2.7）。

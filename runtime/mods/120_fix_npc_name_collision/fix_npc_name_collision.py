...
NPC の名前は LLM が生成する。
小さいモデル（Gemma 系）ほど同じ語を何度も引くので、1つの世界に近い名前が並ぶ。
実際に多いのは次の3種で、完全一致は少数派:
...
`Character.__init__` は名前の唯一の入口（`110_` が確かめた）だが、
そこだけでは足りない。
新しい NPC は `save_data_dict['npcs'][id]` へ先に書かれてから組み立てられる（GAME.md
§2.23）。
だから `self.name` だけ直しても素データには古い名前が残り、次の保存で戻ってくる。
改名したときは id を鍵に持っている辞書（`save_data_dict` / `world_dict` の
`npcs`）を全部書き換える。
既にある鍵への代入なので、セーブの項目の並びは動かない（§2.23）。

## 誰を対象にするか

敵と魔物は対象外。
同じ戦闘に「ゴブリン」が3体並ぶのは重複ではない。
`Character` は敵にも使われる（`generate_character_from_enemy_data`）ので、
受け皿の側では **`npcs` の辞書に id がある個体だけ**を見る。
敵はそこに載らない。
...
`110_` は既存キャラクタを改名したが、あれは**壊れた名前のディレクトリが
Windows 上に存在し得ない**ことが根拠だった。
こちらは違う。
`worlds/<世界>/characters/<名前>/` はもう在って画像が入っている。
改名すると次に作る画像だけが新しい名前の側へ行く。

ロード中（`load_game_new` / `start_game` の最中）に見つけた重複は、
既定では記録だけして名簿に載せる。
載せておけば「後から作られる NPC がそれと衝突する」判定には効く。
設定で `FIX_EXISTING` を立てると、既にいる重複も改名する。

## 名づけを丸ごと引き取る（`ALWAYS_RENAME`。既定は切）

重複しているかに関わらず、これから生まれる NPC の名前を全部こちらで付ける。
LLM が書いた名前は捨てて名簿から引く。
重複が消えるだけでなく、
名前の傾向そのものが名簿で決まる（`npc.json` を入れ替えれば世界の空気が変わる）。
...
| | 既定（切） | `ALWAYS_RENAME` |
|---|---|---|
| これから生まれる NPC | 重複したときだけ付け直す | 全員こちらで付ける |
| 既に世界に居る NPC | 触らない（`FIX_EXISTING` で重複だけ） | 同じ。触らない |
| プレイヤー・敵・魔物 | 触らない | 同じ。触らない |

「既に居る人」を取り違えないことが要点。
`world.characters` と `npcs` は件数が違う（GAME.md §2.23）ので、
名簿を控えるときに**素データの `npcs` も全部控える**。
これを怠ると、
後から組み立てられただけの古参が新顔に見え、**昔からの知り合いが突然改名する**。
同じ NPC を二度裁かない歯止めも要る（`resolve` の頭の番人）。
重複を見て動く既定の側では「衝突していない ＝ 何もしない」が歯止めになっていたが、
`ALWAYS_RENAME` では名前だけでは見分けられない。

> LLM に名前を作らせるのをやめるわけではない。
> 止められるのは結果だけで、生成そのものはゲームのプロンプトの中にある（そこは未調査）。
> 推論の費用は変わらず、出てきた名前を捨てている。
...
`category` は年代と性別を1つで持っている（実データ: `young man` /
`middle-aged woman` / `teenage girl` / `old man` の8種）。
**`woman` は `man` を含む**ので、女性を先に見る。
どちらとも読めなければ引く（名簿を分けている以上、どちらかに倒すしかない）。

### 二つ名は `EPITHET_CHANCE`（既定 10%）

既定を 30% から 10% に下げた（v5）。
`ALWAYS_RENAME` で全員を名簿から命名する遊び方だと、
30% では10人に3人が異名持ちになり「特別な人の印」が薄まる。
10% なら NPC 20人の世界に1人くらいは居て、居れば目立つ。
重複時のみ改名する既定の遊び方ではそもそも改名が稀なので、見せたければ設定で上げる。

名簿の二つ名は3つの形ともそのまま名前の前に付く（`「死神」ドルレイン` /
`沈黙のドルレイン` / `死を運ぶドルレイン`）。

元の名前に付いていた二つ名は引き継がない。
新しい名前は元とは別物なので、元の持ち主の二つ名を連れてくる理由が無い。

### 選び方は乱数（世界ごとに顔ぶれが変わる）

初版は `crc32(id)` で位置を決めていた。
同じ世界を読み直しても同じ名前になる利点はあったが、**世界をまたぐと同じ
id が毎回同じ名前になる**。
遊ぶたび同じ顔ぶれの名前が並ぶ。
名前を選ぶのに再現性は要らないので、乱数に替えた。

乱数は **MOD 専用の `random.Random`**（`RNG`）から引く。
グローバルの `random` を使うとゲーム自身の乱数列がずれる（GAME.md §2.11）。

名簿は全部を混ぜてから前へ進む。
位置を1つ選んでそこから前へ進む形にすると、名簿の前の方だけが繰り返し使われる。

> 乱数でも名前は落ち着く。
> 改名は `character.name` と素データの両方に書くので、
> 次にその NPC を見たときには衝突が無く、二度目の改名は起きない。
> 名前が振れるのは「改名したがセーブせずにやめた」ときだけ。

名簿が読めないとき・名簿が全部使われているときは、
`110_` と同じ立場で記録だけして元の名前のまま通す。
名前を発明する経路は持たない。
"""
...
# MOD 専用の乱数。
# グローバルの `random` から引くとゲーム自身の乱数列がずれる（GAME.md §2.11）。
# 検査は `RNG.seed(0)` で固定する。
RNG = random.Random()
...
def _fold_latin(text):
    ...
    # 重ねた字（`Bell` / `Bel`）は同じ読み。
    # ラテン文字だけ。
    # 仮名で潰すと `ナナシ` が `ナシ` になり、別人が同じ鍵に落ちる。
    out = []
    for ch in text:
...
def read_roster(path):
    ...
    知らない鍵は黙って無視する。
    別の道具で作った名簿をそのまま置けるようにするため。
    読む鍵が1つも無ければ空で返り、呼んだ側が警告を出す。
    """
    if not path:
        return {}, []
    try:
...
def gender_of(context):
    ...
    `woman` は `man` を含むので女性を先に見る。
    読めなければ引く（名簿を男女で分けている以上、どちらかに倒すしかない）。
    """
    text = ""
...
def candidates(pool):
    ...
    位置を頭から探さない。
    名簿の前の方だけが使われて、遊ぶたび同じ名前が並ぶことになる。
    全部を混ぜてから前へ進むので、空いている名前が均等に出る。
    """
    order = list(pool)
    RNG.shuffle(order)
    return order
...
def apply(ctx):
    ...
    # 名簿は MOD のフォルダの中。
    # **`ctx.mod_dir` は `apply()` の中で控える**（外では None になる。
    # GAME.md §2.12 で実際に踏んでいる）。
    ...
    known = {}
    # ロード中は「既にいる人」を見ている。
    # 深さで数えるのは start_game の中から
    # load_game_new が呼ばれても壊れないようにするため。
    loading = {"depth": 0}
    # (id, 元の名前) ごとに1回だけ記録する。
    # 同じ NPC が2つの経路から来ても、直せない重複がロードのたびに現れても、
    # 行が増えないように。
    seen = set()
    ...
    def rename_to(name, cid, context):
        ...
        名簿は男女で分かれているので、片方が尽きたらもう片方も見る（尽きるほど NPC が居る世界で、
        名前が付かないほうを選ぶ理由が無い）。
        """
    ...
    def resolve(cid, name, where, app=None, context=None):
        ...
        if kept is not None and kept[1] == name:
            # この NPC はもう裁いた。
            # `generate_character` で付けた名前を受け皿の
            # `Character.__init__` がもう一度付け直さないための番人で、
            # `ALWAYS_RENAME` のときはこれが唯一の歯止めになる（衝突していなくても改名する経路なので、
            # 名前で見分けられない）。
            return None
        key = canonical(name)
        if not key:
    ...
    def seed(app, where):
        ...
        ここで見つかる重複は既に世界に居るので、
        `FIX_EXISTING` が立っていない限り改名しない（`loading` を立てたまま裁く）。
        """
        known.clear()
        if app is None:
            return 0
        ...
        # `ALWAYS_RENAME` のときはそれがそのまま「昔からの知り合いが突然改名する」になるので、
        # この控えがその歯止めになっている。
        for npcs in npc_tables(app):
    ...
    # 名簿が読めていないと改名が1件も起きない。
    # 必ず名指しで報告する（黙って何もしない MOD になるのが一番分かりにくい）。

...
BLURBS = {
    "解像度": ("アスペクト比は保たれる。倍率 = min(短辺の目標 ÷ 元の短辺, 長辺の上限 ÷ 元の長辺) で、"
               ...
               "ゲームの値のままなら何も書き込まない。"),
    "LoRA 付替": ("ゲームが要求する LoRA の付け替え / 無効化。実ファイル名は拡張子 .safetensors 不要、"
                  "「:強度」を付けられる（例: LCM_LoRA_SDXL:1.0）。「off」でその LoRA を適用しない。"
    ...
    "プロンプト追加": ("画像の種類ごとに (ネガティブ) プロンプトの末尾へ追加する。"
                       ...
                       "「有効」のチェックを外すと、内容を残したまま無効にできる。"),
    "条件付き追加": ("ゲーム本来のプロンプトに特定の語を含む場合だけ末尾へ追記する（男女で LoRA 切替など）。"
                     "種類は any / 立ち絵 / 敵 / 背景。条件は「/」区切りでどれか 1 つ含まれれば成立、"
...
def describe_family(values, game_dir=""):
    ...
    if not path:
        return "チェックポイント: ゲームの選択のまま（系統はゲーム内の選択で決まる）"

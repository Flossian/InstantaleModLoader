...
動いているプロセスに直接聞くことだけになる。
それをやるのがこの mod で、出力された out/recon/targets.txt に載っている名前を、
他の mod がそのまま @ctx.wrap(...) に貼って使う。
...
# 「何が増えて何が消えたか」を機械的に出せなくなる。
# GAME.md §1.5 には、退避があった main_023 → main_024 では 68 ターゲット増を出せたのに、退避が無かった main_024 →
# main_025 では出せなかったと記録されている。
# 退避先は `out/recon_snapshots/<版>_YYYYMMDD.zip` の形で、
# 走るのはビルドが変わったときだけ（同じ版を何度走らせても増えない）。
# 見分け方と「中身の差を引き金にしない」理由は
# `instantale_modloader/recon.py` の「退避」の節にある。
BACKUP_PREVIOUS = True


def apply(ctx):
    # ローダ本体のモジュールは、mod ファイルの先頭ではなく apply() の中で
    # import する。
    # 再注入のたびにローダは sys.modules から消して読み直されるので、
    # ファイル先頭で掴んでも古い方を掴むことがあるため。
    from instantale_modloader import recon

    # 実行環境（Python のバージョン、実行ファイル、
    # ロード済みモジュール数）を先に記録。

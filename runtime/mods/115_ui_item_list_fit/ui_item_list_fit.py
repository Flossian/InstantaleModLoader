...
画面の上端を突き抜けて**先頭の数件が画面の外に出る**。外に出た行は押せないので、
そのアイテムは自由入力に差し込めない。
...
一覧は `scripts.hud.new_hud:ToolListPopup` で、リコンの `modules.json` によれば
**`GridLayout` の派生**。

* だから**列を持てる**。`cols` を 2 にすれば、位置も行の大きさもそのままで
  高さが半分になる。入れ物を作り替えたり、ウィジェットを移し替えたりする必要は
  どこにも無い
* 幅にも余裕がある（一覧の幅 926 に対し、行の幅は 175 前後）

そこでこの MOD がすることは: **`cols` を、窓に収まる列数にする。**
位置（`pos`）には触らない。行の高さも文字の大きさも変えない。行は1件も隠さない。

### ただし、格子が並べてくれないことがある

`cols=2` を入れても見た目が変わらないビルドがある。`size_hint=[1, 1]` なので
一覧の寸法は**親（入力欄の帯）と同じ** 926x78.75 で、中身が要求する高さ
（`minimum_height=1026`）と噛み合っていない。それでも行は 0〜1026 に並んでいる
＝ **行の位置をこの格子が決めていない**。

そこで手順を3つにする:

1. `cols` を入れ、`rows`（個数のプロパティ）が入っていたら外す
   （`cols x rows` の枠が足りないと Kivy はレイアウト自体を行わない）
2. **入れ物の高さを中身ぶんにする**。格子が並べ直すなら、78.75 の箱に押し込まれる
   のを防ぐため先に入れておく（下端は動かさないので伸びるのは上へ）
3. `VERIFY_DELAY` 秒後に**本当に折り返ったか**を見る。行の左端が1種類のままなら
   格子は並べていないので、**行の位置をこちらで入れる**（下端から上へ積み、
   1列埋まったら右の列の下端から続ける）

3 の判定に使うのは「行が実際に何列に並んでいるか」だけで、ビルドの作りを
決めつけない。結果は `out/item_list.log` の `after:` の行に残る。
...
「列を持てること」を条件にしているのは、名前で弾くためではなく、**この直し方が
成り立つ相手かどうか**そのものだから。列を持てない入れ物は、そもそも列にできない。
...
「使ってよい高さ」は窓の高さの `MAX_FILL` と、**一覧の下端から窓の上端まで**の
小さい方。下端は、こちらが触る前の値を1回だけ控えて使い続ける。高さを変えると
下端が動くビルドがあり、そのたびに測り直すと列数が行ったり来たりする。

列数には上限を置く（`MAX_COLUMNS`）。窓の幅から入る列数でも頭打ちにする。
`FIXED_COLUMNS` に 1 以上を入れると、その列数に固定する（「常に2列」）。

## 高さと幅

列を増やすと必要な高さは減る。減った分は**入れ物の側が持っている数**を優先して
入れる。`GridLayout` は中身から `minimum_height` を出すので、それが使えるなら
こちらの計算より正しい。まだ組み直されていない（前の列数のままの）値を掴まないよう、
こちらの計算とかけ離れた値は使わない。
...
控えるのは `cols` と `size_hint` / `size`。**入れ物のウィジェット自身に持たせる**
（MOD 側の変数に持つと、注入し直したときに変えた後の値を設計値として控えてしまう。
`112_` / `113_` と同じ罠）。アイコンをもう一度押す＝一覧が作り直されるので、
そのタイミングで控えを捨てて、ゲームが組んだ寸法から測り直す。

## 触らないもの

一覧の位置、行の高さ・文字の大きさ、中身（どのアイテムが並ぶか）、押したときの
動き、入力欄の文字、セーブ。剥がす（`--unload`）とフックは外れ、そのとき開いて
...
# 一覧が窓の高さに占めてよい割合。窓の縁まで詰めない。
MAX_FILL = 0.9

# 列数の上限。これ以上には増やさない（横に長い一覧は読みにくい）。
MAX_COLUMNS = 4

# 列数を固定する。0 なら件数と窓の高さから自動で決める。
FIXED_COLUMNS = 0
...
# 子のうち行とみなせるものがこの割合を下回るなら、それは一覧ではない
# （背景や枠線が1枚混じる程度は許す）。
ROW_RATIO = 0.6

# 行の大半が文字を持っていること。空の行ばかりの入れ物は一覧ではない
# （アイテム説明の吹き出しがこれで落ちる）。
TEXT_RATIO = 0.6
...
# ビルドのために2回目・3回目を置く。`fit` は何度呼んでも同じ結果になる。
PASS_DELAYS = (0.0, 0.05, 0.25)

# 列にしたあと、本当に折り返ったかを見に行くまでの秒数。最後の当て直しより後。
VERIFY_DELAY = 0.4

# 記録の上限。一覧は開くたびに来るので、書くのは形が変わったときだけ。
# ここに当たると**見届けの行（`after:`）が落ちて**何が起きたか読めなくなるので、
# 同じ形の一覧は1回しか書かないようにしたうえで、上限にも余裕を持たせている。
MAX_LOG = 200

# 控え（触る前の寸法）。ウィジェット自身に持たせる。
...
def apply(ctx):
    ...
    def fresh(widget, rows, before):
        ...
        入れ物を使い回して中身だけ入れ替えるビルドもあるので、**行のどれかが
        新しければ**作り直されたとみなす。
        """
    ...
    def reject(widget, reason):
        ...
        「掴めた／掴めなかった」の理由がログに無いと、実機で1往復ずつ潰す
        羽目になる。**落とした理由まで書く**のがここの仕事。
        """
    ...
    def settled(rows, gap_y):
        ...
        見るのは**行だけ**。入れ物自身の矩形は当てにしない。実機では行が並び
        終わっているのに `ToolListPopup` の矩形が `(0, 0, 926, 78.75)` のまま、
        という瞬間がある（GAME.md §2.14.1。ここを条件に入れると一覧を1つも
        掴めない）。
        """
    ...
    def box_design(box, rows, gap_y):
        ...
        位置の基準も**行から採る**（入れ物の矩形は上のとおり当てにならない）。
    ...
    def remember(box, name):
        ...
        入れ物の矩形をまとめて控えないのは、控えた瞬間の矩形が組み上がる前の値
        だったときに、**戻すとそのおかしな値が入ってしまう**ため。
        """
        design = frames.attr(box, BOX_ATTR, None)
    ...
    def fit_one(box):
        ...
        # 使ってよい高さ。窓に対する上限と、**触る前の**下端から上の空きの小さい方。
        # 下端を控えから採るのは、高さを変えると下端が動くビルドで、測り直すたびに
        # 列数が行ったり来たりするのを止めるため。
        ...
        try:
            if current != columns:
                remember(box, "cols")
                box.cols = columns
            # `rows` も入っていると格子は `cols x rows` で固定され、行数がこちらの
            # 列数と噛み合わない（Kivy は枠が足りなければレイアウトを行わない）。
            # 列数から行数を出させる。
        ...
        track(box)
        # 本当に折り返ったかを見届ける。折り返っていなければ自分で並べる。
    ...
    def relayout(box):
        ...
        `cols` を変えれば Kivy は次のフレームで組み直す。はずだが、`cols=2` を
        入れても見た目が変わらないビルドがある（GAME.md §2.14.1）。促すのは
        ただで、余計に走っても結果は同じなので、こちらからも1回叩く。
        """
    ...
    def verify(box, columns, gap):
        """列にしたあと、**本当に折り返ったか**を見る。

        入れ物が `GridLayout` でも、行の位置をその格子が決めていない作りは
        ありうる（実機の `ToolListPopup` は箱の高さ 78.75 に対して行が 0〜1026 に
        並んでいた）。折り返っていなければ、行の位置をこちらで入れる。
        """
        if not alive(box):
            return
        rows = rows_of(box)
    ...
    def place(box, rows, columns, gap, design):
        ...
        並べ方は今の見え方をそのまま保つ: 下端から上へ積み、1列が埋まったら
        右の列の**下端**から続ける。位置の基準（下端・左端）は控えた値なので、
        何度呼んでも同じ結果になる。
        """
        row_h = max(size_of(row, "height", 0.0) for row in rows)
        row_w = max(size_of(row, "width", 0.0) for row in rows)
    ...
    def resize(box, content_h, content_w):
        ...
        高さは**中身に合わせる。低い側にも合わせる**のが要点。実機の
        `ToolListPopup` は `size_hint=[1, 1]` で入力欄の帯（高さ 78.75）と同じ
        寸法になっており、中身（1026）とまるで合っていない。格子が並べ直すなら
        その 78.75 の中に押し込むことになるので、**先に中身ぶんの高さを入れて
        おく**。下端は動かさないので、伸びるのは上へ。

        高さは入れ物自身が出している `minimum_height` を優先するが、まだ組み
        直されていない（前の列数のままの）値を掴まないよう、こちらの計算と
        かけ離れていたら使わない。
        """
        target = content_h

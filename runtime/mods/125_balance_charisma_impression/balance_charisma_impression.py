...
NPC がプレイヤーに対して抱いている感情は、セーブの
`relationship.player.affinity_text` に**2つ**入っている。

```
"affinity_text": ["警戒心がある", "耐え難いほど魅力的に見えている"]
```

1つ目が好感度の段、2つ目が**見た目の魅力の段**で、どちらも
`scripts.functions:document_emotion_scores_new(affinity, player_charisma)`
が決める（`out/recon/targets.txt`）。この文はそのまま保存され、以後その相手との
会話プロンプトに毎回・全文載る（GAME.md §2.25）。

問題は2つ目のほうにある。**引数はプレイヤーの魅力だけ**なので、

- 誰が見ても同じ段になる（相手の好みが入らない）
- 初対面でも打ち解けた後でも同じ段になる（関係の深さが入らない）

そして段の上端が低い。魅力に振ったキャラは**世界の全員から最上段**
「耐え難いほど魅力的に見えている」で見られ続けることになり、NPC の応答が
無条件に友好的になる ＝ 交渉も説得も勝手に通る。

> 段の英訳がそれぞれ `Not really their type` / `Finds you irresistibly
> attractive` であることからも、ゲーム自身がこれを**相手の好みの問題**として
> 書いているのが分かる。一律なのは「好み」を持てる材料が引数に無いから。

##### どう変えるか

段そのものはゲームのものを使い、**どの段になるか**を2つの要素で決め直す。
...
そこから相手ごとに好みのぶんだけ散る。打ち解けた相手にだけ最上段まで届く。

下限は「ひどく醜く思っている」の1つ上に置いてある（素の段がそれより下なら
そのまま）。好みや親しさで**醜いことにはしない**。魅力に振ったキャラが
相手次第で醜く見えるのは、この MOD が直そうとしている不自然さと同じ種類の
ものになるため。

##### 段の一覧をゲームから読む（文言を持たない）

この MOD は「耐え難いほど魅力的に見えている」のような文字列を1つも持たない。
注入した後の最初の呼び出しで**元の関数を総当たりし**、段の並びをその場で覚える。
...
- 段が1つも見つからない・並びが単調でない場合は**何もしない**（素通し）。
  推測した並びで書き換えるより、効かないほうがよい

覚えた並びと閾値は `out/charisma_impression.log` の先頭に出る。ここが実機での
確認箇所（exe の定数からは閾値が 6/8/11/14/17 と読めるが、そちらは根拠に
していない。VERIFICATION.md §3.23）。

##### 相手が誰かは呼び出し元のフレームから拾う

`document_emotion_scores_new` の引数に NPC は居ない。呼び出し元のフレームを
遡り、`relationship` に `"player"` を持つ `Character`（プレイヤー自身を除く）を
探す。見つからなければ**好みの増減を 0 にする**（親しさのぶんだけが効く）。
拾えなかった回は記録に1度だけ出る。

好みの増減は「世界名:NPC の id:プレイヤー名」の md5 から決める決定的な値で、
遊び直しても同じ相手なら同じになる。乱数を使わないのは、同じ相手の印象が
会話のたびに揺れると「好み」ではなく「気まぐれ」になるため。増減の分布は
中央が厚い三角形（`TASTE_SPREAD=1` なら -1 / ±0 / +1 が 25 / 50 / 25%）。

##### 触らないもの

- 好感度の段（戻り値の1つ目）。会話の結果として動く値で、この MOD の話ではない
- `affinity` そのもの。書き換えるのは**文にする段だけ**で、セーブの数値は動かさない
- 会話プロンプトの組み立て。段が保存された後の経路（`master_ai_facilitator_
  from_conversation` など、会話系5関数を通らないもの）にも同じ文が載るので、
  文を作る1箇所だけを直せば全経路に届く
"""
...
# 相手ごとの好みの幅（段）。0 にすると誰から見ても同じ段になる（親しさだけ効く）。
TASTE_SPREAD = 1
...
# 魅力の側。能力値は作成時 9〜16、セーブで見た上端は 30（GAME.md §2.17）。
# 上端の段がどこから始まるかを確かめたいので、その倍まで振る。
CHARISMA_PROBE = tuple(range(0, 61))

# 好感度の側。実データで見えているのは 0 だけなので、広めに振って端を探す。
...
# 記録の上限。1回の会話で数回しか呼ばれないが、長く遊んでも埋まらないようにする。
MAX_LINES = 400
...
def _taste_of(key, spread):
    ...
    `hash()` を使わないのは、Python の文字列ハッシュが**プロセスごとに変わる**
    ため。同じ相手の印象が起動のたびに変わってしまう。
    """
...
def apply(ctx):
    ...
    def learn(orig):
        ...
        素の関数まで剥がさないのは、**実際に効いている振る舞い**の段を覚えたい
        ため。剥がすと、他の MOD の書き換えを知らないまま段を数えることになる。
        """
    ...
    def npc_in_frames(player):
        ...
        while index < FRAME_DEPTH_MAX:
            ...
            if code.co_name == "<module>":
                # モジュール直下のフレームの中身はグローバル。呼び出しの文脈では
                # ないので、たまたま置かれている Character を掴まない。
                continue
            try:
                # **`dict()` で写す。** Python 3.13 以降の `f_locals` は dict では
                # なく書き戻し用のプロキシで、`isinstance(..., dict)` で弾くと
                # 関数のフレームが1つも読めなくなる（ゲームは 3.10 だが、
    ...
    def emotion_scores(orig, *args, **kwargs):
        ...
        if state["ladders"] is False:
            # 最初の1回で段の並びを覚える。覚えられなければ二度と試さない。
            # 会話の要約は別スレッドで回る（GAME.md §2.25）ので、2つのスレッドが
            # ここへ同時に入りうる。錠は掛けない。総当たりは元の関数を読むだけで
            # 副作用が無く、二重に走っても結果は同じ（記録が1組増えるだけ）。
            try:

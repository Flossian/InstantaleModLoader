...
危険な道は「押す → 生成 → 受注 → 踏破 → 移動」と長く、
その間にゲームの再起動も MOD の注入し直しも挟まりうる。
だから状態はプロセスの中ではなくファイルに持ち、段階（`stage`）で進める。
...
ファイルが正本。
注入し直すと `apply()` がもう一度走り、新しい層は新しい `Journey` を持つ。
古い層が走り続けている（LLM を待っている）間に書いた控えを見落とさないよう、
...
class Journey(object):
    ...
    def _read(self):
        ...
        **読みも `read_json` を通す**（TECH.md §3.11.1）。
        素朴な `open` + 広い `except` だと「まだ道に出ていない（正常）」と「在るのに読めない（一時ロック・外部破損）」が同じ
        None になり、後者でも
    ...
    def save(self):
        # この控えは注入をまたぐ。
        # 道中は「押す → 生成 → 受注 → 踏破 → 移動」と長く、
        # その間にゲームの再起動も注入し直しも挟まりうる。
        # つまり落ちうる局面をまたいで書き続ける。
        # 素朴に open(..., "w") で書くと、その瞬間に落ちれば道が切れる。
        # 差し替えの作法は `instantale_modloader.write_json` に一本化してある。
    ...
    def current(self, world=None, stages=()):
        ...
        `moving`（最後の移動を渡した後）だけは実時間の期限を見て、
        過ぎていたらその場で外す。
        残ったままだと無関係な日数送りまで切り詰めてしまう。
        """
        record = self.record
        if record is None:
            return None

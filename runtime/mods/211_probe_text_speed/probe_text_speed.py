...
発端の症状: **テキスト表示速度の設定を変えても速さが変わらない。** 原因の候補は
3つあり、対策が別々になるので、どれなのかを測って決める:

1. **設定が `app.text_speed` に届いていない。** このとき値そのものが動かない。
2. **`app.text_speed` は動くが、ゲームがそれを間隔に使っていない。**
   （1ティックで進める文字数を変えている、など）
3. **1文字ごとの処理が重くて、設定より遅いところで頭打ちになっている。**
   本文が1文字進むたびに `update_display_text` が走り、そこに**いまは5本の MOD が
   積まれている**（`117_` / `112_` / `113_` / `114_` / `116_`）。1文字あたりの
   処理時間が設定の間隔を超えると、速い設定はどれも「1フレーム1文字」に潰れる。

## 分かっていること（GAME.md §2.3 / VERIFICATION_LOG.md §2.34）

計測は決着済みで、**MOD は無関係**だった ― 設定は届いており、1ティック＝1文字、
間隔は `text_speed` どおり。積まれた MOD の代金は1文字あたり 0.3ms（間隔の 0.6%）。
残った差は**フレームそのものの遅れ**で、Clock は予約時刻を過ぎた最初のフレームで
呼ぶため、フレームが落ちれば `text_speed` どおりには呼べない。

この MOD はその見張りとして残してある。フレームレート（`Clock.get_fps()`）と、
本文のラベルのテクスチャの作り直し（`kivy.uix.label:Label.texture_update` の
回数と時間）まで数えるので、遅くなったときにフックの中と外を切り分けられる。
...
最後が要点。この MOD は**適用順が一番外側**なので（計測は修正より外側 ―
TECH.md §3.2）、`orig(...)` の中に**ゲーム本体と、内側に積まれた MOD 全部**が入る。
...
**この MOD は観測しかしない。** 値は変えず、記録に失敗しても本体は必ず呼ぶ。
"""
...
# 最初の何ティックを1行ずつ出すか。**要約を待たずに間隔が読める**ようにするため
# （要約だけにすると、短いメッセージでは1行も出ないまま終わる）。
DETAIL_TICKS = 40
...
# `app.text_speed` を見張る間隔（秒）。本文が出ていなくても設定の変化を拾う。
POLL_SECONDS = 1.0
...
# 1文字あたりこのミリ秒を超えた塗り直しは、要約とは別に残す（何が重いかの手がかり）。
SLOW_MS = 20.0
...
def apply(ctx):
    ...
    # 計測の途中経過。要約系のカウンタは flush() が 0 に戻す。
    state = {
    ...
    def fps_now():
        ...
        **ティックの間隔はこれに縛られる。** Clock は予約した時刻を過ぎた最初の
        フレームで呼ぶので、フレームが遅ければ `text_speed` どおりには呼べない。
        """
        try:
    ...
    def flush(reason=""):
        ...
        # 判定に使う材料はここまで。結論はログを読む側が出す（推測を書かない）。
    ...
    def note_speed(app, source):
        ...
        if speed == state["speed"]:
            return
        # **締めるのが先** ― 後にすると、前の設定で測った分に新しい設定値の
        # ラベルが付いてしまう（設定と速さの対応を読むためのログなので致命的）。
        flush("speed changed")
    ...
    def note_frame(app):
        ...
        **ワールドによって速さが違う**（同じ `text_speed` でも 46ms と 73ms。
        VERIFICATION_LOG.md §2.34）ので、その差が「ゲーム全体が遅い」のか
        「本文が大きい」のかを分ける。

        本文の大きさを見るのは、ラベルの**テクスチャの作り直しは次のフレームで
        走る**ため ― `update_display_text` の中の計測（`repaint`）には出てこず、
        フレーム時間のほうに乗る。長い本文ほど1文字ごとの作り直しが重くなる。
    ...
    # -- 設定の見張り（本文が出ていなくても拾う） ----------------------------
    # 注入し直したときに**新しい見張りへ入れ替わる**ようにする。
    #
    # `ctx.on_ready(start_poll)` だけでは、注入し直しても見張りが立ち上がらない ―
    # `on_ready` の印はプロセスに残るので2回目は黙って捨てられ、**古い版の見張りが
    # 回り続ける**（TECH.md §3.6）。ログが1行も増えず、「ゲームが何も出していない」
    # と読み違えることになる。
    #
    # 対策は2つ組。印を毎回別のキーにして必ず積み、古い見張りには
    # `ctx.superseded()` で自分から降りてもらう（`revert_all` は Clock の予約を
    # 取り消せない ― TECH.md の `unload()` の但し書き）。世代の持ち回りは
    # ローダの仕事なので、`sys` に合言葉を置くのはやめた（TECH.md §3.6.1）。

    def start_poll():
        """1秒ごとに `app.text_speed` とフレームの様子を読む。

        メインスレッドで動かすため `on_ready` 経由で仕掛ける（`boot()` 自体は
        注入したリモートスレッドの上で走っている）。
        """
        try:
    ...
    # キーに世代を混ぜる ― 混ぜないと2回目以降の注入で積まれない（上の説明）。
        ...
        try:
            ...
            if state["prev"] is not None:
                ...
                if gap > IDLE_GAP:
                    ...
                    state["ticks"] += 1
            state["prev"] = now
            # Kivy が渡す dt は「前回の予約から実際に経った時間」。設定どおりに
            # 呼べているかは、こちらと間隔の両方を並べたほうが確かめやすい。
            if isinstance(dt, float):
                state["dt_sum"] += dt
                state["dt_max"] = max(state["dt_max"], dt)
            state["seen"] += 1
            # 最初の数十回は1行ずつ。要約を待たずに間隔が読める。
    ...
    # ここが `repaint` に出てこない残りの仕事。Kivy はテキストが変わると次の
    # フレームでテクスチャを作り直すので、`update_display_text` の中の計測には
    # 出ず、フレーム時間のほうに乗る。1文字ごとに 1340x2800 を作り直していれば
    # それだけでフレームが落ちる ― **1ティックあたり何回・何ミリ秒**かを数える。
    ...
    def texture_update(orig, self, *args, **kwargs):
        if self is not state["label"]:
            # 本文のラベル以外（ボタン・状態表示）は数えない。計測の対象は
            # 「1文字ごとに作り直している大きいラベル」だけ。
    ...
    def update_display_text(orig, self, instance=None, value=None, *args, **kwargs):
        ...
        try:
            ...
            state["paints"] += 1
            # 1回でどれだけ本文が伸びたか。**間隔ではなく文字数で速さを調節して
            # いる**場合、設定を変えるとここが動く（間隔は動かない）。
    ...
    # 誰が同じ場所に乗っているかを1回だけ残す。犯人を絞るときの出発点になる。

...
引きずる」ことは実測済みで、`conversation_facilitator_after_retrieval` という
...
2系統で覚えられている可能性があるが、**本体側の実体は未実測**。311_ の
...
- 会話の終了は `app.in_conversation` が落ちるのを Clock で見張る
  ...
  `ctx.superseded()` で止まる（TECH.md §5.4・`211_` の教訓）。
...
def leaf_texts(value, out=None):
    ...
    1回目の実測で、JSON の器のまま抜き取って照合すると2通りに誤る
    ことが分かった: 空の `[]` はプロンプトのどこにでも在るので
    「全文相当」に化け（偽陽性）、逆に中身のある dict は鍵や括弧を
    含む断片がプロンプトの**描画済み**の文には無いので「無し」に
...
def apply(ctx):
    ...
    #: 同一人物として diff してしまう（実機で踏んだ ― ハンス(9) とシエラ(9)。
    ...
    def close_tick(_dt):
        # 注入し直されたら止まる（世代の見張り。TECH.md §5.4・`211_` の教訓）。
        if ctx.superseded():
            return False
        try:
            app = ui.find_app()

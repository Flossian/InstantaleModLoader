...
素のゲームの移動手段は `徒歩(3ヵ月)` と `馬車(1000G)` の2つで、日数も料金も文言も
固定されている。この MOD はその3つを mod.json の設定から変えられるようにする。
**既定値はすべて素のゲームの値で、そのままなら挙動は何も変わらない。** 変えた
項目だけが効く（徒歩を 14日 にすれば、の例）:

    [確認画面]  徒歩(14日) / 馬車(1000G・14日)    ← 徒歩は変えたときだけ、馬車は常に日数が出る
                        ↓
                「{name}で目指す。長旅だ...」      ← 文言もテンプレートで差し替え可能
                        ↓
                elapse_days に渡る日数を設定値へ   ← 暦の進め方はゲームのまま
...
| 馬車の料金 | ゲームが引き落とす**前**に差額ぶん所持金をずらす（前払い調整）。引き落としは1回でちょうど設定額 |

**表示と実態が必ず一致する**のがこの作りの利点。ボタンに出す日数は設定値そのもので、
`elapse_days` に渡す数も同じ設定値。ゲームが内部で日数をどう決めていても食い違わない。

「素のままなら触らない」の例外が1つだけある: **馬車のボタンには既定でも日数を
出す**（`馬車(1000G)` → `馬車(1000G・14日)`）。素のゲームは馬車の所要日数を
画面のどこにも出さず（徒歩は `3ヵ月` と出る）、乗るまで何日かかるか分からない。
表示に足す 14 は実測の素の値（`GAME_COACH_DAYS`）なので、表示と実態の一致は
崩れない。挙動（日数・料金・文言）は既定では従来どおり一切触らない。
...
確認画面を開くたびにゲームがラベルを組み直すので、表示も開き直せば元に戻る。
手段の見分けは文字列ではなく `args` の `mode`（実測 `'on_foot'` / `'coach'`。
GAME.md §2.18）**だけ**で行う。ラベルの文字列には下がらない（`kind_of_mode` の
コメントを参照 ― 表示と日数が別々の判定で動くと食い違いを作る）。

## 日数は「渡す数を差し替える」だけ

日数を進めているのは `InstantaleApp.elapse_days(days)`（実測。徒歩で 90 が渡ってくる）。
`orig` は必ず呼ぶので、暦の進め方も日次処理もゲームのまま。窓は
`AreaMoveManager.execute` の間だけで、訓練・休養・他の依頼の日数送りには一切触らない。
1回の移動の中で複数回呼ばれても合計が設定値を超えないよう、予算方式で数える
（`307_` の `TRAVEL_DAYS` と同じ形）。**設定が素の値（`GAME_*`）と同じ手段には
触らない** ― 既定はすべて素の値なので、変えた項目だけが効く。

## 料金は前払い調整（引き落としを1回にする。徴収箇所は推測しない）

馬車代の徴収は `execute` の中で起きる（2026-08-17、`217_` の実測で移動の窓の中の
所持金の差 1000 を観測）。ただし徴収の関数そのものはコンパイル済みで読めないし、
引く額（素の運賃 1000）も変えられない。そこで**引かれる前に「素の運賃 − 設定額」の
ぶんだけ所持金をずらしておく**。ゲームは素の運賃を1回引くだけなので、差し引きは
ちょうど設定額になる。

    手持ち 22788・設定 500:  22788 → (前払い調整) 23288 → (ゲームが1000引く) 22288

最初の版は引かれた**後**に差額を返す方式（`901_` の決済補正と同じ）だったが、
支払いの直後にゲームが描き直す所持金の表示が「1000 引かれた値」のまま残り、
利用者から「文言は500なのに1000減っている」に見えた（実機）。前払いなら
引き落としが最初から1回・設定額ちょうどで、途中の見え方も揃う。ずらした直後に
描画は走らないので、増えた瞬間が画面に出ることもない。

素の運賃はボタンのラベル（`馬車(1000G)`）からそのつど読み取り、読めなければ
実測値に落ちる。手持ちが設定額以上なら調整後は必ず素の運賃以上になるので、
ゲームの残高チェックには弾かれない。設定額に満たないときは押された時点で断る
（`on_button_press` で横取り。画面はそのまま残る）。引き落としが起きなかったら
（弾かれた・徴収が別の場所のビルド）、`finally` で積んだ差額をそのまま返す。

手持ちが素の運賃未満のときの挙動も実測済み（2026-08-18、所持金 0）: 馬車の
ボタンは確認画面に普通に並び、押すと `execute` が走って `金が足りない...` の
一言で中断する。**残高チェックも `execute` の中**なので、前払い調整は必ず
チェックより先に効く（GAME.md §2.18 / VERIFICATION.md §3.27）。

## 文言はテンプレート。空にすればゲームのまま

移動中の文言はすべて `InstantaleApp.add_text(context)` を通る（実測は
徒歩 `徒歩で目指す。長旅だ...`／馬車 `1000ゴールドを支払った。快適な旅だ...`
→ 点 → `辿り着いた。`）。窓の間に名指しの手掛かり（`DEPART_MARKS` /
`ARRIVE_MARKS`）へ当たった文言だけをテンプレートに置き換える。
**テンプレートが空文字列ならゲームのまま**。待機表示の点（`show_loading_text`）
には触らない。手掛かりに当たらなかった文言はログに残す。

馬車の文には**ゲームが金額を焼き込む**が、その金額は料金をいくらに直しても
1000 のまま動かない（実測）。だから馬車の既定テンプレートは実測の文そのもの
（`{price}` 入り）にしてある ― 素の設定なら一字も変わらず、料金を変えれば
文中の金額だけが実際の引き落としに揃う。

テンプレートで使える変数: `{name}`（手段の名前） `{target}`（行き先）
`{origin}`（出発地） `{days}`（設定した日数） `{price}`（馬車の料金）。

## 適用順: `307_area_move_dungeon` より前（内側）に置く

`load_order.json` でこの MOD は 307_ より前にある。307_ は「危険な道」の到着の
移動中、ゲームの `徒歩で目指す...` を伏せる（`add_text` を包んで握り潰す）。
こちらが外側に居るとその文言を先に置き換えてしまい、307_ の伏せが効かなくなる。
内側なら 307_ が伏せた文言はこちらまで届かない。日数はどちらも「渡す数を減らす」
だけなので、重なっても小さいほうが効くだけで壊れない（TECH.md §3.2.1 / §3.3）。
"""
...
# 一切触らない（日数もボタンの表示も料金も）。変えた項目だけが効く。
# 「-1 で無効」のような番人値は使わない ― 画面に出る数字がそのまま素の値になる
# ほうが分かりやすい。素の値との照合は下の GAME_* 定数。
#
# 移動にかかる日数（徒歩=90 は実測。馬車=14 は推定、`217_` で確定させる）。
WALK_DAYS = 90
COACH_DAYS = 14

# 馬車の料金（素のゲームは 1000G。実測）。0 にするとタダ。
COACH_PRICE = 1000

# 手段の名前。ボタンと文言の {name} に入る。馬車を「竜車」にする、など。
WALK_NAME = "徒歩"
COACH_NAME = "馬車"

# ボタンの表示テンプレート。徒歩は日数を**変えた**ときだけこれで表示し直す
# （素の値のままならゲームの表示 `徒歩(3ヵ月)` を残し、呼び名だけ差し替える）。
# 馬車は**常に**この表示 ― 素のゲームは馬車の所要日数をどこにも出さないので、
# 既定でも日数を足す（モジュール docstring の「例外」を参照）。
WALK_BUTTON = "{name}({days}日)"
COACH_BUTTON = "{name}({price}G・{days}日)"

# 移動中の文言テンプレート。**空文字列でゲームのまま。**
# 実測（2026-08-17、`217_`）: 徒歩 `徒歩で目指す。長旅だ...` ／
# 馬車 `1000ゴールドを支払った。快適な旅だ...` ／ 到着 `辿り着いた。`
#
# 馬車の既定は実測の文そのもの（`{price}` 入り）。素の設定なら一字も変わらず、
# **料金を変えると文中の金額が追随する** ― ゲームは料金をいくらに直しても
# 「1000ゴールドを支払った」と言い続けるので（実測）、ここで直すのが唯一の口。
...
# （GAME.md §2.18）。ゲームの更新で変わったらここを書き直す。
WALK_MODES = ("on_foot",)
COACH_MODES = ("coach",)

# 移動中の文言をどれと見なすかの手掛かり。どれも実測の文言に当たる形
# （徒歩 `徒歩で目指す。長旅だ...` ／ 馬車 `1000ゴールドを支払った。快適な旅だ...`。
# 2026-08-17、`217_probe_area_move`）。窓の中でしか見ないので短くてよい。
DEPART_MARKS = ("で目指す", "長旅だ", "ゴールドを支払った", "快適な旅")
ARRIVE_MARKS = ("辿り着いた",)

# 素のゲームの値。**設定がこれと同じ項目には触らない**ための照合値。
# 3つとも実測（徒歩90・運賃1000は GAME.md §2.18、馬車14は 2026-08-17 に
# `217_probe_area_move` の `elapse_days(14)` で確定）。運賃はボタンのラベル
# （`馬車(1000G)`）からも読み取り、読めなかったときの落とし所としてもこの値を使う。
GAME_WALK_DAYS = 90
GAME_COACH_DAYS = 14
GAME_COACH_PRICE = 1000

# ラベルから料金を読む形。`馬車(1000G)` → 1000。桁区切りが入っても読める。
PRICE_RE = re.compile(r"(\d[\d,]*)\s*G")

# 手持ちが設定した運賃に満たないときに出す一言。
...
def kind_of_mode(mode):
    ...
    **文字列（ボタンのラベル）には下がらない。** ラベルからなら推測できるが、
    日数を差し替える窓（`execute`）に来るのは `mode` だけなので、ラベルで
    緩く当てると「表示は変わったのに日数は素のまま」という、この MOD が
    一番やってはいけない食い違いを作る。`mode` が当たらないときは表示も
    日数も料金も文言も**全部ゲームのまま**にして、ログに残す。
    """
    value = str(mode)
...
def move_options(buttons):
    ...
    **`args` は読むだけ**。`[target_area_id, mode]` という並びは `targets.txt` の
    シグネチャそのもので、値の意味は解釈しない（`307_` の `world.move_options`）。
    """
    found = []
...
def apply(ctx):
    ...
    state = {
        # いま `AreaMoveManager.execute` の中に居るかの窓。中身は _open_window。
        "window": None,
        # 確認画面のラベルから読み取った素の運賃（読めた最新の値）。
        "game_price": None,
        # 自分が最後に書いた馬車のラベル。画面がラベルを組み直さないビルドで
        # update_button_display がもう一度来たとき、**自分の書いた 300G を
        # 素の運賃として読み込まない**ための目印。
        "our_coach_label": None,
    }

    def days_limit(kind):
        """その手段に**設定で変えられた**日数。素の値のまま（触らない）なら None。"""
    ...
    def relabel(kind, old):
        ...
        徒歩: 日数を**変えた**ときだけテンプレートで表示し直す。素の値のままなら
        ゲームの実表示（`3ヵ月`）を尊重し、呼び名だけ差し替える。

        馬車: **常に**テンプレートで表示し直す。素のゲームのラベルは `馬車(1000G)`
        で所要日数が読めないため、既定でも日数を足す（表示だけの例外。挙動は
        従来どおり、変えた項目にしか触らない）。日数が素のままのときに出す数は
        実測の素の値（`GAME_COACH_DAYS`）― `elapse_days` で実際に進む数と同じ。
        """
        if kind == "walk":
        ...
        if kind == "coach":
            ...
            new = fmt(COACH_BUTTON, name=COACH_NAME, price=shown_price,
                      days=days)
            # 同じ文字列なら None ― ラベルを組み直さないビルドで自分の書いた
            # ラベルにもう一度来ても、塗り直しを繰り返さない。
    ...
    def confirmation_buttons(orig, self, *args, **kwargs):
        ...
        素の運賃はここで（書き換える**前**のラベルから）読み取って控える。
        描画はもう済んでいるので、変えたときだけ次のフレームで塗り直す
        （`307_` と同じ。二度目は同じ文字列になるので塗り直しは繰り返さない）。
        """
    ...
    def on_button_press(orig, self, button_index, *args, **kwargs):
        ...
        ゲームのボタンは触らず押下だけ握る。確認画面はそのまま残るので、
        徒歩を選び直せる。読めなかったときは通す（値が読めないことを理由に
        遊びを止めない。`307_` の体力の断り方と同じ）。
        """
        try:
            if fare_changed():
    ...
    def settle_fare(app, window):
        ...
        起きていなければ（弾かれた・別の徴収方式のビルド）、前払いで積んだ
        差額をそのまま返す。想定外の値になっていたら触らずに WARN を残す
        （移動の中で料金以外の入金があった場合を壊さないため）。
        """
    ...
    def area_move_execute(orig, self, choice_text=None, *args, **kwargs):
        ...
        料金を変えているときは、ゲームが引き落とす**前に**「素の運賃 − 設定額」の
        ぶんだけ所持金をずらしておく（前払い調整）。ゲームは素の運賃(1000)を
        1回引くだけなので、差し引きはちょうど設定額 ― 「1000引いて500返す」の
        ような紛らわしい動きを画面に出さない（実機で利用者が踏んだ。引いてから
        返す方式だと、支払い直後の所持金表示が1000引かれた値のまま残る）。
        ...
        # 手持ちが設定額以上なら、調整後は必ず素の運賃以上になるので、ゲームの
    ...
    def reword(window, text):
        ...
        名指しの手掛かりに当たった文言だけをテンプレートへ。待機表示の点は
        触らない。当たらなかった文言はログに残す（馬車側の実文言を知るため）。
        """
    ...
    # ------------------------------------------------------------ 自己検証
    # 実経路はエリア移動を1回するまで通らない。ラベルの読み書きだけは作った
    # データで先に確かめておく（`103_` / `215_` と同じ方針）。

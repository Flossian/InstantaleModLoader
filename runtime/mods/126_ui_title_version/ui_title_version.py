...
注入はウィンドウが出てから行われる（TECH.md
§1.4）ので、**注入した時点のタイトル画面はもう組み上がっている**。
`__init__` を包むだけでは、目の前の画面には何も出ず、
一度タイトルへ戻るまで確かめられない。
...
どちらも同じ `decorate()` を呼ぶ。
`apply()` は1プロセスで何度も呼ばれるので（TECH.md
§3.4）、**足す前に自分の印が付いたラベルを外す**。
`on_ready` のキーに世代を混ぜてあるのは、
注入し直したときに目の前のラベルも新しい版へ置き換わるようにするため（TECH.md
§3.6.1）。
...
既定の文字は ASCII だが、設定で日本語にできる。
Kivy の既定フォント（Roboto）に日本語は無いので、**タイトル画面のウィジェットからフォント名を写す**（写さないと豆腐になる。
`116_` と同じ理由）。
写す先が見つからなければ既定のままにする。
ASCII の既定文字なら問題なく出る。
"""
...
#: Kivy の既定フォント。
#: 日本語を持たないので、これを写しても意味が無い（写さなかったのと同じ）。
#: 見つけても採らず、他のウィジェットを見に行く。
DEFAULT_FONT = "Roboto"
...
def apply(ctx):
    ...
    def decorate(screen):
        ...
        # 文字の箱に合わせておかないと、ラベルの既定の大きさ（100x100）のまま隅に寄るので、
        # 文字が窓からはみ出す。
    ...
    # キーに世代を混ぜる。
    # 混ぜないと、注入し直したときに目の前のラベルが古い版のまま残る（TECH.md
    # §3.6.1）。

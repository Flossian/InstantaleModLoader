...
同じ行（`instantale.py:876`）が 1 を渡すようになった。**この mod は何もしない
ことを実機で確認済み**（新規作成で `fixed 0`。効く条件の2つ目が外れる）。
`mod.json` に `superseded` を入れてデバッグモード限定にしてある。

残っている用途は1つだけ。**このバグで既にレベル60で保存されたセーブの修復**で、
本体の修正は既存のセーブには効かない。設定 `REPAIR_LOADED` を入れて読み込むと
直る（既定は切のまま）。以下は当時の記録。

##### 何が起きていたか

ゲーム本体の `InstantaleApp.start_game(world_name)` が、新規開始でプレイヤーの
`Character` を組む行でレベルに **60** を渡している。経験値は渡していない（既定の 0）。
...
`max_hp` は `__init__` の中でレベルから計算されるので、先に直せば HP も
レベル1ぶん（`耐久 × 4.8`）になる。渡された体力上限（レベル1の値）と揃う。

レベルを後から代入し直すのではなく引数を差し替えるのは、`__init__` が
レベルから決めるもの（HP のほか、まだ特定していないものがあるかもしれない）を
**全部まとめて**正しい値にするため。
...
4つ目が要点。「新規開始だから」ではなく「レベルだけが他と食い違っているから」
直す作りなので、`start_game` が続きからの読み込みにも使われていた場合でも
巻き込まない（新規専用の関数だと確かめたわけではない ― 分かっているのは
`create_world` から新規で呼ばれる経路だけ）。

##### 既に保存されてしまったセーブ

読み込み時にも同じ食い違いは見えるが、**既定では直さない**。遊んでいた
キャラのレベルと HP が黙って下がるほうが事故になるため。記録に警告を出すだけに
してあり、直したい人は設定 `REPAIR_LOADED` を入れる。

##### 二段目の保険

`start_game` を包み、抜けた直後にもう一度プレイヤーを見る。`__init__` より後で
レベルを代入し直している経路があった場合（未確認）、そこで気付けるようにするため。
...
# 本体が宣言しているレベルの下限。無ければこの値を使う。
FALLBACK_LEVEL_MIN = 1

# ログの上限。1回の起動で何度も新規作成することはないので小さくてよいが、
...
def apply(ctx):
    ...
    if functions is None:
        # 体力上限の照合ができない＝食い違いを検出できない。推測で書き換えるより
        # 何もしないほうがよい（この mod の唯一の判断材料がこれ）。
    ...
    def verdict(kwargs):
        ...
        if "experience_level" not in kwargs:
            # 位置引数で来ている可能性。並びを数えて当てるのは危ういので触らない。
        ...
        if not isinstance(level, int) or level <= level_min:
            # 本体が直った、または既に最小値。**ここでこの mod は無効になる。**
            return False, None
    ...
    def character_init(orig, self, *args, **kwargs):
        # 判定を先に済ませ、引数を触るのは決まった後だけにする。`safe=True` の
        # 落とし先は「元の関数を同じ引数で呼ぶ」なので、壊れた状態の引数を
        # 残したまま例外を投げると、その引数で本体が走る。
        fix, note = False, None
        try:
        ...
        try:
            ...
            if after != level_min:
                # `__init__` の中でレベルを決め直している。引数だけでは足りない。
    ...
    #
    # `__init__` より後でレベルを代入し直す経路が在るかは未確認。在った場合に
    # 「直したのに 60 のまま」で終わらないよう、ここで気付けるようにしておく。
    # **直さない** ― 見えていない経路を推測で書き換えると、次に読む人が
    # どちらが効いたのか分からなくなる。

...
`ConversationStartManager.__init__` の追跡（`301_` の手口）に頼らなくてもよい。
仲間の label から会話に入る経路（`process_party_member_choice`）でも、会話画面に
なった時点でこのボタンは並ぶので同じように効く。

パーティの名簿はセーブの `game_variables['party']`（`['player', '63', ...]` の
**id の配列**）。ただし**在り処も形も決めつけない** ― 実機では `app.party` が
存在しない時期があり、それで判定が黙って外れてボタンが出なかった。候補を集めて
中身（`'player'` を含むか）で本物を選び、`list` でも `dict` でも読む
...
`get_party_leave_facility` は **`(施設, ノード)` のタプル**を返す（実機で判明）。
`move_npc_to_facility` は施設とノードを別々に取るので、ほどいてそれぞれの位置に
入れる。中身が何なのかは**解釈しない** — ほどいて渡すだけにする
（`301_` で `QuestChoiceManager` の引数を推測して落とした反省）。
...
- `original_party` が**現在の名簿と食い違っている**＝一時的な差し替えの最中
  （入っているだけでは平常。実機では常に名簿と同じ内容で入っていた）
...
閉じずに進めると立ち絵が残って付いてくる（`301_` で実際に起きた）。
...
# **役目は済んだので既定オフ**（2026-07-27）。`method_1 (instantale.py:6602)` は
# `QuestEndManager`（`method_1@6511` / `execute@6634`）と確定した。以後は
# `owner_of` が記録の時点でクラス名を出すので、この表は要らない。
...
def apply(ctx):
    ...
    def blocking_reason(app, member_id):
        ...
        if getattr(app, "current_quest_data", None):
            return "in a quest"
        # **`original_party` は判定に使わない。** 2度続けて読み違えた:
        #
        #   1回目「入っていたら差し替え中」 -> 平常時も名簿と同じ内容で入っており、
        #         仲間が居ると毎回断っていた（2026-07-27）
        #   2回目「名簿と食い違えば差し替え中」 -> 雇用直後は
        #         original_party=['player'] / party=['player','18'] と食い違う。
        #         単に**控えが更新されていないだけ**で、差し替えではなかった
        #         （2026-07-28。またボタンが出なくなった）
        #
    ...
    def game_destination(app, character):
        ...
        `get_party_leave_facility` は実機では **`(Facility, Node)` のタプル**を
        返した（2026-07-26）。`move_npc_to_facility(character_id,
    ...
    #
    #   * **段数で数えない。** `sys._getframe(2)` は `@ctx.wrap` の層を指してしまう
    #     （実機で `from wrapper (…/patch.py:312)` になった）ので、ファイル名で
    #     こちら側のフレームを飛ばす

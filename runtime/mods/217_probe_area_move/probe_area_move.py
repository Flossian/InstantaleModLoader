...
この計測は `314_` / `307_` より後（外側）に置くので、`elapse_days` と `add_text` は
**MOD が差し替える前のゲームの生の値**が録れる（TECH.md §3.2.2 の「計測は修正より
後」）。一方 `update_button_display` のラベルだけは orig が返った後に読むため、
`314_` が書き換えた**後**の姿になる。生のラベルが要るときは `314_` を切って録る
（spec の `args` はどちらでも生のまま）。
...
# 1回の移動で写す文言の上限。移動中の文言は数行のはずで、これを超えるなら
# 想定していない経路（会話など）が窓に混ざっている。超えたことも記録する。
TEXT_LIMIT = 30

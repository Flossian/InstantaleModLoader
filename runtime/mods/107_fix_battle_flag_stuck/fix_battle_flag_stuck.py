...
## 実測（2026-07-27、`207_` のログ全件）

戦闘終了マネージャは2つあり、**フラグを下ろすのは片方だけ**だった。

| 終了マネージャ | `end_phase` 完了時の `in_battle` | 件数 | 入口 |
|---|---|---|---|
| `BattleEndManager` | **0**（ゲーム自身が下ろしている） | 8/8 | クエスト中のエンカウントなど通常の戦闘 |
| `BattleEndInFreeAction` | **1**（下ろし忘れ） | 9/9 | 自由入力・**会話から入った戦闘** |

`106_`（戦闘BGM）で見つけた「復帰呼び出しが app ではなく `self` を渡している」のと
...
```
instantale.py:1458  play_music_from_src(battle)   ← in_battle=1 のときの枝
...
**下ろすのは `in_battle` だけ。** 実測で 1→0 の遷移が確認できているのがこれだけで、

...
つまり `config.json` の `cloud_llm` に新しい名前を直接書くと、
名前は届くが壊れた経路に落ちる。
加えて価格表（`calculate_price`）にも無いキーになる。

そこで名前を足しに行かず、**ゲームには一覧にある名前のまま持たせ、
HTTP に出る直前で差し替える**。
ゲームは `SOURCE_MODEL` のつもりで速い経路（responses.parse +
effort="none"）を組み立て、実際に飛ぶのは `TARGET_MODEL`。
...
`client.beta.chat.completions.parse` も、
最後はここを通る（リソース側の `self._post` は `self._client.post` そのもの）。
エンドポイントごとに包むと SDK の更新で取りこぼすので、1本に絞ってある。
...
出ないときは、ゲーム側が別のモデルを選んでいるか、
クラウドではなくローカル LLM で動いている（`openai` を通らない）。
...
# 実際に送る名前。
# 空にするか SOURCE_MODEL と同じにすると何もしない。
# **`mod.json` の "default" と揃えること**（`tools/check_mods.py` が
# AST で突き合わせる。TECH.md §3.8.3）。
# 宣言のある設定は `config.resolve()` が
# mod.json 側から組み立ててモジュールへ書き込むので、
# ここがずれていると **GUI に出る値とコードを読んで分かる値が食い違う**（効くのは
# mod.json の側）。

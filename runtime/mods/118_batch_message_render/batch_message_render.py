...
def apply(ctx):
    ...
    def compact(text):
        ...
        **そもそも探すのはなぜか。**
        本文は `add_text_display(..., index=-1)` で全文を受け取っているので、
        「届いた時点の添字を控えれば探索は要らない」ように見える。
        塗る相手が `hud.display_text` ならそのとおりだが、実際に塗るのは
        `hud.text_display.text` で、この2つは一致しない（GAME.md §2.3）。
        添字が当てにならなくなる道が4つある:

        * 内側の `117_` が長い本文を「前置き + 省略通知 + 末尾N文字」へ
          載せ替える。切り詰めた瞬間、先頭からの添字が全部ずれる
        * `shown` には控えのどれにも属さない隙間が挟まる（下の `gap`）
        * ゲームは表示文字列を自分の控えから丸ごと組み直すことがある
        * 打ち出し中の改行混入（上のとおり）

        探索は4つとも素通りするが、添字はどれか1つで壊れる。
        だから位置ではなく中身で引き当てている。
        """
        chars = []
        positions = []

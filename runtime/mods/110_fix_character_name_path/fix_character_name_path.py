...
## 何が起きているか（GAME.md §2.15 / VERIFICATION_LOG.md §2.14）

LLM が生成した名前に引用符が混入すると（`試験人形「テストダミー"` のように
`「` で開いて ASCII の `"` で閉じる）、
`"` は Windows のパス構成要素に使えないので `os.makedirs` が
`OSError: [WinError 123]` で落ちる。
...
5箇所で個別に消毒すると、
書き込みと削除で消毒がずれたときに別の不整合（消し残し・迷子）を生む。

名前の唯一の入口は `scripts.characters:Character.__init__` で、
LLM生成・プリセット・プレイヤー・セーブからのロードが全部ここを通る。
ここで名前そのものを正せば、
5箇所は今までどおり「名前をそのまま使う」だけで全部一致する。

引数ではなく `self.name`（`__init__` を抜けた後）を正す。
ゲームが名前をどの引数からどう組み立てているかを知らなくても効くのと、
`name=None` のような経路を巻き込まないため。
...
* 予約デバイス名（`CON` / `NUL` / `COM1` …）はディレクトリ名にできないが、
  直すには名前を発明するしかない。
  一度も観測していないので記録だけする。
* 消毒の結果が空になる名前（全部が不正文字）も元のままにして記録だけ。

## 既存キャラクタの救済

壊れた名前を持つキャラクタはもう世界の中に居る。
`world_data.json` は難読化されていて外部からは書き換えられない（GAME.md
§2.16）ので、**注入後にゲーム内から直す**しかない。
この mod は3か所で名簿を掃く:
...
**旧名のディレクトリは
Windows 上に存在し得ない**（不正文字を含むディレクトリを作成できない ＝ 落ちたのがまさにそれ）。
だから改名しても画像の迷子は発生しない。
これが「既存キャラクタを触ってよい」根拠。

残る懸念: セーブ側 JSON には旧名が残り、次の保存で入れ替わる。
その間、名前で突き合わせる処理があると食い違う。
突き合わせは id で行われているように見えるが未確証なので、
旧名は必ずログに残す（`out/character_name.log`）。

同じ理由で、名前と同じ生文字列を持つ他の属性が同じインスタンスにあれば記録だけする（勝手に書き換えない。
どこが名前の写しなのかは未確認のため）。

世界名は対象外。
`worlds/<世界>/` も同じ壊れ方をしうるが、
世界名の入口は未調査なので触らない（発生すれば `001_` が同じ形で捕まえる）。
"""
...
def sanitize(name):
    ...
    戻り値は `(新しい名前, 理由)`。
    理由はログ用で、`reserved` / `empty` のときは新しい名前が None ＝ 触らない。
    """
    if not isinstance(name, str) or not name:
        return None, None

    cleaned = "".join(REPLACEMENTS.get(ch, ch) for ch in name)
    # 制御文字はパスに置けず、そもそも表示もされない。
    # 写す先が無いので落とす。
    ...
    if not cleaned:
        ...
        # 埋める文字を発明しない。
        return None, "empty"
    # 予約名は「不正文字が1つも無いのに作れない」別種なので、**何も変わっていなくても先に見る**（`CON` はこのままでもディレクトリにできない）。
    # 判定は拡張子の手前まで、大文字小文字を問わない。
    # Windows の規則。
...
def apply(ctx):
    ...
    def echoes(character, raw):
        """名前と同じ生文字列を持つ他の属性。記録だけする（書き換えない）。"""
        found = []
        try:
    ...
    def sweep(app, where):
        ...
        名簿は `{id: Character}`（GAME.md §2.7）。
        保存はしない。
        ゲームが次に保存するときに一緒に入る。
        ここで保存を起こすと、こちらが選んだ時点でセーブを書き換えることになる。
        """
        if app is None:
            return 0
        fixed = 0

...
### ここで `frames.MethodWatch` を使うと**必ず誤爆する**（オフライン検証で判明）

`304_` のようにスタックを見る手も考えたが、この mod は**見張りたい `execute` を
自分で包む**ので成立しない。`MethodWatch` は最初に呼ばれた時点で
`getattr(cls, "execute").__code__` を表にするが、そのときそこに載っているのは
...
## 出すのは「プレイヤーの獲得経験値」の後（実機の並びで決めた）

実機1回目のログ（2026-07-30。VERIFICATION.md §3.12）で分かった訓練1回の並び:
...
# 画面上は何も起きていないように見える（実機1回目がその状態だった。VERIFICATION.md §3.12）。
ANNOUNCE_GAIN = True
...
def apply(ctx):
    ...
    # ------------------------------------------------------------ 画面に出す
    # **その場では出さない。** 実機のログで分かった訓練1回の並びはこうなっている
    # （2026-07-30。VERIFICATION.md §3.12）:
    #
    #     add_text('テストプレイヤーはしばらくの期間を訓練に費やした...')
    #     player.gain_exp(686852)                ← こちらが割り込む位置

...
症状（ユーザー報告・2026-07-26）: **会話から戦闘に入ると、戦闘終了後も戦闘 BGM が
流れ続ける**。通常の戦闘（クエスト中のエンカウント等）では戻っている。
...
（`app.music` はエリア曲のまま → 復帰の呼び出しが「もう鳴っている」と判断して
何もしない → 戦闘曲が流れ続ける）。そこで前後の `app.music` を必ず記録する。
...
NAME = "probe: battle BGM switching (play_music_from_src call sites)"

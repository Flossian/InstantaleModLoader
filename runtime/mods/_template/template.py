...
このフォルダは読み込まれない。
先頭が `_` のフォルダはローダが探索から外すので（`discover()`）、
雛形を置いたままにしても GUI の一覧に出ないし適用もされない。
使うときは**フォルダごとコピーして名前を付ける**:
...
特に効いてくるのは次の2つ:
  ...
  * `apply()` は1プロセスの中で何度も呼ばれる（再注入と遅延当て直し）。
    何度実行しても結果が変わらないように書く（§3.4 / §3.5）
  * 副作用のある初期化は `ctx.on_ready()` に預ける（§3.6）
"""

# 利用者が GUI から変えられる値は、モジュール直下の定数として置き、
...
def apply(ctx):
    ...
    # フックをまたいで持ち回す値は辞書に入れる。
    # `nonlocal` でも書けるが、辞書なら入れ子のどこからでも読み書きできる（既存
    # MOD もこの形）。
    state = {"seen": 0}

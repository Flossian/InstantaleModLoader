...
TECH.md §4.4）。したがって同梱ログは現行版のバグ一覧として読める。

ただし記録されているクラッシュ地点のいくつかは、モジュールのグローバルには
見当たらない（`send_request_on_id` はトレースバックに 62 回出るのに
`vars(module)` には無い ＝ ネスト関数）。地点名から場所を引けないことはあるので、
起きたその瞬間を自分で記録する必要がある。
...
同梱のものとの違いは3点:
  ...
  * 連鎖に番号を振るので、どれが引き金でどれが巻き添えかが一目で分かる。
    Kivy の終了時クラッシュ（100_fix_kivy_shutdown.py）の原因究明に
    時間を取られたのは、まさにこれが分からなかったせい。
...
def apply(ctx):
    ...
    # 存在確認に hasattr を使わない（TECH.md §8。トリップワイヤを仕掛けた相手だと

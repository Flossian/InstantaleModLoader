# -*- coding: utf-8 -*-
"""機能追加: クエストクリアの解散で、仲間を**いま居る町のギルド**に残す。

ゲーム本来のクエストクリアは、同行した仲間を**雇った町（`initial_location`）**へ
帰してしまう。冒険から戻ってきたその足で相手が世界の裏側へ消えるので、また
...
* 解散は `QuestEndManager.method_1`。クエスト放棄側は `QuestRetireManager` が
...
`remove_party_member` と `get_party_leave_facility` だけ**にして、こちらは
「いま解散した相手か」を辞書で引くだけにしてある（常時走る経路を重くしない）。
...
共通部品に上げた。見張る相手の名前＝設計判断だけがこちらに残る）。
...
記録は `302_` と同じ `party_leave.log` に `quest-end:` を付けて書く。パーティの
増減はあちらが既に書いているので、**1つの時系列で解散の一部始終が読める**方が
...
NAME = "feature: quest-end disband leaves members at the guild of the town you are in"
...
# クエスト**放棄**でも同じことをするか。`QuestRetireManager` が同じ役どころだと
# 思われるが**未観測**なので既定では触らない。放棄でも町のギルドに残ってほしく
# なったら True にする（捕まえ方は同じで、対象クラスが増えるだけ）。
...
# 見張るメソッド。解散そのものは `method_1` の中だが、置き直しがどちらの層から
# 呼ばれているかは決めつけない。コードオブジェクトが引けないビルドでは、この名前の
# フレームに出会ったら持ち主クラスまで確かめる予備に落ちる（`frames.MethodWatch`）。
...
# 解散の後、ゲームが置き直すのを待つ秒数。ここまで誰も動かさなければこちらで
# 置く。観測できているビルドでは removal の直後に置き直しが来るので使われない。
PLACE_TIMEOUT = 8.0

# 置き先を差し替えたことを画面に出すか。**ゲーム自身は行き先を言わない**ので、
# 出さないと「離脱した」だけが残り、どこへ行ったのか分からなくなる。
# 出力は手が空いてから（クエスト終了は要約の流し込みが続くため。`300_` の知見）。
ANNOUNCE_DESTINATION = True
ANNOUNCE_TEXT = "{name}は{place}に留まることになった。"

# こちらが**自分で置いた**とき（時間切れの保険が働いたとき）だけ保存する。
...
def apply(ctx):
    ...
    # 解散マネージャのコードオブジェクトを先に引いておき、スタックの `f_code` と
    # 突き合わせる見張り（判定そのものは `frames.MethodWatch`。ここに残るのは
    # 「誰を見張るか」＝この mod の設計判断だけ）。表は初回の判定時に1度だけ引く。
    ...
    def register(app, member_id, source):
        ...
        帰還は解散より先に済んでいるので、ここで引く「いま居る町」は**戻ってきた
        町**になる（実測の順序。docstring 参照）。後から引き直すと、待っている
        間にプレイヤーが移動した町を拾ってしまう。
        """
        ...
        # ゲームが誰も動かさなかったときの保険。Clock なのでメインスレッドで走る
        # （解散はゲーム側の別スレッド）。
    ...
    def announce(app, name, facility):
        ...
        # クエスト終了は報酬・才能・要約と出力が続く。その最中に差し込むと
        # 押し流されるので手が空くのを待つ（`300_` の知見）。既に確定した
        # 出来事なので、待ちきれなくても出す。
    ...
    def late_place(app, member_id):
        ...
        if SAVE_AFTER_LATE_PLACE:
            # ゲームが置いた場合はゲーム自身の保存に乗るので触らない。ここは
            # こちらが勝手に置いた場合だけで、保存されないと次回起動で戻る。
            try:
                app.save_game()
    ...
    def remove_party_member(orig, self, member_id, *args, **kwargs):
        ...
        try:
            ...
            ctx.log_exc("quest-end guild: cannot tell where remove_party_member "
                        "was called from")
        if source is None:
            # 死別・`302_` の会話からの別れ・その他。こちらの管轄ではない。
    ...
    def get_party_leave_facility(orig, self, character_instance=None, *args, **kwargs):
        ...
        **戻り値の形はゲームのものに合わせる。** 実機では `(施設, ノード)` の
        ...
        try:
            ...
            # 解散の中でなければこちらの管轄ではない。**握り潰さずそのまま投げる**
            # （mod があるとゲームの失敗が消える、が一番たちが悪い）。

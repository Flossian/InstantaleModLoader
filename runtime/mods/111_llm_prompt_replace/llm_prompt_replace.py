...
経緯は GAME.md §2.12 / VERIFICATION_LOG.md §2.24。
...
より後＝外側に置き、置換は**圧縮前**の本文を見る（ルールが素のスキーマ repr を狙える
ようにするため。同梱の既定は狙っていないが、利用者のルールは狙える）。完全一致を
前提に書き換える MOD は逆にこちらより外側へ置いて素の本文を見せる
（その宣言はあちら側が持つ）。

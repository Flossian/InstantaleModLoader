...
3. **同じ仕事を二度積まない。** 鍵が同じ仕事は、待っている間と処理している間は1つ
...
class Worker(object):
    ...
    def waiting(self, key) -> bool:
        """その鍵の仕事が待ち行列に居るか、処理中か。`key=` を渡していないときは常に False。"""
        with self.lock:
    ...
    def rebind(self, ctx, run=None, write=None) -> "Worker":
        ...
        （`self.run(job)` が `self.run` を読むのは呼ぶ瞬間の1度きりで、
        あとはその関数の中に居る）。
    ...
    def enqueue(self, job) -> bool:
        ...
        **処理中の鍵も積まない**（鍵を外すのは `run` が終わってから）。
        積む側は「前の結果から変わったか」で積むかを決めていて（`317_` の素材の印、
        `321_` の控え済みの件数）、その結果は `run` が終わるまで書かれない。
        処理中に受けると、同じ入力の仕事がもう1件積まれて LLM を二度呼ぶ。
        処理中に来た変化は、`run` の後の次の照合で積み直される。

...
分けてあるのは責務の都合だけではなく、依存の向きを一方通行にするため
（patch.py → patch_registry.py の片道。逆向きの import は無い）。

台帳が要るのは、このローダが**同じ対象に複数の MOD を意図的に重ねる**から。
ファイル名順の適用はそれ自体が設計で、例えば BGM 系（`104_` / `106_` / `207_`）や
...
なって apply が止まるが、その場合も**止まる前にここへ記録してある**ので、
boot の最後にまとめて報告できる。

台帳は boot ごとに作り直す（`reset`）。前回の boot の帰属を残すと、遅延再適用の
たびに一覧が二重三重になるため。
"""

from __future__ import annotations

# 今 apply() を実行中の MOD。boot() が MOD ごとに出し入れする。
# patch.py は「誰が呼んだか」を知らないので、ここを見て帰属を決める。
_current: str | None = None

# 記録の本体。(種類, 対象, MOD, 補足) の並び。適用順がそのまま並び順になる。
...
def end_mod() -> None:
    ...
    必ず finally から呼ぶこと。apply が例外で抜けたときに前の MOD の名前が
    残っていると、次に記録されたパッチが**無関係な MOD のもの**として台帳に載る。
    """
    global _current
    _current = None
...
def record(kind: str, target: str, *, detail: str = "") -> None:
    ...
    `_current` が None なのは、MOD の apply() の外から当てられた場合
    （遅延再適用の途中や、ローダ自身が当てた場合）。捨てずに "<loader>" として
...
def by_mod() -> dict[str, list[str]]:
    ...
    `by_target` の逆引き。GUI で MOD を1つ選んだときに「この MOD は何処を
    触っているか」を出すのはこちら。台帳を持つ意味の半分はこの向きにある。
    """
    table: dict[str, list[str]] = {}
    for kind, target, mod, _detail in _entries:
        if kind != APPLIED:
...
def conflicts() -> dict[str, list[str]]:
    ...
    「壊れている」ではなく「重なっている」の一覧。重ねるのは正常な使い方なので
    警告ではなく報告として出す（判断するのは MOD 作者）。
    """
...
def format_report() -> list[str]:
    ...
    deferred = entries(DEFERRED)
    if deferred:
        # 対象ごとではなくモジュールごとにまとめる。見張りが待つ単位がモジュールなので。

...
この数も更新すること。32 本は、デバッグモードを切ったときに読み込まれる公開ぶん
（同梱 53 本から、`debug` の 13 本と、本体が取り込んだ 8 本を伏せた残り）。
...
2つ踏んでいる ―「古いプロキシのファイル（22行）が同梱ルール（29行）より優先され、
新しいルールが黙って使われない」「場所が分からないとリクエストごとに数十回 `stat` を
叩くので間引きの仕組みが要る」。固定にした結果、後者は仕組みごと消えた。

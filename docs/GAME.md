...
> （VERIFICATION.md §3.8.1）。**`102_` も 2026-08-10 に降ろした**（`superseded:
> main_023`。こちらは発生源が本体で直っていることを実経路で確認できている。
> §1.6）。この段落の「置いておけばよい」に当てはまるものは、いま残っていない。
...
- **`102_` は 2026-08-10 に `superseded: main_023` で降ろした**（`103_` は main_024 で
...
- クラウド LLM 経路はプロンプト系 MOD が**素通りになることが確定した**
  ...
  実害あり。**プロンプトを `LlamaCppClient.chat` で書き換える MOD は、クラウドでは
  丸ごと素通りする**（同じ MOD でも `llm_manager:quest_referee*` に仕掛けた側は効く
  ので、1本の中で半分だけ効く形になりうる）。`301_` は `LlamaCppClient`
...
（開発中の MOD で実証済み。TECH.md §2.6）。ゲーム側のコードにもセーブにも
触る必要が無い。
...
- **クラウドと分かった時点で、ローカル専用の保留は降ろす**
  （2026-08-10。オフライン検証のみ・実機未確認）。
  送信モジュールが1つだけ載るという上の性質をローダ側の判定に使っている
  （`llm.is_cloud_runtime()` / TECH.md §3.4）。入れる前は `llama_cpp_runtime_completion`
  宛ての保留 14 件が永久に残り、GUI が「段階適用の途中」と言い続けていた
...
- `output_data/` の記録は `LlamaCppClient.chat` より上流で取られる。
  ...
  内容は記録に一切映らない。2026-07-28 に、`chat` で書き換えている MOD を
  「効いていない」と誤判定しかけた。同じ記録の中で、実機で効いていることが確定している `105_` の

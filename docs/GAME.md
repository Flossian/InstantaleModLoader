...
（ただし `102_` / `108_` はその後どちらも `superseded` で降ろした。§1.6 / §3.8.1）。
...
- `output_data/` の記録は `LlamaCppClient.chat` より上流で取られる。
  ...
  `chat` に仕掛けた MOD の判定は、その MOD 自身のログで行うこと（§2.43 も参照）
...
- 魅力は低いほうから ひどく醜く思っている / あまり好みではない / 魅力を感じている /
  ...
  （実機で読み直したところ**閾値は 2 / 5 / 11 / 14 で、5〜10 の帯には文が付かない** ＝
  `None` を含めて6段。VERIFICATION.md §3.23）

...
| `118_batch_message_render` | 本文の出し方（逐次／一括）と、読み終わった本文の灰色化 | オフライン検証済（57件）・実機未適用（手元では切ったまま）。既定を逐次表示＋クリックで打ち切りに変えた（一括は `BATCH_MODE=always`）。打ち切りは「`index` ＝ いま何文字目まで出ているか」の推定に乗っており、前後1文字まで見て噛み合わなければ打ち切らずに逐次のまま続ける ＝ 推定が外れても本文は壊れない。灰色化の基準に経過セッション数（`FRESH_MODE=sessions`）を足した。一括表示の前提は実測と一致していて、二度手間を外した後でも1文字あたり 8.1ms（間隔の21%）が残る ＝ 狙いどころは合っている | §2.34 |
...
python tools/test_batch_message_render.py      # 118_  57件

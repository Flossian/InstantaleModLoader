...
| `314_area_move_custom` | エリア移動の日数・馬車の料金・移動中の文言を設定で変える。ボタンに実際の日数が出る | **実機で成立**（2026-08-17、往復4移動）。徒歩 90→14/60・馬車の運賃 1000→500・ラベル表示・`307_` との同居まで確認。馬車の文言と日数14もこの実測で確定し、既定テンプレートを実測文に更新。料金は「1000引いて払い戻す」が紛らわしいという指摘で**前払い調整**（1回の引き落としで設定額ちょうど）に作り替え、実機3回で成立（利用者確認、2026-08-18）。同日、拒否・手持ち不足の素の挙動（`金が足りない...` で中断、ボタンは出る）・既定ラベル `馬車(1000G・14日)` も実機で確認。残るのは「設定額以上・1000G未満」で実際に乗る通しと、文言テンプレートの自作変更だけ（§3.27） | 60件全通 |
| `315_vacation_custom` | 宿の宿泊期間（選択式。週単位の短期滞在と、年齢による加算のON/OFF付き）・部屋の名前・宿代を設定で変える | **実機で成立**（2026-08-18、`2週間`・加算OFF・部屋のリネームで3泊）。ラベル・`period_months` の差し替え・日数の切り詰め（30→14）・文言の置き換え・連泊の積み直しまで確認。実装時に未実測だった4つ（徴収箇所・`quality` の実値・日数送りの経路・`age` の型）も同時に確定。同じ実機で**4つ目の部屋 `犬小屋(0G)`** が見つかり v3 で対応。週単位のときの LLM 描写の食い違い（ゲームがプロンプトに月数を焼き込む）も v4 で対応し、**実機2回目で描写まで揃うことを確認**（`二週間にわたる滞在`）。残るのは年齢加算ONと、犬小屋のリネーム・宿代変更の実機（§3.28） | 95件全通 |
...
**何を足したか。** タイトル画面（`scripts.hud.hud_start:StartScreen`）の隅に
`modloader v1.6.0` のラベルを1枚置く。ゲームの値は何も読まず、何も書き換えない。

**実機で確認済み**（利用者、2026-08-13。§2.47）。表示・タイトルへ戻ったときの
組み直し・窓の追従・タイトルの操作、いずれも問題なし。未確認は残っていない。

根拠として押さえてあるもの:

- 対象の実在（`out/recon/targets.txt` 1105 行の
  `scripts.hud.hud_start:StartScreen.__init__`）と、その画面が Kivy の `Screen`
  ＝ `RelativeLayout` であること（`out/recon/modules.json` の `mro`）。`pos_hint`
  で隅に寄せられるのはこれが根拠
- オフライン19件（`tools/tests/test_ui_title_version.py`）。偽の Kivy と偽の
  `StartScreen` で、2つの経路・重ね置き・文字の差し替え・置き場所・フォントを写す
  ところまで通っている

**設計の要点。** 足す経路が2つある。注入は窓が出てから走るので（TECH.md §1.4）、
`__init__` を包むだけでは注入した時点のタイトル画面に何も出ない。`ctx.on_ready` で
`Window` から画面を辿る側と、`__init__` を包む側の両方が同じ関数を呼び、足す前に
自分の印が付いたラベルを外すので、どちらから来ても1枚になる。

**この件でいちばん危なかったのは、画面の子を1枚増やすこと。** HUD では同じことが
アイテムの移動・装備を壊している（§2.33）。タイトル画面には「最初の子」を読む側が
居ないというのが足してよい根拠で、実機の確認はそこを見ていた。

---

### 3.25 ローカルLLMの応答速度（`127_`）: 再現確認済み・余白は未実測（2026-08-13〜14）

**何をするか。** `llama-server` を専用1スロット（`--parallel 1`）で起こす。窓
（`--ctx-size`）は既定では触らない。機構は GAME.md §2.12.2、実測は §2.48。

**確認済み。**

- 書き換えが実際に届くこと（`out\llm_speed.log` の `[REWRITE]`、2026-08-13。
  v2 の `-cpent` 追加もライブのコマンドラインと `/slots` で確認、2026-08-14）
- 速くなること（体感・実測とも。§2.48）
- 効き方の条件分け（SWA の有無 × VRAM の余裕の4条件。§2.48）
- **別セッションによる再現**（2026-08-14。再計算トークン数と KV は完全一致、
  速度は環境差で一律2〜3%低いが比率は保たれる。§2.48）
- 別セッションによる独立再測定で4主張とも再現（2026-08-14、`out\reverify_127\` の
  キット。§2.48 の再検証。`--ctx-size` の振り直し・崖の向こう側の崩壊・窓超えの
  HTTP 400 まで全行再現。アイドル時 VRAM が 150 MiB 増えただけで 26B 統合が
  崖を越えた＝崖の位置は常駐ソフト次第でずれる、の実例つき）

**未確認。**

- **VRAM の余白（決着・2026-08-14）。** 26B・既定でプレイ15分を20秒間隔で実測。
  通常 15,590 前後、画像生成後の定常 15,748〜15,873 で、目安の 16,200 MiB を一度も
  超えず（余白の最小 503 MiB）。画像生成中はドライバが LLM の約3.4 GB を退避させ、
  終わると戻る。§2.48 の観点の追い足し
- **`--parallel 1` の待ち（決着・2026-08-14）。** `216_probe_llm_overlap` の
  プロセス内実測 260 件で判定。真の多重送信は 20 件（7.7%）・深さは常に2まで・
  ほぼ全件が同じ場面の約0.1秒差のペア発火で、直列化による後着の追加待ちは実測
  0.4〜0.9 秒。体感も遅延なし。プロキシログ 6%・`/slots` ポーリング ≤7.5% とも
  一致。残る理論上の最悪は「長い要求（最大 31 秒）の後ろに会話が並ぶ」だが今回は
  未発生。probe は入れたままなので母数は勝手に増え、傾向が変われば読み直す。§2.48
- **応答長の分布（決着・2026-08-14）。** `output_data\` 16185件の実測で、応答は
  中央 124 / p90 336 トークン、プロンプトは中央 1825 / p90 6194 / p99 9595。
  窓 16384 超えも実在。§2.48 の観点の追い足し
- **`--cache-reuse 256` の効き（決着・2026-08-14）。** 効果ゼロと確定（§2.48）。
  そもそもゲーム既定に含まれず、利用者が設定欄へ書き足したもの。有無で差が出ない。
- **SWA のチェックポイント復元（決着・2026-08-14）。** `--parallel` を明示すると
  復元が必要になり、失敗すると4件に1件がフル再計算になる。
  `--checkpoint-every-n-tokens 256` を併せて渡せば素のゲームと同等に戻る
  （VRAM は増えない）。`127_` は既定で両方を渡す。§2.48
- **他の GPU・他のモデル。** 実測は RTX 4070 Ti SUPER 1台のみ。SWA を持たない
  モデルで VRAM に余裕がある場合は効果ゼロと確認済みだが、害が無いことの確認は
  Qwen3.5-9B 1本だけ
- **短縮形の別名（`-c` / `-np` / `-cpent`）の扱い。** MOD は長い旗しか見ない。
  欄に短縮形があると SLOTS=0 が効かない・欄の `-cpent` を尊重できない
  （§2.48 の観点の追い足し）。別名表を持たせるかは判断待ち
- **SLOTS>=2 & CTX_SIZE=0 の窓縮小。** 窓が 16384÷N に縮み、SLOTS=2 では実機
  分布の p99 が超える。mod.json の note に縮む側の警告が無い（§2.48）
- **ゲームの欄剥がしは `-c` にも効くのか。** 剥がしの実測（GAME.md §2.12.1）は
  長い旗 `--ctx-size` だけ。もし `-c` が素通りなら、欄に `-c` を書くだけで MOD 抜き
  でも窓が変わる（後勝ちは実測済み）。実機1回で決まる
- **画像側サイドカー（背景除去）の実機動作（決着・2026-08-14）。** プレイ中に
  背景1枚＋キャラ画像一式（透過処理込み）が正常に生成され、llama-server も同一
  プロセスのまま生き残った。素通しは実機でも無害

### 3.26 アイテム画像の偏り（`128_`）: 実機未確認（2026-08-16 実装、オフラインは決着）

**何をするか。** アイテム画像の選定（`Embedding.get_similar_id:get_similar_embedding_id`）を
包み、「最も似た1枚」固定を「top1 の類似度の 0.8 倍以上・最大10候補の中で、
プロセス内の使用回数が最少の1枚（同数なら類似度の高いほう）」に変える。乱数は無い。
併せて、素の埋め込み辞書に無い305枚（document 105 / creature_part 171 /
mushroom 28 / accessory 1）へキャプションを付けて作った追加辞書を同梱し、
実行時に合成する。

**オフラインで確認済み（2026-08-16）。**

- **偏りの実測。** 5世界のセーブを復号して494個を集計（テストワールドの
  プレイヤー所持品と「ヴェスティア - コピー」は除外）。potion は52個中35個
  （67%）が同じ1枚、drink 38個中12個、body_armor 43個中11個。medicine は
  そもそも画像が4枚しかない
- **原因の切り分け。** 生成ログの `item_appearance`（英語一文）が定型的で、
  決定的 argmax が同じ画像に落とすのが主因。画像埋め込みの重心に近い「ハブ画像」
  仮説は否定（最頻画像は重心類似ランクで 61枚中56位など軒並み下位）。
  完全同一文はまれ（potion 72件中0、small_weapon 84件中6）で、微差の文が
  top-k 集合を少しずつずらすため、候補を広げる方式が効く
- **埋め込みの再現。** 同梱モデル（all-MiniLM-L6-v2）を numpy で再実装し、
  実セーブで選ばれた画像の argmax 再現率 109/114（96%。不一致5件は同名
  アイテムの再生成によるキャプション取り違えの可能性が高い）
- **選定方式の通しシミュレーション。** 実クエリ2,471件（生成ログ順、
  テストワールド除外）で MOD のロジックをそのまま回した。top1 シェア／使用種類:
  potion 47.2%/15種 → 8.3%/33種、small_weapon 26.2%/24種 → 4.8%/45種、
  clothing 34.2%/14種 → 9.6%/22種、creature_part 9.0%/121種 → 1.2%/234種。
  追加辞書の画像は creature_part で 420件中198件、document で 72件中16件選ばれた
- **追加辞書の自己検索。** 305キャプション全件が、合成辞書の中で自分の画像を
  top1 に引く（305/305。新規分が既存に埋もれず、実際に選ばれ得る）
- **document フォルダの実態。** 110〜214 の追加画像の大半は紙物でなく
  鍵・フラスコ・ランタン等の小物だった。キャプションは見えるものに正直に
  付けてあるので、紙らしい外見文からは引かれにくい（document で16件に
  とどまったのはこのため）

**実機1回目（2026-08-16〜17、フックの発火だけ確認）。** アイテム生成9回で
フックは発火したが、全件が `TypeError`（`ctx.mod_dir` は apply() の外では
None になる仕様。ローダの docstring に明記があるのに控え忘れた）で
safe=True の素の挙動へ落ちていた。ゲームは落ちず、選ばれた画像も素のまま。
apply() 内で控える形に修正済み（2026-08-17）。**選定・追加辞書はまだ一度も
実機を通っていない。**

**実機2回目（2026-08-17、選定経路が成立）。** 修正後の注入で店の品揃えを生成。
8件すべてが `out\item_image.log` に記録され、エラーは無し（`safe hook failed` は
前回の9件から増えず）。類似度の足切りも設計どおりで、近傍が密な品種は
`10 qualified`（small_weapon / plant / gem / relic）、まばらな品種は自動で絞られ
`1 qualified`（clothing / drink / medicine）、potion は `5 qualified`。8件とも
`rank 0`（素と同じ絵）だったのは、8件が全部別品種で各品種の1個目だったため
（使用回数はセッション内で数えるので、1個目は必ず素と同じ。仕様どおり）。

**実機で確かめること（未確認）。**

1. ~~フックの発火~~（実機2回目で決着。`pool` / `chose` とも出ている）
2. **返り値キー → 画像パスの解決（最重要）。** キーから `image_src` を組む処理は
   コンパイル済みで読めておらず、`<folder>/<キー>.png` と推測している。とくに
   creature_part の追加分はキーが日本語ファイル名（`モンスタ－・動物の素材_36` 等）。
   追加辞書の画像が選ばれた回（ログの `chose モンスタ－…` の行）に、アイコンが
   正常に表示されるかを見る。**壊れたら設定「同梱の追加辞書を使う」を OFF**
   （均しだけは効き続ける）
...
**何をするか。** エリア移動の確認画面のラベル
（`AreaMoveCofirmation.update_button_display` の後で `text` だけ書き換え）、
移動中の日数（`AreaMoveManager.execute` の窓の間だけ `elapse_days` の引数を差し替え）、
移動中の文言（同じ窓の `add_text` をテンプレート置換）、馬車の料金（`execute` の
前後の所持金の差分を設定額に直す。`901_` の決済補正と同じ方式）。

**実測済みの土台の上に建っている部分**（GAME.md §2.18）: 徒歩の日数 90、
`mode` の実値 `'on_foot'` / `'coach'`、徒歩の出発・到着の文言、`elapse_days` を
減らせば実際に短く着くこと（`307_` が確認済み）。
...
**実機で確かめる手順。** デバッグモードで `217_probe_area_move` を入れて
普通に遊ぶだけ（probe は `314_` / `307_` より外側なので、ゲームの生の値が録れる）:

1. 馬車で1回移動する → `out\area_move.jsonl` の `move` の行。
   `texts`（馬車の出発の文言）・`elapse_days_calls`（馬車の実日数）・
   `gold_moved`（1000 なら「徴収は execute の中」が確定）
2. 手持ちを 1000G 未満にして確認画面を開く → `confirm` の行。馬車のボタンが
   並ぶか。押したときに `restriction` / `move` のどちらへ行くか
3. `314_` の料金を変えて馬車に乗る → `out\area_move_custom.log` の `fare:` の行が
   対で出る（`gold ... before the game charges`＝前払い調整 →
   `charged ... in one deduction`＝1回の引き落とし）
4. 徒歩の日数を変えて移動 → ボタンの表示（`label:`）と `days:` の行、
   実際の暦の進みが一致すること

**実機1回目（2026-08-17、往復4移動で主要経路が成立）。** `out\area_move.jsonl` に
confirm 6件・move 4件（徒歩2・馬車2）。

| 見たもの | 結果 |
|---|---|
| 徒歩の日数 90 → 14 / 90 → 60 | 成立。ボタンの表示（`徒歩(14日)` / `徒歩(60日)`）と `elapse_days` に渡った数が一致 |
| 馬車の運賃 1000 → 500 | 成立。ゲームは 1000 を引き、`fare:` の精算で差し引き 500 に（`gold 23288 -> 22788`）。**ただし引いてから返す方式は、支払い直後の所持金表示が「1000引かれた値」のまま残り、利用者には「1000減った」に見えた**。同日、引き落とす前に差額をずらしておく**前払い調整**（1回の引き落としで設定額ちょうど）へ作り替えた。実機は次回 |
...
**実機2回目（2026-08-17〜18、前払い調整が成立・利用者確認）。** 馬車3回とも
...
の対が残り、引き落としは1回・設定額ちょうど。文言（`500ゴールドを支払った`）と
画面の所持金の動きが一致することは利用者が確認（2026-08-18）。`WARN fare:` は無し。
馬車のボタンに常に日数を出す表示（2026-08-17 追加）も、料金を変えた設定では
`馬車(500G・14日)` が実機に出ている（`label:` の行）。
...
1. 手持ちが「設定額以上・素の運賃(1000G)未満」で実際に乗る経路（前払い調整で
   残高チェックを越える域。仕組み上は3回目の実測で裏が取れているが、通しは未踏）
2. 文言テンプレートを自分で書き換えた表示（既定テンプレートの料金追随は
   `500ゴールドを支払った` まで画面で確認済み）

### 3.28 宿の宿泊期間・部屋・宿代（`315_` / `218_`）: 実機未確認（2026-08-18 実装・同日 v2）

**何をするか。** 部屋と `宿泊する(Nヵ月)` のラベル
（`InstantaleApp.refresh_choice_buttons` の前に `text` だけ書き換え）、
宿泊期間（選択式。月単位は `DisplayVacationChoice.__init__` に渡る
`period_months` を差し替え。**週単位は months=1 を渡して、宿泊のマネージャの
`execute` の窓の間だけ `elapse_days` の合計を日数の予算で切り詰める** ―
`314_` の予算方式）、年齢による期間の加算（`app.player.age` を読む。
30代+1・40代+2・50代以上+3、上限 4週間/6ヵ月、ON/OFF可）、
宿代（`VacationStartManager.execute` の前後で前払い調整＋精算。`314_` の馬車と
同じ方式）。

**実測済みの土台の上に建っている部分**（GAME.md §2.17）: 宿泊の
`process_choice` の並び（`DisplayVacationChoice` → `VacationStartManager` →
活動 → `VacationEndManager`）、部屋3つのラベルと料金（10/100/1000G）、連泊
`まだ宿泊する` の spec が `VacationStartManager` であること、各クラスの
`__init__` の署名（targets.txt）、実セーブの Character に `age` が居ること、
1ヵ月=30日（`徒歩(3ヵ月)`=`elapse_days(90)`）。素の期間が年齢で 3〜6ヵ月の
変動式であることは仕様情報（2026-08-18。実測は 3 と 4 のみ）。
...
**実機で確かめる手順。** デバッグモードで `218_probe_vacation` を入れて
普通に泊まるだけ（probe は `315_` より外側なので、所持金と日数はゲームの
生の値が録れる）:

1. 素の設定のまま1泊する → `out\vacation.jsonl` の `room_choice` の行に
   部屋ボタンの `args`（`quality` の実値）、`execute` の行の `gold_moved` に
   徴収の箇所と額。`宿泊する(3ヵ月)` の spec の args と `player.age` は
   `display_vacation_choice` の行（年齢と `period_months` の対応表になる）
2. 宿代を変えて泊まる → `out\vacation_custom.log` の `price:` の行が対で出る
   （`gold ... before the game charges`＝前払い調整 →
   `charged ... in one deduction`＝1回の引き落とし）
3. 月単位の期間にして泊まる → ボタンの表示（`label:`）・
   「Nヵ月泊まることにした。」・`218_` の `elapse_days_calls` が同じ月数で
   揃うこと（年齢の加算が乗った値で）
4. 週単位の期間にして泊まる → `days:` の行（30 → 7 のような切り詰め）と、
   文言の置き換え（`text:` の行）。`218_` 側の暦の進みが実際に7日/14日で
   あること。`WARN stay: ... no elapse_days call` が出たら日数送りが別経路
5. 連泊（`まだ宿泊する`）→ 2周目の `execute` の行でも所持金が動くか
   （動けば再徴収あり。`315_` は2周目も設定額に直し、週単位の予算も積み直す）
...
**実機2回目（2026-08-18 10:02〜10:03、v4。設定は1回目と同じ `2週間`・加算OFF・
簡易寝台→「相部屋」で3泊）。** v3 / v4 で足した2つがどちらも成立:
...
| 錨の当たり方 | `prompt at ... kept as is:` は**1行も出なかった** ― この回に通った休養・社交・社交の解決のプロンプトはすべて `の宿泊` / `の滞在` の錨に当たっている（訓練・労働・物乞いは未通過） |
...
> 218_ の記録に `1ヵ月泊まることにした。` が残るのは正しい。probe は `315_` より
> **外側**なので、書き換え前のゲームの生の文言を録る（TECH.md §3.2.2）。
> 画面に出るのは `315_` が書き換えた `2週間泊まることにした。` で、
> `vacation_custom.log` の `text:` の行がその対を残している。

**残る未確認。**

1. **年齢による加算の実機**（2回とも加算OFF で遊んでいる）。とくに月単位の
   加算でゲームと同じ月数になる場合に「何も動かない」こと
2. 犬小屋（`kennel`）の実機 ― リネームと、**タダの部屋を有料にしたとき**の
   前払い調整（表示と認識は実機で成立。オフラインでは引き落としも通っている）
3. 宿代を変えたときの `price:` の対（前払い調整 → 1回の引き落とし）。
   `314_` の馬車では実機で確認済みだが、宿では未確認
4. LLM の描写の書き換えは**休養・社交・社交の解決で成立**（実機2回目）。
   **訓練・労働・物乞いのプロンプトはまだ通っていない** ― 錨（`の宿泊` /
   `の滞在`）から外れた言い回しなら `prompt at ... kept as is:` にその一節が
   残るので、出たら錨を足す。`out/events.log` は長い行を途中で切るため、
   実文言はこの口からしか読めない

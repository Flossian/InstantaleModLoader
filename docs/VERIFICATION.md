...
| `123_fix_new_character_level` | 新規作成したキャラが経験値0のままレベル60で始まる（本体が `instantale.py:876` で 60 を渡している） | オフライン検証済（28件全通・`tools/test_new_character_level.py`）・実機未確認。原因は `214_` の実機1回目で確定済みなので、残っているのは修正の効きだけ。最初の新規作成で `out\new_character_level.log` に `experience_level 60 -> 1` が出て、人物欄が `1レベル`・HP が `耐久 × 4.8` になることを見る。効く条件を「レベルだけが他と食い違っている」に絞ってあるので、本体が直れば自動で無効になる（そのとき記録は1行も増えない） | §2.36 |
...
| `313_event_ability_check` | 2系統の成否判定（クエストのフィールドイベントと、街・会話の自由入力）に能力値の加点を入れる（15 から3点ごとに +4%、28〜30 で +20%）。自由入力側は参照能力値が無いので行動文から推定する（実機で推定 6/6 が妥当・1問あたり 0.8〜1.6 秒。§3.18） | オフライン検証済・**実機未確認**。確率に付く負の差の正体は未特定だが、上限式（`credibility*10+20`）が単調なので推測せずに済む形にしてある。§2.37 / §3.17 / §3.18 | 65件全通 |
...
| `213_probe_npc_memory` | `311_npc_profile_memory` とゲーム自身の記憶（会話終了時の要約 → `memory`・retrieval）がどれだけ重複しているか。`memory` / `life_log` / `relationship` / `knowledge` の実体、会話プロンプトへの載り方（毎回全文か一部か）、プロンプト内の重複量の3点を測り、311_ の対応（抽出プロンプトでの棲み分け・抽出タイミングの変更）を決める材料にする | 主要部は決着（2026-08-08・会話2回実測）。会話終了時に `conversation_resolver` が `current_log`（累積形の要約の追記）と `relationship` を書き、`current_log` / `relationship` / `profile` / `personality` は**毎回全文**プロンプトに載る。`311_` の注入と同じ事実が3箇所に並ぶことを確認。3回目の実測で `current_log` は一時置き場と判明 ― **日付の変更**で `life_log` へ移り（`str()` の丸写しで無損失）、以後は `life_log` として毎回全文載る。**情報が落ちているのは移送ではなく `resolver`** で、出力量が会話の長さにほぼ依らず一定（8ターン・messages 5,597字の会話→118字）。書き直しのたびに固定の短さへ圧縮されるため、この経路の情報は単調に減る。**要約が走らない実例を2件観測**（(1) 2026-08-08 15:30〜: 会話中に行動処理 `master_ai_facilitator_from_conversation` へ進んだ後、`resolver` 不発のままセッション終了。会話の内容は `311_` の毎ターン抽出だけに残った ― 取りこぼしの保険が実際に効いた形。(2) 同 16:18〜: 会話を閉じないままタイトル/別セーブのロードへ抜け、要約されずに消えた）。resolver の呼び出し元も確定（`ConversationEndManager.execute → finish_conversation → resolve_conversation`。GAME.md §2.25）― 要約の入口は終了ボタンの manager 1つだけで、そこを通らない抜け方は全て素通りする構造。抜け方ごとの網羅（戦闘移行・依頼受注・勧誘成立・MOD による閉じ）は未了。probe v4 で世界跨ぎの id 衝突を修正（ハンス(9)とシエラ(9)を同一人物として diff していた）。ゲーム側の新しい重複も2種確認: `life_log` に同一要約の5連コピー（エドガー・911字が毎回全文載る）、`master_ai_from_conversation` のプロンプトにプロフィール行 x4（損262字）。**未実測**: `memory`（5鍵 dict）と `knowledge` の更新契機（会話と時間経過では動かず）。この計測を受けて2つ入れた ― `311_` v4（`facts` を抽出に差し戻し、書き直しで落ちた事実を戻す。オフライン186件全通）と、`111_` の置換ルール1行（`resolver` の「一切情報を損なわないが簡潔に」を、削ってよい対象を描写・情感に限る形へ。実物のプロンプトに当たることは `output_data` のダンプで確認済み・**要約が実際に濃くなるかは実機で未検証**）。GAME.md §2.25 に記録。照合は v2 で葉ベースに直した（空 `[]` の偽陽性と JSON 断片の偽陰性を1回目で踏んだ） |
| `215_probe_event_roll` | ミニイベントの確率に付く負の差（-2〜-40）が何に連動するか。`Character.calculate_attribute` が判定の窓の間に呼ばれるか（＝能力値が判定経路に入っているか） | 仕掛けただけ・**未実施**。offline の材料は出尽くしている（§2.37）。手順は §3.17 |
...
| バグ | 状態 |
|---|---|
| （無し） | `OSError: [WinError 123]` は `110_fix_character_name_path` で決着（実装 §2.14 / 実機 §2.15）。新規キャラのレベル60（§2.36）は `123_fix_new_character_level` で修正済み（実機未確認） |
...
### 2.36 新規キャラが最初からレベル60（2026-08-08、原因確定・ゲーム本体）
...
HP のほうは `max_hp ≒ 耐久 × レベル × 0.867`（レベル1で `耐久 × 4.8`、
レベル60・耐久30 で 1560、耐久16 で 832）。どちらも式は確定していないが、
...
- **修正 MOD は実装した**（`123_fix_new_character_level`、オフライン28件全通・
  実機未確認）。`Character.__init__` の引数を差し替えてレベルを 1 に戻す
  （`max_hp` も `__init__` の中でレベルから計算されるので同時に直る）。
  効く条件を「新規開始だから」ではなく**「レベルだけが他と食い違っているから」**
  に置いたのが要点 ― `start_game` が続きからの読み込みにも使われていた場合に
  巻き込まないため、および手でレベル60・経験値0に編集したセーブ（体力上限が
  レベル相応）を巻き戻さないため。既に**このバグで**保存されたセーブは
  既定では直さず、記録に警告だけ出す（設定 `REPAIR_LOADED` で直せる）
- ゲームの版がいつ変わったか（7月22日の正常な新規開始と 8月8日の間）。
  本体へ報告するなら添えたい

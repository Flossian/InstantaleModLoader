...
判明した真の原因と、`alias_scan` が無ければ素通りしていた件は TECH.md §5.1。
...
`207_` の計測、実プレイ12戦で原因が確定した（詳細は TECH.md §7.11）。決め手になったログ:
...
「戦闘中なら何もしない」の条件に引っかかって毎回黙って降りていた（TECH.md §7.10）。
...
残るのは運用感の調整だけ（`CHANCE_BY_TYPE` と `COOLDOWN_MOVES` を実プレイの頻度に合わせる）。
...
いずれも「セーブに出ている形＝実行時の形」と決めつけたのが原因。結論は TECH.md §7.8 / §7.2。
...
> **テストのクラスをグローバル名から派生させないこと**（TECH.md §5.3）。
> 派生元は `BASES` の表に控える。
...
進行ループと `Literal` の推移は TECH.md §7.9 に移した（ゲームがどう動いているかなので）。
...
- **`output_data/` に書き換えが映らない。** 保存は `chat` より上流なので、記録された
  プロンプトは書き換え前のもの。「効いていない」と読みかけた。TECH.md §7.12 に
...
| `sweep after ...` が1行も出ないまま `mixer` が増える | 後始末が走っていない。起点の条件を疑う（前回は `in_battle` の居残り。TECH.md §7.10） |
...
- フラグ名が意味するとおりに動くとは限らない（`in_shopping` と同じ形。TECH.md §7.6）
...
（3つのうちどれで `bgm` が確定するかはコンパイル済みのため未確定。TECH.md §7.11）。
...
4. **多重起動抑止 / `--parallel 1`**（後日対応と指示済み。TECH.md §7.12）
5. **ネイティブクラッシュダンプ 7件**（未着手領域。TECH.md §11）

...
この確認の副産物が2つあり、どちらも他の mod の土台になった。スレッドの扱いが確定した
こと（`process_choice` はメインスレッド、`execute` は別スレッド）と、`in_shopping` が
当てにならないと判明したこと（素の移動38回すべてで True。不発39件のうち38件がこれだった。
GAME.md §2.6）。
...
その本来の行間そのものが広いこと（Kivy の既定 `line_height=1.0` なら
...
3. `facility_move_to` / 空 `Literal[]`。計測は仕掛け済みで、発生すれば自動で原因が
...
二重に適用しても結果が変わらないので無害で、漏れの検出器として使えるという判断。多重起動抑止だけは

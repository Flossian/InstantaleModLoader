...
| `305_mini_quest` | 戦闘を伴わないミニクエスト（採集・救助・偵察） | **デバッグ用に降格（2026-08-06、ユーザー判断）。** 品質が目標に届かないため、既定では読み込まない（`mod.json` の `"debug": true`）。依頼の中身と非戦闘の進行までは実機で成立、**終わり方が定まらない**（実機4回。§2.19〜§2.23）。最後の修正（撤退の判定を戻り値ごと差し替え）は**未検証**。再開するなら §3.10 から | 115件全通。うち4件は `output_data/` の実プロンプトとの突き合わせ（進行 600 / 生成 133。2026-08-09 時点。**記録は `111_` 適用後**なので §2.43 を読むこと） |
...
python tools/test_mini_quest.py       # 305_  115件（うち4件は output_data/ の実プロンプトと突き合わせ）
...
`tools/test_mini_quest.py` の「進行プロンプト全件に目印が在る」が、記録済みの
実プロンプト 600 件のうち **580 以降**で落ちた。落ちた目印は `305_` の
`- retire_from_the_quest:` の1行。
...
> ゲーム内の動作は壊れていない。`305_` はプロンプトを組む時点（`quest_referee*`）で
> 当たり、`111_` は送信の直前で当たるので、`305_` が見るのは今も素の文である。
> 壊れていたのは**照合に使っている資料の性格**だけ。

**対処（`305_`）:** 行末に依存しない形へ変えた。`REFEREE_LINE_SWAPS` を新設し、
...
`305_` 以外に影響は無い（同梱ルール 30 本と `305_` の目印を突き合わせた結果、
当たるのはこの1本だけ）。
...
先に `python tools/test_mini_quest.py`（115件）を通しておくこと。設置・押下・生成・

...
配布物に入れない MOD は `900`〜`999` で採番し、
配ると決めたときに番号帯に応じた正式な番号へ振り直す。

**帯が意味するのは「配らない」の1つだけで、理由は問わない。**
大半が作りかけなので名前は「開発中」のままにしてあるが、実際には3通りが同じ帯に居る。

| 配らない理由 | 例 |
|---|---|
| まだ実機で確かめていない・作りかけ | `905_fix_world_overview`（世界を1つ生成すれば決着する） |
| 完成しているが、手元の事情に閉じていて配る意味が無い | `900_cloud_model_override`（ゲームの選択肢に無いモデルを使う） |
| 直るが、素のゲームでは踏めない（発生源が外の道具に限られる） | `906_fix_new_shop_data`（セーブエディタで足した店。GAME.md §2.28） |

理由ごとに旗を立てないのは、**どれも要求することが同じ1つ**だから
（フォルダ名を変えない限り配布物に入らない）。
理由そのものはその MOD の `DOC.md` の先頭に書く。
...
| ファイル | 使いどころ |
|---|---|
| `out/recon/targets.txt` | **これが本命。**`module:qualname(signature)` 形式で 1,585件 |
...
> 番号順は `after`/`before` を3箇所で破る（`117`→`112` / `213`→`311` / `215`→`313`）。
...
ここを読まないと「28個中3個が `apply-error`」を GUI に出せない。
...
**クエストの格納先**（GAME.md §2.9。読むのはどちらでもよいが、書くときは必ず両方）:
...
| 使う側 | 何のために |
|---|---|
| `309_` / `902_` | 自分で組んだ文言を画面に出す直前に `rewrite_coins` を通す |
...
| **ゲームが自分で作った値には、既に自分の細工が乗っている** | `318_`（上げた帯からゲームが新しい依頼を作るので、初見の素の値は「いまの値 - いま乗せている量」で控える。素通しにすると生成のたびに二重に乗る） |

...
新しい主人公に引き継がれる（`331_` の実機 2026-09-14）。
...
| セーブと同じ寿命のもの（建物・主人・契約・帳簿） | `playthrough_key`（`modfacility` / `modnpc` / `330_` / `331_`） |
...
- 建物や主人の id は周回をまたいで重なる（`<土地>-<番>`）。登録簿はプロセスで1つなので、**ロードのたびに層を積み直す**（`331_` の `keepers_registered.clear()`）。`modfacility.forget` は控えの写しも捨て、新築は `spawn(fresh=True)` で写しを使わない（残すと新しい主人公の宿が前の主人公の宿の名で建つ。実機 2026-09-14）
...
> **前の版の層（`notes` / `fields`）が残り続ける**（実機 2026-09-13。`331_` の主人に足したはずの
...
> （`331_` の店の主人に品が7つ在っても控えは空だった。2026-09-14）。
...
> `world.characters[id]` を差し替える（`331_` の実機 2026-09-13。`config['difficulty_level']` が変わり
...
> （`331_facility_investment` の実機。2026-09-13）。
...
（`331_` の宿屋 2026-09-14。`330_` は旗を先に落としていたので踏まなかった ― 同じ作りを2本が別々に持っていた）。
...
（`331_` の実機 2026-09-14。入口に戻ったのに店の絵のままだった）。
...
（`331_` の道場で実セーブ 2026-09-16。ここだけは立ち位置も選択肢も替えず、絵だけ替える）。
...
`無料で泊まる` / `会話する` / `売上を受け取る` / `出る` のように会話が真ん中に挟まっていた（2026-09-20 の実機）。直した後を宿・店・闘技場・自宅の4種類で確かめた（VERIFICATION.md §3.63 #7d）。
...
`_already` で素通りして後ろに残ったままだった（実機 2026-09-20。宿泊を終えた直後の自分の宿で `会話する` → `無料で泊まる` の順）。
...
居た場所に戻すのが正しい（`331_` の実機 2026-09-14）。
...
> オフラインは `tools\tests\test_modfacility.py` 157件。
>
> **最初の使い手は `330_real_estate`（版20）。** 2本目は `331_facility_investment`（版1）で、
...
**値段を書くのはローダ1枚。** MOD は「いくらにするか」だけを答える。

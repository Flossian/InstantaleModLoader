...
| 引数 | |
|---|---|
| `tag` | 時刻と本文の間に**そのまま**挟む（区切りの記号も込みで渡す）。角括弧の形（`"[BGMFIX]"`）と区切りの形（`LOG_TAG + ":"`）が両方使われていて、どちらも実機の記録として GAME.md / VERIFICATION_LOG.md に引用されている。体裁を揃えると、その引用が次のプレイのログと一致しなくなる |
...
| ゲームの文言を持たずに、ゲームの分類を書き換える | `125_`（段の並び（`document_emotion_scores_new` が返す文の梯子）を注入後の最初の呼び出しで総当たりして覚え、以後は**位置**で扱う。文字列を1つも持たないので言語設定にも版差にも巻き込まれず、並びが読めなければ素通しに倒れる） |
| 引数に居ない相手を、呼び出し元のフレームから拾う | `125_`（`document_emotion_scores_new` は NPC を受け取らない。呼び出し元を遡り「`relationship` に `"player"` を持つ `Character`（プレイヤー自身を除く）」を探す。**能力で選ぶ**ので型名を知らずに済む。見つからなければその要素だけ効かせない） |
...
| 注入した時点でもう組み上がっている画面へ足す | `126_`（`StartScreen.__init__` を包むだけでは、目の前のタイトル画面に何も出ない（注入は窓が出てから走るため。§1.4）。`ctx.on_ready` で `Window` から画面を辿って**その場で足す**側と、`__init__` を包んで**次から足す**側の2つを持ち、同じ関数を呼ぶ。足す前に自分の印が付いたものを外すので、どちらから来ても1枚） |

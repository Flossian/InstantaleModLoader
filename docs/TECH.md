...
（`315_vacation_custom`）と、その期間に合わせたい MOD（`914_real_estate` の自分の家の滞在）がある。
...
| 決まり | 理由 |
|---|---|
| **既定はゲーム自身の式**をローダが持つ | 変える MOD が無くても答えが出る。式の写しを各 MOD に置かない（914 と 315 が別々に持っていた） |
...
| MOD を外したら `forget(owner)` | 置きっぱなしの答えが残らない |

**借り物は最後の手段にする。** ゲームから実際に観測できる値
（`914_` なら宿屋の部屋の選択肢の spec に載る月数）が手に入ったら、そちらを優先する。
窓口は「まだ観測できていない最初の1回」のためのもの。

> 最初は `sys` の属性を MOD どうしで直接共有していた（`_instantale_vacation_length`）。
> 規約上は許される形（`shares` で断る）だが、配布済みの 315 が開発中の 914 のために
...
| `TRAINING` | `days_per_year` / `course_years` / `activity_years` | `fn(app)` | 1年＝365日、開始時3年、活動は 1/2/2/3年（`231_probe_training`。GAME.md §2.17） | まだ無い |
...
シムで足りないのは**項目が表の形をしているとき**。`915_facility_investment` は種類6×3項目＋共通5の 23 項目で、
...
新しい主人公に引き継がれる（`915_` の実機 2026-09-14）。

```python
key = state.playthrough_key(app)                     # "灰の街×ミツバ"
key = state.playthrough_key_of_dict(save_data_dict)  # `World.__init__` の中ではこちら
```

| 何を持つか | 鍵 |
|---|---|
| 世界ごとの設定（BGM・通貨の単位・道） | `world_key`（周回を跨いで残す） |
| セーブと同じ寿命のもの（建物・主人・契約・帳簿） | `playthrough_key`（`modfacility` / `modnpc` / `914_` / `915_`） |

- 主人公の名は `save_data_dict["player_data"]["name"]` → 実行時の `app.player.name` の順。読めなければ世界名だけ（前と同じファイル）
- **`World.__init__` の中では `app` の辞書も `player` もまだ前の周回を指していることがある。** 引数の `save_data_dict` を `playthrough_key_of_dict` に渡し、建て直しの間はその鍵を持ち回る（`modfacility` の `_KEY_OVERRIDE_ATTR`、`914_` / `915_` の `state["key_override"]`）
- 建物や主人の id は周回をまたいで重なる（`<土地>-<番>`）。登録簿はプロセスで1つなので、**ロードのたびに層を積み直す**（`915_` の `keepers_registered.clear()`）。`modfacility.forget` は控えの写しも捨て、新築は `spawn(fresh=True)` で写しを使わない（残すと新しい主人公の宿が前の主人公の宿の名で建つ。実機 2026-09-14）
- 同じ名前で作り直せば前の周回を引き継ぐ（本人の判断 2026-09-14。セーブに周回の id は無く、`original_ability_scores` も `age` も遊んでいる間に変わる）
...
> 以前は誰の積み直しでも捨てていて、`229_` が施設の主（`915_` の主人）に被せるたびに
> 参照が落ち、`place` が `.location` を据えられずに会話の一覧から消えた（2026-09-13）。
...
> **前の版の層（`notes` / `fields`）が残り続ける**（実機 2026-09-13。`915_` の主人に足したはずの
> 出資者の一文が、注入し直しても頼み文に出なかった）。
> それでもログを流したくない MOD は、`apply()` ごとに空から始まる集合で1度だけ積めばよい（`915_`）。
...
> 相手の居ない残骸だけが並ぶ（実機 2026-09-14。「NPC が消えた」）。
...
> オブジェクト ― どちらも JSON に落ちないので、素直に控えると持ち物が丸ごと消える
> （`915_` の店の主人に品が7つ在っても控えは空だった。2026-09-14）。
> 控えるのはセーブと同じ12項目の辞書、戻すのはゲーム自身（`generate_item_from_dict`）。
> `Character.__init__` は `inventory` を受けないので、組み直した後に入れ直す。
> この知識は `items` に1か所だけ置く（`914_` の保管庫もそこを呼ぶ）。

> **名簿の実体が真実。** 本体は詳細生成（会話の直前）の後に `Character` を作り直して
> `world.characters[id]` を差し替える（`915_` の実機 2026-09-13。`config['difficulty_level']` が変わり
...
> **`place` には `world` を渡す。** `World.__init__` を包んでいる間は `app.world` がまだ前の世界で、
...
> （`915_facility_investment` の実機。2026-09-13）。
...
**選択肢（`buttons` / `buttons_backup` / `buttons_backup_for_shopping`）は落とさない**（2026-09-14）。
...
（部屋選び 2026-09-13、会話の最中と会話相手の一覧 2026-09-14。道には判定が1つも無かった）。
**MOD は自分の進行中の旗（宿泊中・滞在中）で画面を判断しない。** ゲームは場面を終える処理の**中で**
施設の画面を組み直し、その時点では MOD の旗がまだ立っているので足されず、終えた後に組み直しは来ない
（`915_` の宿屋 2026-09-14。`914_` は旗を先に落としていたので踏まなかった ― 同じ作りを2本が別々に持っていた）。
...
（宿泊の後のロードで踏んだ。2026-09-14）。宿泊の等級の部屋の絵は建物の絵のうちなので、終えた後もそのまま残す。
...
| 文言 | 塗り直しのたびに層の今の文言で更新する（既に在るボタンは差し替えずに文言だけ）。売上の額のように押した後に変わる文言のため（実機 2026-09-14） |

**ゲームは立ち位置に書く id を途中で切ることがある。**
`mod:915_…:3-1` の中で保存したら `player_data["location"]` が `'mod'` だけになっていた
（実機 2026-09-14。同じセーブの `game_variables` には切れていない id も在ったので、切るのは立ち位置の書き手だけ）。
...
（`915_` の実機 2026-09-14。入口に戻ったのに店の絵のままだった）。
...
（`915_` の道場で実セーブ 2026-09-16。ここだけは立ち位置も選択肢も替えず、絵だけ替える）。
...
（実機 2026-09-14。カスティアは入口・区画・役場・宿を1組ずつ持つノードを2つ持っていた）。
ノードどうしは繋がっていないので、
...
項目ごと捨てると、実機の闘技場のように試合の後の `config` が丸ごと消える（2026-09-13）。
写しは保存の前に外れて後で戻り（`hide` / `restore`）、世界を読み直すときも `forget` の前に外れる。
...
写しを置く建物は要るものだけにする（`915_` は闘技場だけ。宿屋は実体で足りた）。

**会話や部屋選びの最中でも中のまま。** ゲームは会話の途中を保存して再開できるので、
居た場所に戻すのが正しい（`915_` の実機 2026-09-14）。
...
**背景は MOD が描かず、頼みもしない。** 建物へ入る移動でゲームが
...
（`915_` の店と宿で絵ができた）。ロードは保存した絵をそのまま出し、宿泊はゲームが等級の部屋の絵に替える。
...
> 実機で分かったのは「ノードが2つある街」（上。`914_` の家がどこからも入れなかった）。
> 移す側も実機で通った（2026-09-15。ロード直後に控えの場所へ建て直し → 行けないノードと分かって壊し →
> プレイヤーの側へ建て直し。`914_` の家と `915_` の道場が同じ4行で移り、二重にはならなかった）。
...
> オフラインは `tools\tests\test_modfacility.py` 128件。
>
